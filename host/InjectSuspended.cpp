...
void InjectSuspended(char *exepath, char *dirpath, BOOL bSuspended, BOOL bCommitPage, TARGETMAP *tm, PRIVATEMAP *pm)
{
	...
	while(TRUE) { // fake loop
		...
		OutTrace("injection start address=%#x\n", StartAddress);
		...
		// wait until the thread reaches the entry point
		CONTEXT context;
		...
		if(bSuspended){
			...
			unsigned int i;
			while(TRUE){
				for (i = 0; i < 100 && context.Eip != (DWORD)StartAddress; ++i ){
					// patience.
					Sleep(50);
				...
				if(bFailed) break;
				if (context.Eip == (DWORD)StartAddress) break;
				if(i == 100){
					// requesting a user decision after timeout
					int nMBResult = MessageBoxEx(0, 
						"Start address was not reached. Wait another 5 seconds ?", "Injection", 
						MB_ICONEXCLAMATION | MB_OKCANCEL, NULL);
					if (IDCANCEL == nMBResult){
						OutTrace("Awaiting of injection completion abandoned\n");
						break;
					}
				}
			}
		...
		while (TRUE) // multiple attempts to await injection completion signal
        {
            DWORD dwWaitResult = WaitForSingleObject(hEvent, 5000); // timeout increased to 5 seconds
            ...
            // requesting a user decision after timeout
            int nMBResult = MessageBoxEx(0, 
                "No injection completion signal. Wait another 5 seconds ?", "Injection", 
                MB_ICONEXCLAMATION | MB_OKCANCEL, NULL);
            if (IDCANCEL == nMBResult){
                OutTrace("Awaiting of injection completion abandoned\n");
                break;
