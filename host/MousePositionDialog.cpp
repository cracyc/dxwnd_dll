...
void CMousePositionDialog::OnTimer(UINT_PTR nIDEvent)
{
	...
	// dinput Message viewfinder ...
	if((DxWndStatus.DInputX != (short)-1) && (DxWndStatus.DInputY != (short)-1)){
		x = (DxWndStatus.DInputX * w) / DxWndStatus.Width;
		y = (DxWndStatus.DInputY * h) / DxWndStatus.Height;
		if(x < 0) x = 0;
		if(x >= w) x = w-1;
		if(y < 0) y = 0;
		if(y >= h) y = h-1;
		pt.x = x - 16;
		pt.y = y - 16;
		m_imageList.Draw(dc, PIC_VIEWFINDER_DINPUT, pt, ILD_NORMAL);
	}

	// sdl Message viewfinder ...
	if((DxWndStatus.SDLCursorX != (short)-1) && (DxWndStatus.SDLCursorY != (short)-1)){
		x = (DxWndStatus.SDLCursorX * w) / DxWndStatus.Width;
		y = (DxWndStatus.SDLCursorY * h) / DxWndStatus.Height;
		if(x < 0) x = 0;
		if(x >= w) x = w-1;
		if(y < 0) y = 0;
		if(y >= h) y = h-1;
		pt.x = x - 16;
		pt.y = y - 16;
		m_imageList.Draw(dc, PIC_VIEWFINDER_SDL, pt, ILD_NORMAL);
	}
...
BOOL CMousePositionDialog::OnInitDialog()
{
	...
	RGBQUAD Green = {0x00, 0xFF, 0x00, 0x00};
	RGBQUAD Blue  = {0xFF, 0x00, 0x00, 0x00};
	CBitmap bm0, bm1, bm2, bm3, bm4, bm5;
	CDC *dc;
	RECT Rect;
	int w, h;	
	...
	m_imageList.Create(31, 31, ILC_MASK|ILC_COLOR24, 0, 8);
	if(!(bm0.LoadBitmap(IDB_FINDER_RED))) MessageBox("LoadBitmap IDB_FINDER_RED failed");
	m_imageList.Add(&bm0, RGB(255,255,255));
	if(!(bm1.LoadBitmap(IDB_FINDER_GREEN))) MessageBox("LoadBitmap IDB_FINDER_GREEN failed");
	m_imageList.Add(&bm1, RGB(255,255,255));
	if(!(bm2.LoadBitmap(IDB_FINDER_BLUE))) MessageBox("LoadBitmap IDB_FINDER_BLUE failed");
	m_imageList.Add(&bm2, RGB(255,255,255));
	if(!(bm3.LoadBitmap(IDB_FINDER_YELLOW))) MessageBox("LoadBitmap IDB_FINDER_YELLOW failed");
	m_imageList.Add(&bm3, RGB(255,255,255));
	if(!(bm4.LoadBitmap(IDB_FINDER_DINPUT))) MessageBox("LoadBitmap IDB_FINDER_DINPUT failed");
	m_imageList.Add(&bm4, RGB(255,255,255));
	if(!(bm5.LoadBitmap(IDB_FINDER_SDL))) MessageBox("LoadBitmap IDB_FINDER_SDL failed");
