// dxwndhost.h : DXWNDHOST Application's main header file
//

#if !defined(AFX_DXWNDHOST_H__C68B92A2_C45F_4B59_986D_5BBC6BAD3AD8__INCLUDED_)
#define AFX_DXWNDHOST_H__C68B92A2_C45F_4B59_986D_5BBC6BAD3AD8__INCLUDED_

#if _MSC_VER > 1000
#pragma once
#endif // _MSC_VER > 1000

#ifndef __AFXWIN_H__
	#error include 'stdafx.h' before including this file for PCH
#endif

#include "resource.h"       // Main symbols
#include "PictureEx.h"	// Added by ClassView

extern int MessageBoxLang(UINT, UINT, UINT);
extern int MessageBoxLangArg(UINT, UINT, UINT, ...);
extern void HandleExpertFields(CDialog *tab, int *tags);
extern BOOL CommitAndInject(DWORD, const char *, LPVOID, DWORD);
extern BOOL Inject(DWORD, const char *);
extern BOOL IsWinXP();

// DxWnd host app private data
// fields here are associated to TARGETMAP records, but need not to be passed
// to the dxwnd hook callback, so they are left in a separate array to save 
// the (limited) IPC space and allow for more record entryes (currently 255).

#define MAX_NOTES 1024
#define MAX_REGISTRY 2048
#define MAX_TITLE 80

typedef struct PRIVATEMAP
{
	char title[MAX_TITLE+1];
	char launchpath[MAX_PATH+1];
	char cmdline[MAX_PATH+1];
	char startfolder[MAX_PATH+1];
	char FakeHDPath[MAX_PATH+1];
	char FakeCDPath[MAX_PATH+1];
	char FakeCDLabel[256+1];
	char msshims[MAX_PATH+1];
	char *notes;
	char *registry;
	CString IconString; 
	HICON Icon; 
	BOOL updated;
	DWORD CommitAddress;
	DWORD CommitLength;
}PRIVATEMAP;

typedef struct {
	TARGETMAP TM;
	PRIVATEMAP PM;
	char ExePath[MAX_PATH];
} ThreadInfo_Type;
ThreadInfo_Type;

extern char gInitPath[MAX_PATH+1];
extern char gInitialWorkingDir[MAX_PATH+1];
extern void OutTrace(const char *, ...);
extern BOOL gbDebug;
extern BOOL gbExpertMode;
extern BOOL gbWineConfig;
extern BOOL gIsToolbarWndVisible;

extern void InjectSuspended(char *, char *, BOOL, BOOL, TARGETMAP *, PRIVATEMAP *);
extern void InjectDetours(char *, char *, BOOL, BOOL, TARGETMAP *, PRIVATEMAP *);
#ifdef ADDINJECTACP
extern void InjectAPC(char *, char *, BOOL, BOOL, TARGETMAP *, PRIVATEMAP *);
extern void InjectAPC2(char *, char *, BOOL, BOOL, TARGETMAP *, PRIVATEMAP *);
#endif // ADDINJECTACP
extern DWORD WINAPI InjectDebug(void *);
extern DWORD WINAPI InjectProxy(void *);
extern DWORD WINAPI InjectHook(void *);

//#define WARNTIMERS MessageBeep(0xFFFFFFFF)
#define WARNTIMERS

/////////////////////////////////////////////////////////////////////////////
// CDxwndhostApp:
// For the definition of this class action please see the file dxwndhost.cpp.
//

class CDxwndhostApp : public CWinApp
{
public:
	CDxwndhostApp();

// Override
	// ClassWizard Generates a virtual function overrides.
	//{{AFX_VIRTUAL(CDxwndhostApp)
	public:
	virtual BOOL InitInstance();
	//}}AFX_VIRTUAL

// Implementation
	//{{AFX_MSG(CDxwndhostApp)
	afx_msg void OnAppAbout();
	afx_msg void OnViewHelp();
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
};

/////////////////////////////////////////////////////////////////////////////

//{{AFX_INSERT_LOCATION}}
// Microsoft Visual C++ Will insert additional declarations immediately before the previous line.

#endif // !defined(AFX_DXWNDHOST_H__C68B92A2_C45F_4B59_986D_5BBC6BAD3AD8__INCLUDED_)
