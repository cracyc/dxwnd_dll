...
char * MatchExe(char *FileName)
{
	...
	//hSDB = pSdbOpenDatabase("C:\\Windows\\AppPatch\\Custom\\{4c6e51bd-c2a8-432e-8d1a-5be18fd8da7c}.sdb", NT_PATH);
	//if(!hSDB){
	//	sprintf(sBuf, "Can't open shims database: err=%d\n", GetLastError());
	//	return (sBuf);
	//}
	...
	if(!hSDB){
		sprintf(sBuf, "Can't initialize shims database: err=%d\n", GetLastError());
		return (sBuf);
	}

	bRet = pSdbGetMatchingExe(NULL, (LPCWSTR)szFileName, NULL, NULL, 0, &result);
	//bRet = pSdbGetMatchingExe(hSDB, (LPCWSTR)szFileName, NULL, NULL, 0, &result);
	if(!bRet){
