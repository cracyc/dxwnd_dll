...
void CStatusDialog::OnTimer(UINT_PTR nIDEvent)
{
	...
	if(DxStatus==DXW_RUNNING){
		...
		strcat(sMsg, sMsg2);
		if(Target->tflags & ADDTIMESTAMP) {
			sprintf_s(sMsg2, 1024, 
				"\nUptime[sec] = %d", DxWndStatus.upTime);
			strcat(sMsg, sMsg2);		}
		...
		if(DxWndStatus.DInputX != (short)-1){
			sprintf_s(sMsg2, 1024, 
				"\nDInput = (%d,%d)", DxWndStatus.DInputX, DxWndStatus.DInputY);
			strcat(sMsg, sMsg2);
		}
		...
		if(Target->flags10 & FAKEHDDRIVE){
			sprintf(sMsg2, "\nfake HD=%c:", DxWndStatus.HDLabel);
			strcat(sMsg, sMsg2);
		}
		if(Target->flags10 & FAKECDDRIVE){
			sprintf(sMsg2, "\nfake CD=%c: vol.%d", DxWndStatus.CDLabel, DxWndStatus.CDIndex + 1);
			strcat(sMsg, sMsg2);
		}
	}
