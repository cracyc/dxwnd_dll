...
DWORD WINAPI InjectProxy(void *p)
{
	...
	dwProxyMask = 0;
	for(int i=0; ProxyList[i]; i++) {
		char sCopyFrom[MAX_PATH+1], sCopyTo[MAX_PATH+1];
		sprintf_s(sCopyFrom, MAX_PATH, "%s\\proxy\\%s", GetDxWndPath(), ProxyList[i]);
		sprintf_s(sCopyTo, MAX_PATH, "%s\\%s", path, ProxyList[i]);
	...
	if(ThInfo->TM.flags7 & CLEARSHIMS) flags |= CREATE_SUSPENDED;
	...
	if (!CreateProcessA(ThInfo->ExePath, lpCommandLine, 0, 0, false, 0, NULL, path, &sinfo, &pinfo)){
		...
		if(ThInfo->TM.flags7 & CLEARSHIMS) {
			...
			MessageBoxEx(0, DebugMessage, "Debug", MB_ICONEXCLAMATION|MB_OK, NULL);
			OutTrace("%s\n", DebugMessage);
			return 0;
			}
		}
