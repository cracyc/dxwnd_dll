...
CWnd *pParent;
char AvailableCDDrives['Z'-'A'+2];

static void InitializeCDDrives()
{
	// build the list of available CD drives
	AvailableCDDrives[0]='?'; // default
	int ordinal = 1;
	for (int i='D'; i<='Z'; i++){
		char sDrivePath[3];
		sprintf(sDrivePath, "%c:", i);
		if(::GetDriveTypeA(sDrivePath) == DRIVE_CDROM){
			AvailableCDDrives[ordinal++] = (char)i;
		}
	}
	AvailableCDDrives[ordinal]=0; // terminator
}
...
{
	// v2.06.06: bug fix - the sequence \n must not be unescaped if found in a registry key within square brakets.
	if(!*dest)	*dest=(char *)malloc(strlen(s)+100);
	else		*dest=(char *)realloc(*dest, strlen(s)+100); 
	char *t = *dest;
	bool isKey=FALSE;
	bool isFirstChar=TRUE;
	if((*s == '[') && isFirstChar) isKey = TRUE;
	for(; *s; s++){
		isFirstChar = FALSE;
		if((*s=='\\') && (*(s+1)=='n') && !isKey){
			*t++ = '\r';
...
static void RecoverKayboardLayout()
{
	CHAR CurKeyboardLayout[KL_NAMELENGTH ];
	extern CHAR gKeyboardLayoutName[KL_NAMELENGTH ];
	extern HKL ghkl;
	GetKeyboardLayoutNameA(CurKeyboardLayout);
	if(strcmp(CurKeyboardLayout, gKeyboardLayoutName)){
		HKL hkl = LoadKeyboardLayout(gKeyboardLayoutName, 0);	
		if (gQuietMode){
			ActivateKeyboardLayout(hkl, 0);	
		}
		else {
			HKL res;
			OutTrace("current keyboard layout=%s hkl=%x\n", CurKeyboardLayout, hkl);
			if (MessageBox(NULL, "Keyboard layout was changed. Recover?", "warning", MB_YESNO | MB_ICONQUESTION)==IDYES) {
				res = ActivateKeyboardLayout(hkl, KLF_REORDER);
				if(res == NULL) OutTrace("ActivateKeyboardLayout(%s) failed err=%d\n", gKeyboardLayoutName, GetLastError());
				if(!SystemParametersInfoA(SPI_SETDEFAULTINPUTLANG, 0, (PVOID)&ghkl, NULL)){
					OutTrace("SystemParametersInfoA(SPI_SETDEFAULTINPUTLANG) failed err=%d\n", GetLastError());
				};
			}
		}
	}
...
static BOOL FileExists(char *sTargetPath)
{
    ...
    if (hFile == INVALID_HANDLE_VALUE) { 
		OutTrace("FileExists: path=%s err=%d\n", sTargetPath, GetLastError());
		ret = (GetLastError() == ERROR_SHARING_VIOLATION) ? TRUE : FALSE;
    }
	else {
		CloseHandle(hFile);
		ret = TRUE;
	}
	//OutTrace("file %s exists=%s\n", sTargetPath, ret ? "YES" : "NO");
	return ret;
}
...
BEGIN_MESSAGE_MAP(CDxwndhostView, CListView)
	...
	ON_COMMAND(ID_WINDOW_HIGHLIGHT, OnWindowHighlight)
	ON_COMMAND(ID_WINDOW_MINIMIZE, OnWindowMinimize)
	...
	ON_COMMAND(ID_EDITLIST_COPYPATH, OnCopyPath)
	ON_COMMAND(ID_EDITLIST_PASTEPATH, OnPastePath)
	...
	ON_COMMAND(ID_VIEW_3DEFFECTS, OnView3DEffects)
	...
	ON_COMMAND(ID_TOOLS_IMPORTFROMLOG, OnImportFromLog)
	...
	ON_COMMAND(ID_RECOVER_SYSTEMCURSORS, OnRecoverSystemCursors)
	...
	ON_COMMAND(ID_TOOLS_IDENTIFYMONITORS, OnIdentifyMonitors)
	ON_WM_KEYUP()
	ON_WM_KEYDOWN()
	ON_WM_RBUTTONDOWN()
	ON_WM_HOTKEY()
...
static void RestoreMouseSpeed(int iInitialMouseSpeed)
{
	int iMouseSpeed = 0;
	if(!SystemParametersInfo(SPI_GETMOUSESPEED, NULL, (PVOID)&iMouseSpeed, NULL)){
		OutTrace("GPI_SETMOUSESPEED error %d\n", GetLastError());
		return;
	};
	if(iMouseSpeed != iInitialMouseSpeed){
		OutTrace("recovering mouse speed %d->%d\n", iMouseSpeed, iInitialMouseSpeed);
		if(!SystemParametersInfo(SPI_SETMOUSESPEED, NULL, (PVOID)iInitialMouseSpeed, NULL)){
			OutTrace("Set SPI_SETMOUSESPEED error %d\n", GetLastError());
		}
	}
}
...
void SetPrivateMapFromDialog(PRIVATEMAP *p, CTargetDlg *dlg)
{
	...
	strnncpy(p->msshims, (char *)dlg->m_MSShims.GetString(), MAX_PATH);
	if(p->notes)
...
{
	t->dxversion = (dlg->m_DXVersion > 1) ? dlg->m_DXVersion + 5 : dlg->m_DXVersion;
	...
	if(dlg->m_EnablePatcher) t->flags18 |= ENABLEPATCHER;
	if(dlg->m_SharedHook) t->flags13 |= SHAREDHOOK;
	if(dlg->m_HookVideoAdapter) t->flags16 |= HOOKVIDEOADAPTER;
	if(dlg->m_RunByDxWnd) t->flags16 |= RUNBYDXWND;
	if(dlg->m_CopyNoShims) t->flags7 |= CLEARSHIMS;
	if(dlg->m_DisableGhosting) t->flags20 |= DISABLEGHOSTING;
	...
	if(dlg->m_ForceWindowing) t->flags3 |= FORCEWINDOWING;
	...
	if(dlg->m_CenterOnExit) t->flags20 |= CENTERONEXIT;
	if(dlg->m_AdaptMouseSpeed) t->flags12 |= ADAPTMOUSESPEED;
	if(dlg->m_LockCursorIcons) t->flags14 |= LOCKCURSORICONS;
	if(dlg->m_LockSysCursorIcons) t->flags16 |= LOCKSYSCURSORICONS;
	if(dlg->m_RecoverSysCursorIcons) t->flags16 |= RECOVERSYSCURSORICONS;
	...
	if(dlg->m_GetAllMessages) t->flags20 |= GETALLMESSAGES;

	if(!dlg->m_EarlyHook) dlg->m_InjectionMode = 0;
	switch(dlg->m_InjectionMode){
		case 0: break;
		case 1: t->flags2 |= STARTDEBUG; break;
		case 2: t->flags7 |= INJECTSUSPENDED; break;
		case 3: t->flags12|= INJECTPROXY; break;
		case 4: t->flags15|= INJECTAPC; break;
		case 5: t->flags19|= STARTDETOURS; break;
		//case 5: t->flags15|= INJECTAPC2; break;
	}
	// if the injection mode is not compatible with early hook, pick
	// a default choice INJECTSUSPENDED
	if(dlg->m_EarlyHook && (dlg->m_InjectionMode == 0)){
	...
	switch(dlg->m_DCEmulationMode){
		...
		case 4: t->flags15 |= GDIASYNCDC; break;
	}

	switch(dlg->m_ResTypes){
		case 0: t->flags4 |= SUPPORTSVGA; break;
		case 1: t->flags4 |= SUPPORTHDTV; break;
		case 2: t->flags4 |= (SUPPORTSVGA|SUPPORTHDTV); break;
		case 3: t->flags4 |= NATIVERES; break;
	}	
	...
	switch(dlg->m_TextureHandling){
		case 0: break;
		case 1: t->flags5 |= TEXTUREDUMP; break;
		case 2: t->flags5 |= TEXTURETRANSP; break;
		case 3: t->flags5 |= TEXTUREHIGHLIGHT; break;
		case 4: t->flags5 |= TEXTUREHACK; break;
	}

	switch(dlg->m_SonProcessMode){
		case 0: break;
		case 1: t->flags4 |= SUPPRESSCHILD; break;
		case 2: t->flags5 |= DEBUGSON; break;
		case 3: t->flags5 |= INJECTSON; break;
		case 4: t->flags19 |= DETOURSSON; break;
	}	

	switch(dlg->m_HookMode){
		case 0: break; // default IAT scan
		case 1: t->flags4 |= HOTPATCH; break; // hybrid
		case 2: t->flags15 |= HOTPATCHALWAYS; break;
	}	

	switch(dlg->m_IATAlignedMode){
		case 1: t->flags9 |= IATBYTEALIGNED; break;
		case 0: t->flags9 |= IATWORDALIGNED; break;
		case 2: t->flags9 |= NOIATSCAN; break;
		case 3: t->flags10 |= PEFILEHOOK; break;
	}
	...
	switch(dlg->m_LogMode){
		...
		case 4: t->tflags |= (OUTTRACE|OUTCIRCULAR); break;
	}	

	switch(dlg->m_LogSeverity){
		case 0: break;
		case 1: t->tflags |= LOGERRORS; break;
		case 2: t->tflags |= LOGTRACE; break;
		case 3: t->tflags |= LOGDEBUG; break;
	}	
	...
	switch(dlg->m_ClipperMode){
		...
		case 3: t->flags5 |= MAXCLIPPER; break;
	}

	switch(dlg->m_FourCCMode){
		case 0: break;
		case 1: t->flags10 |= HWFOURCC; break;
		case 2: t->flags10 |= SWFOURCC; break;
	}
	...
	switch(dlg->m_ShowFPS){
		case 0: break;
		case 1: t->flags2 |= SHOWFPSOVERLAY; break;
		case 2: t->flags2 |= SHOWFPS; break;
	}

	switch(dlg->m_ZOrder){
		case 0: break;
		case 1: t->flags5 |= UNLOCKZORDER; break;
		case 2: t->flags9 |= LOCKTOPZORDER; break;
	}

	switch(dlg->m_VertexProcessing){
		case 0: break;
		case 1: t->flags10 |= FORCEHWVERTEXPROC; break;
		case 2: t->flags10 |= FORCESWVERTEXPROC; break;
		case 3: t->flags10 |= FORCEMXVERTEXPROC; break;
	}

	switch(dlg->m_ForceDevType){
		case 0: break;
		case 1: t->flags17 |= FORCEDEVTYPE1; break;
		case 2: t->flags17 |= FORCEDEVTYPE2; break;
		case 3: t->flags17 |= (FORCEDEVTYPE1|FORCEDEVTYPE2); break;
		case 4: t->flags17 |= FORCEDEVTYPE3; break;
		default: break;
	}

	switch(dlg->m_GFocusMode){
		case 0: break;
		case 1: t->flags5 |= GLOBALFOCUSON; break;
		case 2: t->flags5 |= GLOBALFOCUSOFF; break;
	}

	switch(dlg->m_AltTabMode){
		case 0: break;
		case 1: t->flags7 |= NODISABLEALTTAB; break;
		case 2: t->flags5 |= DISABLEALTTAB; break;
	}

	switch(dlg->m_CDEmulationMode){
		case 0: break;
		case 1: t->flags15 |= PLAYFROMCD; t->flags8 |= VIRTUALCDAUDIO; break;
		case 2: t->flags15 |= CDAUTOMATICRIP; t->flags8 |= VIRTUALCDAUDIO; break;
		case 3: t->flags8 |= VIRTUALCDAUDIO; break;
	}

	switch(dlg->m_CDEmulationVersion){
		case 0: t->flags15 |= EMULATEWIN9XMCI; break;
		case 1: break;
		case 2: t->flags15 |= EMULATEVISTAMCI; break;
	}

	switch(dlg->m_ZBufferMode){
		case 0: break;
		case 1: t->flags17 |= FORCEZBUFFERON; break;
		case 2: t->flags17 |= FORCEZBUFFEROFF; break;
	}

	switch(dlg->m_DitherMode){
		case 0: break;
		case 1: t->flags17 |= FORCEDITHERING; break;
		case 2: t->flags17 |= CLEARDITHERING; break;
	}

	switch(dlg->m_OverlayMode){
		case 0: break;
		case 1: t->flags16 |= EMULATEOVERLAY; break;
		case 2: t->flags16 |= (EMULATEOVERLAY | TRANSPARENTOVERLAY); break;
	}

	switch(dlg->m_FlipMode){
		case 0: break;
		case 1: t->flags6 |= HALFFLIPEMULATION; break;
		case 2: t->flags4 |= FULLFLIPEMULATION; t->flags6 |= HALFFLIPEMULATION; break;
	}

	switch(dlg->m_CoopMode){
		case 0: break;
		case 1: t->flags18 |= LOCKNORMALCOOP; break;
		case 2: t->flags18 |= LOCKEXCLUSIVECOOP; break;
	}
	...
	if(dlg->m_OutOnTempFolder) t->tflags |= OUTONTEMPFOLDER;
	if(dlg->m_OutAllErrors) t->tflags |= OUTALLERRORS;
	if(dlg->m_AssertDialog) t->tflags |= ASSERTDIALOG; // hidden ???

	if(dlg->m_OutWinMessages) t->tflags2 |= OUTWINMESSAGES;
	if(dlg->m_OutDWTrace) t->tflags2 |= OUTDXWINTRACE;
	if(dlg->m_OutOGLTrace) t->tflags2 |= OUTOGLTRACE;
	if(dlg->m_OutSDLTrace) t->tflags2 |= OUTSDLTRACE;
	if(dlg->m_OutTimeTrace) t->tflags2 |= OUTTIMETRACE;
	if(dlg->m_OutSoundTrace) t->tflags2 |= OUTSOUNDTRACE;
	if(dlg->m_OutInputs) t->tflags2 |= OUTINPUTS;
	if(dlg->m_OutLocale) t->tflags2 |= OUTLOCALE;
	if(dlg->m_OutFPS) t->tflags2 |= OUTFPS;
	if(dlg->m_OutSysLibs) t->tflags2 |= OUTSYSLIBS;
	if(dlg->m_OutWGTrace) t->tflags2 |= OUTWINGTRACE;
	if(dlg->m_OutCOMTrace) t->tflags2 |= OUTCOMTRACE;
	if(dlg->m_OutDDRAWTrace) t->tflags2 |= OUTDDRAWTRACE;
	if(dlg->m_OutD3DTrace) t->tflags2 |= OUTD3DTRACE;
	if(dlg->m_CursorTrace) t->tflags2 |= OUTCURSORTRACE;
	if(dlg->m_ImportTable) t->tflags2 |= OUTIMPORTTABLE;
	if(dlg->m_OutVGA) t->tflags2 |= OUTVGA;
	if(dlg->m_OutStdIO) t->tflags2 |= OUTSTDIO;
	if(dlg->m_RegistryOp) t->tflags2 |= OUTREGISTRY;
	if(dlg->m_OutFileIO) t->tflags2 |= OUTFILEIO;
	if(dlg->m_OutGDI) t->tflags2 |= OUTGDI;
	if(dlg->m_OutDShow) t->tflags2 |= OUTDSHOW;
	if(dlg->m_OutDrv) t->tflags2 |= OUTDRV;
	if(dlg->m_TraceHooks) t->tflags2 |= TRACEHOOKS;
	...
	dlg->m_LogSeverity = 0;  // essential
	if(t->tflags & LOGERRORS)		dlg->m_LogSeverity = 1; // must be first one ....
	if(t->tflags & LOGTRACE)		dlg->m_LogSeverity = 2; 
	if(t->tflags & LOGDEBUG)		dlg->m_LogSeverity = 3;
	...
	if(dlg->m_SharedMouse) t->flags17 |= SHAREDMOUSE;
	if(dlg->m_PackMouseData) t->flags18 |= PACKMOUSEDATA;
	...
	if(dlg->m_JoystickEffects) t->flags16 |= JOYSTICKEFFECTS;
	...
	if(dlg->m_ForceNOPs) t->flags16 |= FORCENOPS;
	if(dlg->m_NoBAADFOOD) t->flags9 |= NOBAADFOOD;
	if(dlg->m_LimitResources) t->flags2 |= LIMITRESOURCES;
	if(dlg->m_FixFileDialog) t->flags15 |= FIXFILEDIALOG;
	if(dlg->m_MIDISetInstrument) t->flags15 |= MIDISETINSTRUMENT;
	if(dlg->m_IgnoreScheduler) t->flags15 |= IGNORESCHEDULER;
	if(dlg->m_SlowDownExceptions) t->flags15 |= SLOWDOWNEXCEPTIONS;
	if(dlg->m_DuplicateHandleFix) t->flags15 |= DUPLICATEHANDLEFIX;
	if(dlg->m_EmulateWin95) t->flags15 |= EMULATEWIN95;
	if(dlg->m_ForceHalftoneTiny) t->flags15 |= FORCEHALFTONETINY;
	if(dlg->m_FixFullWinStyle) t->flags15 |= FIXFULLWINSTYLE;
	if(dlg->m_HideDisplayModes) t->flags15 |= HIDEDISPLAYMODES;
	//if(dlg->m_GDIAsyncDC) t->flags15 |= GDIASYNCDC;
	if(dlg->m_Ramp2RGBDevice) t->flags16 |= RAMP2RGBDEVICE;
	if(dlg->m_Ramp2MMXDevice) t->flags16 |= RAMP2MMXDEVICE;
	if(dlg->m_CullModeNone) t->flags15 |= CULLMODENONE;
	if(dlg->m_NoSurfaceNew) t->flags16 |= NOSURFACENEW;
	if(dlg->m_EmulateWin9XHeap) t->flags16 |= EMULATEWIN9XHEAP;
	if(dlg->m_ForceD3DRefDevice) t->flags16 |= FORCED3DREFDEVICE;
	if(dlg->m_FixCreateFileMap) t->flags16 |= FIXCREATEFILEMAP;
	if(dlg->m_EmuGetCommandLine) t->flags16 |= EMUGETCOMMANDLINE;
	if(dlg->m_MCINotifyTopmost) t->flags16 |= MCINOTIFYTOPMOST;
	if(dlg->m_ForceSimpleWindow) t->flags16 |= FORCESIMPLEWINDOW;
	if(dlg->m_ConfirmEscape) t->flags16 |= CONFIRMESCAPE;
	if(dlg->m_CDROMDriveType) t->flags3 |= CDROMDRIVETYPE;
	if(dlg->m_HideCDROMEmpty) t->flags4 |= HIDECDROMEMPTY;
	if(dlg->m_HideCDROMReal) t->flags17 |= HIDECDROMREAL;
	...
	if(dlg->m_DisableWinHook) t->dflags2 |= DISABLEWINHOOK;
	...
	if(dlg->m_Experimental2) t->dflags2 |= EXPERIMENTAL2;
	if(dlg->m_Experimental3) t->dflags2 |= EXPERIMENTAL3;
	if(dlg->m_Experimental4) t->dflags2 |= EXPERIMENTAL4;
	if(dlg->m_Experimental5) t->dflags2 |= EXPERIMENTAL5;
	if(dlg->m_Experimental6) t->dflags2 |= EXPERIMENTAL6;
	if(dlg->m_Experimental7) t->dflags2 |= EXPERIMENTAL7;
	if(dlg->m_Experimental8) t->dflags2 |= EXPERIMENTAL8;
	if(dlg->m_FixRandomPalette) t->dflags2 |= FIXRANDOMPALETTE;
	if(dlg->m_NoPower2Fix) t->flags4 |= NOPOWER2FIX;
	if(dlg->m_NoMultiSample) t->flags17 |= NOMULTISAMPLE;
	if(dlg->m_ForceVBufOnSysMem) t->flags17 |= FORCEVBUFONSYSMEM;
	if(dlg->m_ZBufOnSysMemory) t->flags17 |= ZBUFONSYSMEMORY;
	if(dlg->m_ZBufOnVidMemory) t->flags19 |= ZBUFONVIDMEMORY;
	if(dlg->m_TrimVertexBuffer) t->flags18 |= TRIMVERTEXBUFFER;
	if(dlg->m_DepthBufZClean) t->flags18 |= DEPTHBUFZCLEAN;
	if(dlg->m_EnableZooming) t->flags18 |= ENABLEZOOMING;
	if(dlg->m_FilterRGBTextures) t->flags18 |= FILTERRGBTEXTURES;
	if(dlg->m_NoD3DOptimize) t->flags18 |= NOD3DOPTIMIZE;
	if(dlg->m_TrimD3DPoints) t->flags19 |= TRIMD3DPOINTS;
	if(dlg->m_AltPixelCenter) t->flags18 |= ALTPIXELCENTER;
	if(dlg->m_FixColorKey) t->flags18 |= FIXCOLORKEY;
	if(dlg->m_StretchFullRect) t->flags14 |= STRETCHFULLRECT;
	if(dlg->m_ColorKeyToAlpha) t->flags16 |= COLORKEYTOALPHA;
	if(dlg->m_ForceWBasedFog) t->flags16 |= FORCEWBASEDFOG;
	if(dlg->m_ForceWBasedFogWDDM) t->flags19 |= FORCEWBASEDFOGWDDM;
	if(dlg->m_ForceD3D9on12) t->flags16 |= FORCED3D9ON12;
	if(dlg->m_FixTnLRhw) t->flags16 |= FIXTNLRHW;
	if(dlg->m_MapMemB0000) t->flags16 |= MAPMEMB0000;
	if(dlg->m_TrimChildWindows) t->flags16 |= TRIMCHILDWINDOWS;
	if(dlg->m_TrimMainWindow) t->flags16 |= TRIMMAINWINDOW;
	if(dlg->m_FixAILBug) t->flags17 |= FIXAILBUG;
	if(dlg->m_LockCursorShape) t->flags17 |= LOCKCURSORSHAPE;
	if(dlg->m_FixOverlappedResult) t->flags17 |= FIXOVERLAPPEDRESULT;
	if(dlg->m_ForceFilterNearest) t->flags17 |= FORCEFILTERNEAREST;
	if(dlg->m_HideDirectDraw) t->flags17 |= HIDEDIRECTDRAW;
	if(dlg->m_CompensateOpenGLCopy) t->flags17 |= COMPENSATEOGLCOPY;
	if(dlg->m_EmuDDSyncShim) t->flags18 |= EMUDDSYNCSHIM;
	if(dlg->m_StretchBackBuffer) t->flags18 |= STRETCHBACKBUFFER;
	if(dlg->m_LegacyKernel32) t->flags19 |= LEGACYKERNEL32;
	if(dlg->m_DDrawLegacyCallback) t->flags19 |= DDRAWLEGACYCALLBACK;
	if(dlg->m_AsyncWaveOutOpen) t->flags19 |= ASYNCWAVEOUTOPEN;
	if(dlg->m_AsyncWaveOutClose) t->flags19 |= ASYNCWAVEOUTCLOSE;
	if(dlg->m_TransparentDialog) t->flags19 |= TRANSPARENTDIALOG;
	if(dlg->m_EmulateWinResize) t->flags19 |= EMULATEWINRESIZE;
	if(dlg->m_DisableLayeredWin) t->flags19 |= DISABLELAYEREDWIN;
	if(dlg->m_ScaleChildWin) t->flags19 |= SCALECHILDWIN;
	if(dlg->m_TrimTextYPos) t->flags19 |= TRIMTEXTYPOS;
	if(dlg->m_NativeDialogs) t->flags16 |= NATIVEDIALOGS;
	if(dlg->m_NoPerfCounter) t->flags4 |= NOPERFCOUNTER;
	if(dlg->m_DisableFogging) t->flags4 |= DISABLEFOGGING;
	if(dlg->m_ControlFogging) t->flags13 |= CONTROLFOGGING;
	if(dlg->m_SetFogColor) t->flags16 |= SETFOGCOLOR;
	...
	if(dlg->m_EmulateCDMixer) t->flags13 |= EMULATECDMIXER;
	if(dlg->m_EmulateCDAux) t->flags13 |= EMULATECDAUX;
	if(dlg->m_HideMuteControls) t->flags13 |= HIDEMUTECONTROLS;
	if(dlg->m_PlaySoundFix) t->flags13 |= PLAYSOUNDFIX;
	if(dlg->m_StopSound) t->flags14 |= STOPSOUND;
	if(dlg->m_FixDefaultMCIId) t->flags11 |= FIXDEFAULTMCIID;
	if(dlg->m_LockCDTray) t->flags12 |= LOCKCDTRAY;
	if(dlg->m_BypassWaveOutPos) t->flags13 |= BYPASSWAVEOUTPOS;
	...
	if(dlg->m_EmulateSoundPan) t->flags18 |= EMULATESOUNDPAN;
	if(dlg->m_NoSoundLoop) t->flags14 |= NOSOUNDLOOP;
	if(dlg->m_FixDSoundBuffer) t->flags19 |= FIXDSOUNDBUFFER;
	if(dlg->m_BypassDSound) t->flags18 |= BYPASSDSOUND;
	if(dlg->m_NoWaveOut) t->flags19 |= NOWAVEOUT;
	if(dlg->m_MCISingleThreaded) t->flags20 |= MCISINGLETHREADED;
	if(dlg->m_DSInitVolume) t->flags12 |= DSINITVOLUME;
	if(dlg->m_LockVolume) t->flags9 |= LOCKVOLUME;
	if(dlg->m_EmulateVolume) t->flags15 |= EMULATEVOLUME;
	if(dlg->m_SafeMidiOut) t->flags11 |= SAFEMIDIOUT;
	if(dlg->m_MidiAutoRepair) t->flags13 |= MIDIAUTOREPAIR;
	if(dlg->m_SetCDVolume) t->flags10 |= SETCDVOLUME;
	//if(dlg->m_ForceMCISysInfo) t->flags18 |= FORCEMCISYSINFO;
	...
	if(dlg->m_SDLFixMouse) t->flags18 |= SDLFIXMOUSE;
	...
	if(dlg->m_HookDirectShow) t->flags17 |= HOOKDIRECTSHOW;
	if(dlg->m_ScaleDirectShow) t->flags18 |= SCALEDIRECTSHOW;
	if(dlg->m_IgnoreAddFilter) t->flags18 |= IGNOREADDFILTER;
	if(dlg->m_SuppressIVMR) t->flags18 |= SUPPRESSIVMR;
	if(dlg->m_IgnoreGraphErrors) t->flags18 |= IGNOREGRAPHERRORS;
	...
	if(dlg->m_NoRampDevice) t->flags14 |= NORAMPDEVICE;
	if(dlg->m_FogVertexCap) t->flags16 |= FOGVERTEXCAP;
	if(dlg->m_FogTableCap) t->flags16 |= FOGTABLECAP;
	if(dlg->m_NoRGBDevice) t->flags14 |= NORGBDEVICE;
	if(dlg->m_NoMMXDevice) t->flags14 |= NOMMXDEVICE;
	...
	if(dlg->m_LockPixelFormat) t->flags17 |= LOCKPIXELFORMAT;
	if(dlg->m_FixAspectRatio) t->flags13 |= FIXASPECTRATIO;
	...
	//if(dlg->m_EmulateOverlay) t->flags16 |= EMULATEOVERLAY;
	//if(dlg->m_TransparentOverlay) t->flags16 |= TRANSPARENTOVERLAY;
	...
	if(dlg->m_PathLocale) t->flags12 |= PATHLOCALE;
	if(dlg->m_NoSetText) t->flags14 |= NOSETTEXT;
	if(dlg->m_FixKeyboardType) t->flags14 |= FIXKEYBOARDTYPE;
	...
	if(dlg->m_ForceColorKeyOff) t->flags13 |= FORCECOLORKEYOFF;
	if(dlg->m_SpongeBobHack) t->flags13 |= SPONGEBOBHACK;
	if(dlg->m_DSoundReplace) t->flags13 |= DSOUNDREPLACE;
	if(dlg->m_FightingForceFix) t->flags13 |= FIGHTINGFORCEFIX;
	if(dlg->m_CenterDialogs) t->flags13 |= CENTERDIALOGS;
	if(dlg->m_DxVersionLie) t->flags13 |= DXVERSIONLIE;
	if(dlg->m_LegacyBaseUnits) t->flags13 |= LEGACYBASEUNITS;
	if(dlg->m_PatchMSVBVM) t->flags13 |= PATCHMSVBVM;
	if(dlg->m_XRayTweak) t->flags13 |= XRAYTWEAK;
	if(dlg->m_FixDCColorDepth) t->flags13 |= FIXDCCOLORDEPTH;
	if(dlg->m_DefuseFlashBomb) t->flags13 |= DEFUSEFLASHBOMB;
	if(dlg->m_LockAllWindows) t->flags13 |= LOCKALLWINDOWS;
	if(dlg->m_IgnoreFSysErrors) t->flags13 |= IGNOREFSYSERRORS;
	if(dlg->m_HeapPadAllocation) t->flags14 |= HEAPPADALLOCATION;
	if(dlg->m_HeapZeroMemory) t->flags19 |= HEAPZEROMEMORY;
	if(dlg->m_DisableColorKey) t->flags14 |= DISABLECOLORKEY;
	if(dlg->m_NoShellExecute) t->flags14 |= NOSHELLEXECUTE;
	if(dlg->m_CustomPerfCount) t->flags14 |= CUSTOMPERFCOUNT;
	if(dlg->m_MidiOutBypass) t->flags14 |= MIDIOUTBYPASS;
	if(dlg->m_FixedBitBlt) t->flags14 |= FIXEDBITBLT;
	if(dlg->m_HeapLeak) t->flags14 |= HEAPLEAK;
	if(dlg->m_MW2WaveOutFix) t->flags14 |= MW2WAVEOUTFIX;
	if(dlg->m_SafeHeap) t->flags14 |= SAFEHEAP;
	if(dlg->m_RepairHeap) t->flags18 |= REPAIRHEAP;
	if(dlg->m_Win16CreateFile) t->flags14 |= WIN16CREATEFILE;
	if(dlg->m_Win16FindFileFix) t->flags14 |= WIN16FINDFILEFIX;
	if(dlg->m_SlowCDStatus) t->flags14 |= SLOWCDSTATUS;
	if(dlg->m_WaveOutUsePrefDev) t->flags14 |= WAVEOUTUSEPREFDEV;
	if(dlg->m_FixMacromediaReg) t->flags14 |= FIXMACROMEDIAREG;
	if(dlg->m_LockSystemMenu) t->flags14 |= LOCKSYSTEMMENU;
	if(dlg->m_StickyWindows) t->flags14 |= STICKYWINDOWS;
	if(dlg->m_DumpText) t->flags14 |= DUMPTEXT;
	//if(dlg->m_EmulateWin9XMCI) t->flags15 |= EMULATEWIN9XMCI;
	if(dlg->m_EmulateFloppyDrive) t->flags15 |= EMULATEFLOPPYDRIVE;
	if(dlg->m_FilterColorKey) t->flags18 |= FILTERCOLORKEY;
	if(dlg->m_ForceColorKey) t->flags18 |= FORCECOLORKEY;
	if(dlg->m_ColorKey2Palette) t->flags18 |= COLORKEY2PALETTE;
	if(dlg->m_FastPrimaryUpdate) t->flags15 |= FASTPRIMARYUPDATE;
	if(dlg->m_LimitFreeDisk) t->flags15 |= LIMITFREEDISK;
	if(dlg->m_LimitFreeRAM) t->flags15 |= LIMITFREERAM;
	if(dlg->m_LimitVideoRAM) t->flags15 |= LIMITVIDEORAM;
	if(dlg->m_LimitTextureRAM) t->flags15 |= LIMITTEXTURERAM;
	if(dlg->m_EmulateWinHelp) t->flags17 |= EMULATEWINHELP;
	if(dlg->m_SuppressDEP) t->flags17 |= SUPPRESSDEP;
	if(dlg->m_DeferredWinPos) t->flags15 |= DEFERREDWINPOS;
	if(dlg->m_DispelTweak) t->flags15 |= DISPELTWEAK;
	if(dlg->m_DisablePSGP) t->flags13 |= DISABLEPSGP;
	if(dlg->m_DisableD3DMMX) t->flags13 |= DISABLED3DMMX;
	if(dlg->m_DisableD3DxPSGP) t->flags13 |= DISABLED3DXPSGP;
	if(dlg->m_D3DXDoNotMute) t->flags13 |= D3DXDONOTMUTE;
	if(dlg->m_DisableDEP) t->flags13 |= DISABLEDEP;
	...
	if(dlg->m_ForceHalfTone) t->flags15 |= FORCEHALFTONE;
	...
	if(dlg->m_NoLAVFilters) t->flags18 |= NOLAVFILTERS;
	if(dlg->m_FixMoviesColor) t->flags6 |= FIXMOVIESCOLOR;
	if(dlg->m_StretchMovies) t->flags6 |= STRETCHMOVIES;
	if(dlg->m_BypassActiveMovie) t->flags18 |= BYPASSACTIVEMOVIE;
	...
	if(dlg->m_TrimMousePosition) t->flags |= TRIMMOUSEPOSITION;
	...
	if(dlg->m_MarkClipRgn) t->dflags |= MARKCLIPRGN;
	if(dlg->m_DumpDIBSection) t->dflags |= DUMPDIBSECTION;
	if(dlg->m_CaptureScreens) t->dflags |= CAPTURESCREENS;
	if(dlg->m_PatchExecuteBuffer) t->flags17 |= PATCHEXECUTEBUFFER;
	if(dlg->m_DumpDevContext) t->dflags |= DUMPDEVCONTEXT;
	if(dlg->m_DumpCPUID) t->dflags |= DUMPCPUID;
	if(dlg->m_ForceXMirroring) t->dflags2 |= FORCEXMIRRORING;
	if(dlg->m_ForceYMirroring) t->dflags2 |= FORCEYMIRRORING;
	...
	if(dlg->m_SetPriorityLow) t->flags15 |= SETPRIORITYLOW;
	if(dlg->m_KillVSync) t->flags12 |= KILLVSYNC;
	if(dlg->m_FakeDate) t->flags16 |= FAKEDATE;
	if(dlg->m_UpTimeClear) t->flags14 |= UPTIMECLEAR;
	if(dlg->m_UpTimeStress) t->flags14 |= UPTIMESTRESS;
	...
	if(dlg->m_FixMouseBiasX) t->flags17 |= FIXMOUSEBIASX;
	if(dlg->m_FixMouseBiasY) t->flags17 |= FIXMOUSEBIASY;
	...
	if(dlg->m_UseNanosleep) t->flags13 |= USENANOSLEEP;
	if(dlg->m_CustomTimeShift) t->flags13 |= CUSTOMTIMESHIFT;
	if(dlg->m_LimitFrequency) t->flags14 |= LIMITFREQUENCY;
	...
	if(dlg->m_GLExtensionsTrim) t->flags13 |= GLEXTENSIONSTRIM;
	if(dlg->m_GLFixClamp) t->flags13 |= GLFIXCLAMP;
	if(dlg->m_NoScissorTest) t->flags18 |= NOSCISSORTEST;
	if(dlg->m_Sample2Coverage) t->flags14 |= SAMPLE2COVERAGE;
	...
	if(dlg->m_AsyncBlitMode) t->flags15 |= ASYNCBLITMODE;
	if(dlg->m_FakeVersion) t->flags2 |= FAKEVERSION;
	if(dlg->m_FullRectBlt) t->flags16 |= FULLRECTBLT;
	if(dlg->m_FixDCPalette) t->flags16 |= FIXDCPALETTE;
	if(dlg->m_MergeFrontBuffers) t->flags17 |= MERGEFRONTBUFFERS;
	if(dlg->m_RecoverExclusiveErr) t->flags17 |= RECOVEREXCLUSIVEERR;
	if(dlg->m_ForceDx1Hook) t->flags18 |= FORCEDX1HOOK;
	if(dlg->m_SurfaceUnlock) t->flags18 |= SURFACEUNLOCK;
	if(dlg->m_EmulateClipper) t->flags19 |= EMULATECLIPPER;
	if(dlg->m_UseBltFast) t->flags19 |= USEBLTFAST;
	if(dlg->m_NoDirectSound) t->flags19 |= NODIRECTSOUND;
	if(dlg->m_ForceEmulDriver) t->flags19 |= FORCEEMULDRIVER;
	if(dlg->m_FixBitMapColor) t->flags19 |= FIXBITMAPCOLOR;
	if(dlg->m_PlainBackBuffer) t->flags18 |= PLAINBACKBUFFER;
	if(dlg->m_CenterToWin) t->dflags |= CENTERTOWIN;
	if(dlg->m_DisableDelete) t->dflags |= DISABLEDELETE;
	if(dlg->m_DumpDShowGraph) t->dflags |= DUMPDSHOWGRAPH;
	if(dlg->m_ForceD3DCheckOK) t->dflags2 |= FORCED3DCHECKOK;
	if(dlg->m_LimitDdraw) t->flags7 |= LIMITDDRAW;
	if(dlg->m_SuppressOverlay) t->flags7 |= SUPPRESSOVERLAY;
	if(dlg->m_WDDMNoOverlay) t->flags19 |= WDDMNOOVERLAY;
	...
	if(dlg->m_NoIcons) t->dflags |= NOICONS;
	if(dlg->m_NoFillRect) t->flags4 |= NOFILLRECT;
	if(dlg->m_NonAntialiasedFonts) t->flags20 |= NONANTIALIASEDFONTS;
	...
	if(dlg->m_WinAutoRepaint) t->flags17 |= WINAUTOREPAINT;
	...
	if(dlg->m_FakePentium3) t->flags17 |= FAKEPENTIUM3;
	if(dlg->m_SafeAllocs) t->flags11 |= SAFEALLOCS;
	if(dlg->m_HookLegacyEvents) t->flags19 |= HOOKLEGACYEVENTS;
	if(dlg->m_HookLegacyWave) t->flags19 |= HOOKLEGACYWAVE;
	if(dlg->m_QualityFonts) t->flags8 |= QUALITYFONTS;
	if(dlg->m_ReplacePrivOps) t->flags5 |= REPLACEPRIVOPS;
	if(dlg->m_PossiblyProtected) t->flags17 |= POSSIBLYPROTECTED;
	if(dlg->m_InitialRes) t->flags7 |= INITIALRES;
	if(dlg->m_MaximumRes) t->flags7 |= MAXIMUMRES;
	if(dlg->m_MinimumRes) t->flags20 |= MINIMUMRES;
	if(dlg->m_CustomRes) t->flags10 |= CUSTOMRES;
	if(dlg->m_RemapSysFolders) t->flags11 |= REMAPSYSFOLDERS;
	if(dlg->m_ResetMultiVolume) t->flags18 |= RESETMULTIVOLUME;
	if(dlg->m_RemapRootFolder) t->flags19 |= REMAPROOTFOLDER;
	if(dlg->m_HandleCDLock) t->flags20 |= HANDLECDLOCK;
	if(dlg->m_SetCDAudioPath) t->flags14 |= SETCDAUDIOPATH;
	if(dlg->m_CDHack) t->flags17 |= CDHACK;
	if(dlg->m_IsDOS) t->flags19 |= ISDOSEXECUTABLE;
	if(dlg->m_IsWin16) t->flags19 |= ISWIN16EXECUTABLE;
	if(dlg->m_DefMoviesColor) t->flags19 |= DEFMOVIESCOLOR;
	if(dlg->m_BestMoviesColor) t->flags19 |= BESTMOVIESCOLOR;
	if(dlg->m_EmulateXMirroring) t->flags20 |= EMULATEXMIRRORING;
	...
	t->WaveVolume = dlg->m_WaveVolume;
	t->MidiVolume = dlg->m_MidiVolume;
	t->CDAVolume = dlg->m_CDAVolume;
	t->GeneralVolume = dlg->m_GeneralVolume;
	...
	t->CDADrive = dlg->m_CDADriveLetter; 
	if((t->CDADrive >= 'D') && (t->CDADrive <= 'Z'))OutTrace("set CDADrive=%c:\n", t->CDADrive);
	t->CodePage = dlg->m_CodePage;
	t->Locale = dlg->m_Locale;
	...
	switch(dlg->m_CPUVendorId){
		case 0: break;
		case 1: t->dflags |= CPUFAKEDXWNDCPU; break;
		case 2: t->dflags |= CPUGENUINEINTEL; break;
		case 3: t->dflags |= CPUAUTHENTICAMD; break;
		case 4: t->dflags |= CPUCYRIXINSTEAD; break;
	}
}
...
static void SetDlgFromTarget(TARGETMAP *t, CTargetDlg *dlg)
{
	...
	dlg->m_OutOnTempFolder = t->tflags & OUTONTEMPFOLDER ? 1 : 0;
	dlg->m_OutAllErrors = t->tflags & OUTALLERRORS ? 1 : 0;
	dlg->m_AssertDialog = t->tflags & ASSERTDIALOG ? 1 : 0;

	//dlg->m_OutDebug = t->tflags & LOGDEBUG ? 1 : 0;
	dlg->m_OutWinMessages = t->tflags2 & OUTWINMESSAGES ? 1 : 0;
	dlg->m_OutDWTrace = t->tflags2 & OUTDXWINTRACE ? 1 : 0;
	dlg->m_OutOGLTrace = t->tflags2 & OUTOGLTRACE ? 1 : 0;
	dlg->m_OutSDLTrace = t->tflags2 & OUTSDLTRACE ? 1 : 0;
	dlg->m_OutTimeTrace = t->tflags2 & OUTTIMETRACE ? 1 : 0;
	dlg->m_OutSoundTrace = t->tflags2 & OUTSOUNDTRACE ? 1 : 0;
	dlg->m_OutInputs = t->tflags2 & OUTINPUTS ? 1 : 0;
	dlg->m_OutLocale = t->tflags2 & OUTLOCALE ? 1 : 0;
	dlg->m_OutFPS = t->tflags2 & OUTFPS ? 1 : 0;
	dlg->m_OutSysLibs = t->tflags2 & OUTSYSLIBS ? 1 : 0;
	dlg->m_OutWGTrace = t->tflags2 & OUTWINGTRACE ? 1 : 0;
	dlg->m_OutCOMTrace = t->tflags2 & OUTCOMTRACE ? 1 : 0;
	dlg->m_OutDDRAWTrace = t->tflags2 & OUTDDRAWTRACE ? 1 : 0;
	dlg->m_OutD3DTrace = t->tflags2 & OUTD3DTRACE ? 1 : 0;
	dlg->m_CursorTrace = t->tflags2 & OUTCURSORTRACE ? 1 : 0;
	dlg->m_ImportTable = t->tflags2 & OUTIMPORTTABLE ? 1 : 0;
	dlg->m_OutVGA = t->tflags2 & OUTVGA ? 1 : 0;
	dlg->m_OutStdIO = t->tflags2 & OUTSTDIO ? 1 : 0;
	dlg->m_RegistryOp = t->tflags2 & OUTREGISTRY ? 1 : 0;
	dlg->m_OutFileIO = t->tflags2 & OUTFILEIO ? 1 : 0;
	dlg->m_OutGDI = t->tflags2 & OUTGDI ? 1 : 0;
	dlg->m_OutDShow = t->tflags2 & OUTDSHOW ? 1 : 0;
	dlg->m_OutDrv = t->tflags2 & OUTDRV ? 1 : 0;
	dlg->m_TraceHooks = t->tflags2 & TRACEHOOKS ? 1 : 0;

	// ... logs

	dlg->m_DXVersion = (t->dxversion > 6) ? t->dxversion - 5 : t->dxversion;
	...
	dlg->m_EnablePatcher = t->flags18 & ENABLEPATCHER ? 1 : 0;
	dlg->m_SharedHook = t->flags13 & SHAREDHOOK ? 1 : 0;
	dlg->m_HookVideoAdapter = t->flags16 & HOOKVIDEOADAPTER ? 1 : 0;
	dlg->m_RunByDxWnd = t->flags16 & RUNBYDXWND ? 1 : 0;
	dlg->m_CopyNoShims = t->flags7 & CLEARSHIMS ? 1 : 0;
	dlg->m_DisableGhosting = t->flags20 & DISABLEGHOSTING ? 1 : 0;
	...
	dlg->m_ForceWindowing = t->flags3 & FORCEWINDOWING ? 1 : 0;
	...
	dlg->m_CenterOnExit = t->flags20 & CENTERONEXIT ? 1 : 0;
	dlg->m_AdaptMouseSpeed = t->flags12 & ADAPTMOUSESPEED ? 1 : 0;
	dlg->m_LockCursorIcons = t->flags14 & LOCKCURSORICONS ? 1 : 0;
	dlg->m_LockSysCursorIcons = t->flags16 & LOCKSYSCURSORICONS ? 1 : 0;
	dlg->m_RecoverSysCursorIcons = t->flags16 & RECOVERSYSCURSORICONS ? 1 : 0;
	...
	dlg->m_GetAllMessages = t->flags20 & GETALLMESSAGES ? 1 : 0;
	...
	if(t->flags15 & INJECTAPC) dlg->m_InjectionMode = 4;
	if(t->flags19 & STARTDETOURS) dlg->m_InjectionMode = 5;
	//if(t->flags15 & INJECTAPC2) dlg->m_InjectionMode = 5;
	dlg->m_EarlyHook = (dlg->m_InjectionMode != 0);
	...
	if(t->flags15 & GDIASYNCDC ) dlg->m_DCEmulationMode = 4;

	dlg->m_ResTypes = 0;
	if(t->flags4 & SUPPORTSVGA) dlg->m_ResTypes = 0;
	if(t->flags4 & SUPPORTHDTV) dlg->m_ResTypes = 1;
	if((t->flags4 & SUPPORTHDTV) && (t->flags4 & SUPPORTSVGA)) dlg->m_ResTypes = 2;
	if(t->flags4 & NATIVERES) dlg->m_ResTypes = 3;
	...
	if(t->flags5 & TEXTUREDUMP) dlg->m_TextureHandling = 1;
	if(t->flags5 & TEXTURETRANSP) dlg->m_TextureHandling = 2;
	if(t->flags5 & TEXTUREHIGHLIGHT) dlg->m_TextureHandling = 3;
	if(t->flags5 & TEXTUREHACK) dlg->m_TextureHandling = 4;
	...
	if(t->flags19 & DETOURSSON) dlg->m_SonProcessMode = 4;

	dlg->m_HookMode = 0;
	if(t->flags4 & HOTPATCH) dlg->m_HookMode = 1;
	if(t->flags15 & HOTPATCHALWAYS) dlg->m_HookMode = 2;
	...
	dlg->m_ForceDevType = 0;
	if(t->flags17 & FORCEDEVTYPE1) dlg->m_ForceDevType += 1;
	if(t->flags17 & FORCEDEVTYPE2) dlg->m_ForceDevType += 2;
	if(t->flags17 & FORCEDEVTYPE3) dlg->m_ForceDevType += 4;
	...
	dlg->m_CDEmulationMode = 0;
	if(t->flags15 & PLAYFROMCD) dlg->m_CDEmulationMode = 1;
	if(t->flags15 & CDAUTOMATICRIP) dlg->m_CDEmulationMode = 2;
	if((t->flags8 & VIRTUALCDAUDIO) && (dlg->m_CDEmulationMode == 0)) dlg->m_CDEmulationMode = 3;

	dlg->m_CDEmulationVersion = 1;
	if(t->flags15 & EMULATEWIN9XMCI) dlg->m_CDEmulationVersion = 0;
	if(t->flags15 & EMULATEVISTAMCI) dlg->m_CDEmulationVersion = 2;

	dlg->m_ZBufferMode = 0;
	if(t->flags17 & FORCEZBUFFERON) dlg->m_ZBufferMode = 1;
	if(t->flags17 & FORCEZBUFFEROFF) dlg->m_ZBufferMode = 2;

	dlg->m_DitherMode = 0;
	if(t->flags17 & FORCEDITHERING) dlg->m_DitherMode = 1;
	if(t->flags17 & CLEARDITHERING) dlg->m_DitherMode = 2;

	dlg->m_OverlayMode = 0;
	if(t->flags16 & EMULATEOVERLAY) dlg->m_OverlayMode = 1;
	if(t->flags16 & TRANSPARENTOVERLAY) dlg->m_OverlayMode = 2;

	dlg->m_FlipMode = 0;
	if(t->flags6 & HALFFLIPEMULATION) dlg->m_FlipMode = 1;
	if(t->flags4 & FULLFLIPEMULATION) dlg->m_FlipMode = 2;

	dlg->m_CoopMode = 0;
	if(t->flags18 & LOCKNORMALCOOP) dlg->m_CoopMode = 1;
	if(t->flags18 & LOCKEXCLUSIVECOOP) dlg->m_CoopMode = 2;
	...
	dlg->m_SharedMouse = t->flags17 & SHAREDMOUSE ? 1 : 0;
	dlg->m_PackMouseData = t->flags18 & PACKMOUSEDATA ? 1 : 0;
	...
	dlg->m_JoystickEffects = t->flags16 & JOYSTICKEFFECTS ? 1 : 0;
	...
	dlg->m_ForceNOPs = t->flags16 & FORCENOPS ? 1 : 0;
	...
	dlg->m_EmulateCDMixer = t->flags13 & EMULATECDMIXER ? 1 : 0;
	dlg->m_EmulateCDAux = t->flags13 & EMULATECDAUX ? 1 : 0;
	dlg->m_HideMuteControls = t->flags13 & HIDEMUTECONTROLS ? 1 : 0;
	dlg->m_PlaySoundFix = t->flags13 & PLAYSOUNDFIX ? 1 : 0;
	dlg->m_StopSound = t->flags14 & STOPSOUND ? 1 : 0;
	dlg->m_FixDefaultMCIId = t->flags11 & FIXDEFAULTMCIID ? 1 : 0;
	dlg->m_LockCDTray = t->flags12 & LOCKCDTRAY ? 1 : 0;
	dlg->m_BypassWaveOutPos = t->flags13 & BYPASSWAVEOUTPOS ? 1 : 0;
	...
	dlg->m_EmulateSoundPan = t->flags18 & EMULATESOUNDPAN ? 1 : 0;
	dlg->m_NoSoundLoop = t->flags14 & NOSOUNDLOOP ? 1 : 0;
	dlg->m_FixDSoundBuffer = t->flags19 & FIXDSOUNDBUFFER ? 1 : 0;
	dlg->m_BypassDSound = t->flags18 & BYPASSDSOUND ? 1 : 0;
	dlg->m_NoWaveOut = t->flags19 & NOWAVEOUT ? 1 : 0;
	dlg->m_MCISingleThreaded = t->flags20 & MCISINGLETHREADED ? 1 : 0;
	dlg->m_DSInitVolume = t->flags12 & DSINITVOLUME ? 1 : 0;
	dlg->m_LockVolume = t->flags9 & LOCKVOLUME ? 1 : 0;
	dlg->m_EmulateVolume = t->flags15 & EMULATEVOLUME ? 1 : 0;
	dlg->m_SafeMidiOut = t->flags11 & SAFEMIDIOUT ? 1 : 0;
	dlg->m_MidiAutoRepair = t->flags13 & MIDIAUTOREPAIR ? 1 : 0;
	dlg->m_SetCDVolume = t->flags10 & SETCDVOLUME ? 1 : 0;
	//dlg->m_ForceMCISysInfo = t->flags18 & FORCEMCISYSINFO ? 1 : 0;
	...
	dlg->m_SDLFixMouse = t->flags18 & SDLFIXMOUSE ? 1 : 0;
	...
	dlg->m_HookDirectShow = t->flags17 & HOOKDIRECTSHOW ? 1 : 0;
	dlg->m_ScaleDirectShow = t->flags18 & SCALEDIRECTSHOW ? 1 : 0;
	dlg->m_IgnoreAddFilter = t->flags18 & IGNOREADDFILTER ? 1 : 0;
	dlg->m_SuppressIVMR = t->flags18 & SUPPRESSIVMR ? 1 : 0;
	dlg->m_IgnoreGraphErrors = t->flags18 & IGNOREGRAPHERRORS ? 1 : 0;
	...
	dlg->m_NoRampDevice = t->flags14 & NORAMPDEVICE ? 1 : 0;
	dlg->m_FogVertexCap = t->flags16 & FOGVERTEXCAP ? 1 : 0;
	dlg->m_FogTableCap = t->flags16 & FOGTABLECAP ? 1 : 0;
	dlg->m_NoRGBDevice = t->flags14 & NORGBDEVICE ? 1 : 0;
	dlg->m_NoMMXDevice = t->flags14 & NOMMXDEVICE ? 1 : 0;
	...
	dlg->m_LockPixelFormat = t->flags17 & LOCKPIXELFORMAT ? 1 : 0;
	dlg->m_FixAspectRatio = t->flags13 & FIXASPECTRATIO ? 1 : 0;
	...
	dlg->m_FixFileDialog = t->flags15 & FIXFILEDIALOG ? 1 : 0;
	dlg->m_MIDISetInstrument = t->flags15 & MIDISETINSTRUMENT ? 1 : 0;
	dlg->m_IgnoreScheduler = t->flags15 & IGNORESCHEDULER ? 1 : 0;
	dlg->m_SlowDownExceptions = t->flags15 & SLOWDOWNEXCEPTIONS ? 1 : 0;
	dlg->m_DuplicateHandleFix = t->flags15 & DUPLICATEHANDLEFIX ? 1 : 0;
	dlg->m_EmulateWin95 = t->flags15 & EMULATEWIN95 ? 1 : 0;
	dlg->m_ForceHalftoneTiny = t->flags15 & FORCEHALFTONETINY ? 1 : 0;
	dlg->m_FixFullWinStyle = t->flags15 & FIXFULLWINSTYLE ? 1 : 0;
	dlg->m_HideDisplayModes = t->flags15 & HIDEDISPLAYMODES ? 1 : 0;
	//dlg->m_GDIAsyncDC = t->flags15 & GDIASYNCDC ? 1 : 0;
	dlg->m_Ramp2RGBDevice = t->flags16 & RAMP2RGBDEVICE ? 1 : 0;
	dlg->m_Ramp2MMXDevice = t->flags16 & RAMP2MMXDEVICE ? 1 : 0;
	dlg->m_CullModeNone = t->flags15 & CULLMODENONE ? 1 : 0;
	dlg->m_NoSurfaceNew = t->flags16 & NOSURFACENEW ? 1 : 0;
	dlg->m_EmulateWin9XHeap = t->flags16 & EMULATEWIN9XHEAP ? 1 : 0;
	dlg->m_ForceD3DRefDevice = t->flags16 & FORCED3DREFDEVICE ? 1 : 0;
	dlg->m_FixCreateFileMap = t->flags16 & FIXCREATEFILEMAP ? 1 : 0;
	dlg->m_EmuGetCommandLine = t->flags16 & EMUGETCOMMANDLINE ? 1 : 0;
	dlg->m_MCINotifyTopmost = t->flags16 & MCINOTIFYTOPMOST ? 1 : 0;
	dlg->m_ForceSimpleWindow = t->flags16 & FORCESIMPLEWINDOW ? 1 : 0;
	dlg->m_ConfirmEscape = t->flags16 & CONFIRMESCAPE ? 1 : 0;
	dlg->m_CDROMDriveType = t->flags3 & CDROMDRIVETYPE ? 1 : 0;
	dlg->m_HideCDROMEmpty = t->flags4 & HIDECDROMEMPTY ? 1 : 0;
	dlg->m_HideCDROMReal = t->flags17 & HIDECDROMREAL ? 1 : 0;
	...
	dlg->m_DisableWinHook = t->dflags2 & DISABLEWINHOOK ? 1 : 0;
	...
	dlg->m_Experimental2 = t->dflags2 & EXPERIMENTAL2 ? 1 : 0;
	dlg->m_Experimental3 = t->dflags2 & EXPERIMENTAL3 ? 1 : 0;
	dlg->m_Experimental4 = t->dflags2 & EXPERIMENTAL4 ? 1 : 0;
	dlg->m_Experimental5 = t->dflags2 & EXPERIMENTAL5 ? 1 : 0;
	dlg->m_Experimental6 = t->dflags2 & EXPERIMENTAL6 ? 1 : 0;
	dlg->m_Experimental7 = t->dflags2 & EXPERIMENTAL7 ? 1 : 0;
	dlg->m_Experimental8 = t->dflags2 & EXPERIMENTAL8 ? 1 : 0;
	dlg->m_FixRandomPalette = t->dflags2 & FIXRANDOMPALETTE ? 1 : 0;
	dlg->m_NoPower2Fix = t->flags4 & NOPOWER2FIX ? 1 : 0;
	dlg->m_NoMultiSample = t->flags17 & NOMULTISAMPLE ? 1 : 0;
	dlg->m_ForceVBufOnSysMem = t->flags17 & FORCEVBUFONSYSMEM ? 1 : 0;
	dlg->m_ZBufOnSysMemory = t->flags17 & ZBUFONSYSMEMORY ? 1 : 0;
	dlg->m_ZBufOnVidMemory = t->flags19 & ZBUFONVIDMEMORY ? 1 : 0;
	dlg->m_TrimVertexBuffer = t->flags18 & TRIMVERTEXBUFFER ? 1 : 0;
	dlg->m_DepthBufZClean = t->flags18 & DEPTHBUFZCLEAN ? 1 : 0;
	dlg->m_EnableZooming = t->flags18 & ENABLEZOOMING ? 1 : 0;
	dlg->m_FilterRGBTextures = t->flags18 & FILTERRGBTEXTURES ? 1 : 0;
	dlg->m_NoD3DOptimize = t->flags18 & NOD3DOPTIMIZE ? 1 : 0;
	dlg->m_TrimD3DPoints = t->flags19 & TRIMD3DPOINTS ? 1 : 0;
	dlg->m_AltPixelCenter = t->flags18 & ALTPIXELCENTER ? 1 : 0;
	dlg->m_FixColorKey = t->flags18 & FIXCOLORKEY ? 1 : 0;
	dlg->m_StretchFullRect = t->flags14 & STRETCHFULLRECT ? 1 : 0;
	dlg->m_ColorKeyToAlpha = t->flags16 & COLORKEYTOALPHA ? 1 : 0;
	dlg->m_ForceWBasedFog = t->flags16 & FORCEWBASEDFOG ? 1 : 0;
	dlg->m_ForceWBasedFogWDDM = t->flags19 & FORCEWBASEDFOGWDDM ? 1 : 0;
	dlg->m_ForceD3D9on12 = t->flags16 & FORCED3D9ON12 ? 1 : 0;
	dlg->m_FixTnLRhw = t->flags16 & FIXTNLRHW ? 1 : 0;
	dlg->m_MapMemB0000 = t->flags16 & MAPMEMB0000 ? 1 : 0;
	dlg->m_TrimChildWindows = t->flags16 & TRIMCHILDWINDOWS ? 1 : 0;
	dlg->m_TrimMainWindow = t->flags16 & TRIMMAINWINDOW ? 1 : 0;
	dlg->m_FixAILBug = t->flags17 & FIXAILBUG ? 1 : 0;
	dlg->m_LockCursorShape = t->flags17 & LOCKCURSORSHAPE ? 1 : 0;
	dlg->m_FixOverlappedResult = t->flags17 & FIXOVERLAPPEDRESULT ? 1 : 0;
	dlg->m_ForceFilterNearest = t->flags17 & FORCEFILTERNEAREST ? 1 : 0;
	dlg->m_HideDirectDraw = t->flags17 & HIDEDIRECTDRAW ? 1 : 0;
	dlg->m_CompensateOpenGLCopy = t->flags17 & COMPENSATEOGLCOPY ? 1 : 0;
	dlg->m_EmuDDSyncShim = t->flags18 & EMUDDSYNCSHIM ? 1 : 0;
	dlg->m_StretchBackBuffer = t->flags18 & STRETCHBACKBUFFER ? 1 : 0;
	dlg->m_LegacyKernel32 = t->flags19 & LEGACYKERNEL32 ? 1 : 0;
	dlg->m_DDrawLegacyCallback = t->flags19 & DDRAWLEGACYCALLBACK ? 1 : 0;
	dlg->m_AsyncWaveOutOpen = t->flags19 & ASYNCWAVEOUTOPEN ? 1 : 0;
	dlg->m_AsyncWaveOutClose = t->flags19 & ASYNCWAVEOUTCLOSE ? 1 : 0;
	dlg->m_TransparentDialog = t->flags19 & TRANSPARENTDIALOG ? 1 : 0;
	dlg->m_EmulateWinResize = t->flags19 & EMULATEWINRESIZE ? 1 : 0;
	dlg->m_DisableLayeredWin = t->flags19 & DISABLELAYEREDWIN ? 1 : 0;
	dlg->m_ScaleChildWin = t->flags19 & SCALECHILDWIN ? 1 : 0;
	dlg->m_TrimTextYPos = t->flags19 & TRIMTEXTYPOS ? 1 : 0;
	dlg->m_NativeDialogs = t->flags16 & NATIVEDIALOGS ? 1 : 0;
	dlg->m_NoPerfCounter = t->flags4 & NOPERFCOUNTER ? 1 : 0;
	dlg->m_DisableFogging = t->flags4 & DISABLEFOGGING ? 1 : 0;
	dlg->m_ControlFogging = t->flags13 & CONTROLFOGGING ? 1 : 0;
	dlg->m_SetFogColor = t->flags16 & SETFOGCOLOR ? 1 : 0;
	...
	//dlg->m_EmulateOverlay = t->flags16 & EMULATEOVERLAY ? 1 : 0;
	//dlg->m_TransparentOverlay = t->flags16 & TRANSPARENTOVERLAY ? 1 : 0;
	...
	dlg->m_PathLocale = t->flags12 & PATHLOCALE ? 1 : 0;
	dlg->m_NoSetText = t->flags14 & NOSETTEXT ? 1 : 0;
	dlg->m_FixKeyboardType = t->flags14 & FIXKEYBOARDTYPE ? 1 : 0;
	...
	dlg->m_ForceColorKeyOff = t->flags13 & FORCECOLORKEYOFF ? 1 : 0;
	dlg->m_SpongeBobHack = t->flags13 & SPONGEBOBHACK ? 1 : 0;
	dlg->m_DSoundReplace = t->flags13 & DSOUNDREPLACE ? 1 : 0;
	dlg->m_FightingForceFix = t->flags13 & FIGHTINGFORCEFIX ? 1 : 0;
	dlg->m_CenterDialogs = t->flags13 & CENTERDIALOGS ? 1 : 0;
	dlg->m_DxVersionLie = t->flags13 & DXVERSIONLIE ? 1 : 0;
	dlg->m_LegacyBaseUnits = t->flags13 & LEGACYBASEUNITS ? 1 : 0;
	dlg->m_PatchMSVBVM = t->flags13 & PATCHMSVBVM ? 1 : 0;
	dlg->m_XRayTweak = t->flags13 & XRAYTWEAK ? 1 : 0;
	dlg->m_FixDCColorDepth = t->flags13 & FIXDCCOLORDEPTH ? 1 : 0;
	dlg->m_DefuseFlashBomb = t->flags13 & DEFUSEFLASHBOMB ? 1 : 0;
	dlg->m_LockAllWindows = t->flags13 & LOCKALLWINDOWS ? 1 : 0;
	dlg->m_IgnoreFSysErrors = t->flags13 & IGNOREFSYSERRORS ? 1 : 0;
	dlg->m_HeapPadAllocation = t->flags14 & HEAPPADALLOCATION ? 1 : 0;
	dlg->m_HeapZeroMemory = t->flags19 & HEAPZEROMEMORY ? 1 : 0;
	dlg->m_DisableColorKey = t->flags14 & DISABLECOLORKEY ? 1 : 0;
	dlg->m_NoShellExecute = t->flags14 & NOSHELLEXECUTE ? 1 : 0;
	dlg->m_CustomPerfCount = t->flags14 & CUSTOMPERFCOUNT ? 1 : 0;
	dlg->m_MidiOutBypass = t->flags14 & MIDIOUTBYPASS ? 1 : 0;
	dlg->m_FixedBitBlt = t->flags14 & FIXEDBITBLT ? 1 : 0;
	dlg->m_HeapLeak = t->flags14 & HEAPLEAK ? 1 : 0;
	dlg->m_MW2WaveOutFix = t->flags14 & MW2WAVEOUTFIX ? 1 : 0;
	dlg->m_SafeHeap = t->flags14 & SAFEHEAP ? 1 : 0;
	dlg->m_RepairHeap = t->flags18 & REPAIRHEAP ? 1 : 0;
	dlg->m_Win16CreateFile = t->flags14 & WIN16CREATEFILE ? 1 : 0;
	dlg->m_Win16FindFileFix = t->flags14 & WIN16FINDFILEFIX ? 1 : 0;
	dlg->m_SlowCDStatus = t->flags14 & SLOWCDSTATUS ? 1 : 0;
	dlg->m_WaveOutUsePrefDev = t->flags14 & WAVEOUTUSEPREFDEV ? 1 : 0;
	dlg->m_FixMacromediaReg = t->flags14 & FIXMACROMEDIAREG ? 1 : 0;
	dlg->m_LockSystemMenu = t->flags14 & LOCKSYSTEMMENU ? 1 : 0;
	dlg->m_StickyWindows = t->flags14 & STICKYWINDOWS ? 1 : 0;
	dlg->m_DumpText = t->flags14 & DUMPTEXT ? 1 : 0;
	//dlg->m_EmulateWin9XMCI = t->flags15 & EMULATEWIN9XMCI ? 1 : 0;
	dlg->m_EmulateFloppyDrive = t->flags15 & EMULATEFLOPPYDRIVE ? 1 : 0;
	dlg->m_FilterColorKey = t->flags18 & FILTERCOLORKEY ? 1 : 0;
	dlg->m_ForceColorKey = t->flags18 & FORCECOLORKEY ? 1 : 0;
	dlg->m_ColorKey2Palette = t->flags18 & COLORKEY2PALETTE ? 1 : 0;
	dlg->m_FastPrimaryUpdate = t->flags15 & FASTPRIMARYUPDATE ? 1 : 0;
	dlg->m_LimitFreeDisk = t->flags15 & LIMITFREEDISK ? 1 : 0;
	dlg->m_LimitFreeRAM = t->flags15 & LIMITFREERAM ? 1 : 0;
	dlg->m_LimitVideoRAM = t->flags15 & LIMITVIDEORAM ? 1 : 0;
	dlg->m_LimitTextureRAM = t->flags15 & LIMITTEXTURERAM ? 1 : 0;
	dlg->m_EmulateWinHelp = t->flags17 & EMULATEWINHELP ? 1 : 0;
	dlg->m_SuppressDEP = t->flags17 & SUPPRESSDEP ? 1 : 0;
	dlg->m_DeferredWinPos = t->flags15 & DEFERREDWINPOS ? 1 : 0;
	dlg->m_DispelTweak = t->flags15 & DISPELTWEAK ? 1 : 0;
	dlg->m_DisablePSGP = t->flags13 & DISABLEPSGP ? 1 : 0;
	dlg->m_DisableD3DMMX = t->flags13 & DISABLED3DMMX ? 1 : 0;
	dlg->m_DisableD3DxPSGP = t->flags13 & DISABLED3DXPSGP ? 1 : 0;
	dlg->m_D3DXDoNotMute = t->flags13 & D3DXDONOTMUTE ? 1 : 0;
	dlg->m_DisableDEP = t->flags13 & DISABLEDEP ? 1 : 0;
	...
	dlg->m_ForceHalfTone = t->flags15 & FORCEHALFTONE ? 1 : 0;
	...
	dlg->m_NoLAVFilters = t->flags18 & NOLAVFILTERS ? 1 : 0;
	dlg->m_FixMoviesColor = t->flags6 & FIXMOVIESCOLOR ? 1 : 0;
	dlg->m_StretchMovies = t->flags6 & STRETCHMOVIES ? 1 : 0;
	dlg->m_BypassActiveMovie = t->flags18 & BYPASSACTIVEMOVIE ? 1 : 0;
	...
	dlg->m_TrimMousePosition = t->flags & TRIMMOUSEPOSITION ? 1 : 0;
	...
	dlg->m_MarkClipRgn = t->dflags & MARKCLIPRGN ? 1 : 0;
	dlg->m_DumpDIBSection = t->dflags & DUMPDIBSECTION ? 1 : 0;
	dlg->m_CaptureScreens = t->dflags & CAPTURESCREENS ? 1 : 0;
	dlg->m_PatchExecuteBuffer = t->flags17 & PATCHEXECUTEBUFFER ? 1 : 0;
	dlg->m_DumpDevContext = t->dflags & DUMPDEVCONTEXT ? 1 : 0;
	dlg->m_DumpCPUID = t->dflags & DUMPCPUID ? 1 : 0;
	dlg->m_ForceXMirroring = t->dflags2 & FORCEXMIRRORING ? 1 : 0;
	dlg->m_ForceYMirroring = t->dflags2 & FORCEYMIRRORING ? 1 : 0;
	...
	dlg->m_SetPriorityLow = t->flags15 & SETPRIORITYLOW ? 1 : 0;
	dlg->m_KillVSync = t->flags12 & KILLVSYNC ? 1 : 0;
	dlg->m_FakeDate = t->flags16 & FAKEDATE ? 1 : 0;
	dlg->m_UpTimeClear = t->flags14 & UPTIMECLEAR ? 1 : 0;
	dlg->m_UpTimeStress = t->flags14 & UPTIMESTRESS ? 1 : 0;
	...
	dlg->m_FixMouseBiasX = t->flags17 & FIXMOUSEBIASX ? 1 : 0;
	dlg->m_FixMouseBiasY = t->flags17 & FIXMOUSEBIASY ? 1 : 0;
	...
	dlg->m_UseNanosleep = t->flags13 & USENANOSLEEP ? 1 : 0;
	dlg->m_CustomTimeShift = t->flags13 & CUSTOMTIMESHIFT ? 1 : 0;
	dlg->m_LimitFrequency = t->flags14 & LIMITFREQUENCY ? 1 : 0;
	...
	dlg->m_GLExtensionsTrim = t->flags13 & GLEXTENSIONSTRIM ? 1 : 0;
	dlg->m_GLFixClamp = t->flags13 & GLFIXCLAMP ? 1 : 0;
	dlg->m_NoScissorTest = t->flags18 & NOSCISSORTEST ? 1 : 0;
	dlg->m_Sample2Coverage = t->flags14 & SAMPLE2COVERAGE ? 1 : 0;
	...
	dlg->m_AsyncBlitMode = t->flags15 & ASYNCBLITMODE ? 1 : 0;
	dlg->m_FakeVersion = t->flags2 & FAKEVERSION ? 1 : 0;
	dlg->m_FullRectBlt = t->flags16 & FULLRECTBLT ? 1 : 0;
	dlg->m_FixDCPalette = t->flags16 & FIXDCPALETTE ? 1 : 0;
	dlg->m_MergeFrontBuffers = t->flags17 & MERGEFRONTBUFFERS ? 1 : 0;
	dlg->m_RecoverExclusiveErr = t->flags17 & RECOVEREXCLUSIVEERR ? 1 : 0;
	dlg->m_ForceDx1Hook = t->flags18 & FORCEDX1HOOK ? 1 : 0;
	dlg->m_SurfaceUnlock = t->flags18 & SURFACEUNLOCK ? 1 : 0;
	dlg->m_EmulateClipper = t->flags19 & EMULATECLIPPER ? 1 : 0;
	dlg->m_UseBltFast = t->flags19 & USEBLTFAST ? 1 : 0;
	dlg->m_NoDirectSound = t->flags19 & NODIRECTSOUND ? 1 : 0;
	dlg->m_ForceEmulDriver = t->flags19 & FORCEEMULDRIVER ? 1 : 0;
	dlg->m_FixBitMapColor = t->flags19 & FIXBITMAPCOLOR ? 1 : 0;
	dlg->m_PlainBackBuffer = t->flags18 & PLAINBACKBUFFER ? 1 : 0;
	dlg->m_CenterToWin = t->dflags & CENTERTOWIN ? 1 : 0;
	dlg->m_DisableDelete = t->dflags & DISABLEDELETE ? 1 : 0;
	dlg->m_DumpDShowGraph = t->dflags & DUMPDSHOWGRAPH ? 1 : 0;
	dlg->m_ForceD3DCheckOK = t->dflags2 & FORCED3DCHECKOK ? 1 : 0;
	dlg->m_LimitDdraw = t->flags7 & LIMITDDRAW ? 1 : 0;
	dlg->m_SuppressOverlay = t->flags7 & SUPPRESSOVERLAY ? 1 : 0;
	dlg->m_WDDMNoOverlay = t->flags19 & WDDMNOOVERLAY ? 1 : 0;
	...
	dlg->m_NoIcons = t->dflags & NOICONS ? 1 : 0;
	dlg->m_NoFillRect = t->flags4 & NOFILLRECT ? 1 : 0;
	dlg->m_NonAntialiasedFonts = t->flags20 & NONANTIALIASEDFONTS ? 1 : 0;
	...
	dlg->m_WinAutoRepaint = t->flags17 & WINAUTOREPAINT ? 1 : 0;
	...
	dlg->m_FakePentium3 = t->flags17 & FAKEPENTIUM3 ? 1 : 0;
	dlg->m_SafeAllocs = t->flags11 & SAFEALLOCS ? 1 : 0;
	dlg->m_HookLegacyEvents = t->flags19 & HOOKLEGACYEVENTS ? 1 : 0;
	dlg->m_HookLegacyWave = t->flags19 & HOOKLEGACYWAVE ? 1 : 0;
	dlg->m_QualityFonts = t->flags8 & QUALITYFONTS ? 1 : 0;
	dlg->m_ReplacePrivOps = t->flags5 & REPLACEPRIVOPS ? 1 : 0;
	dlg->m_PossiblyProtected = t->flags17 & POSSIBLYPROTECTED ? 1 : 0;
	dlg->m_InitialRes = t->flags7 & INITIALRES ? 1 : 0;
	dlg->m_MaximumRes = t->flags7 & MAXIMUMRES ? 1 : 0;
	dlg->m_MinimumRes = t->flags20 & MINIMUMRES ? 1 : 0;
	dlg->m_CustomRes = t->flags10 & CUSTOMRES ? 1 : 0;
	dlg->m_RemapSysFolders = t->flags11 & REMAPSYSFOLDERS ? 1 : 0;
	dlg->m_ResetMultiVolume = t->flags18 & RESETMULTIVOLUME ? 1 : 0;
	dlg->m_RemapRootFolder = t->flags19 & REMAPROOTFOLDER ? 1 : 0;
	dlg->m_HandleCDLock = t->flags20 & HANDLECDLOCK ? 1 : 0;
	dlg->m_SetCDAudioPath = t->flags14 & SETCDAUDIOPATH ? 1 : 0;
	dlg->m_CDHack = t->flags17 & CDHACK ? 1 : 0;
	dlg->m_IsDOS = t->flags19 & ISDOSEXECUTABLE ? 1 : 0;
	dlg->m_IsWin16 = t->flags19 & ISWIN16EXECUTABLE ? 1 : 0;
	dlg->m_DefMoviesColor = t->flags19 & DEFMOVIESCOLOR ? 1 : 0;
	dlg->m_BestMoviesColor = t->flags19 & BESTMOVIESCOLOR ? 1 : 0;
	dlg->m_EmulateXMirroring = t->flags20 & EMULATEXMIRRORING ? 1 : 0;
	...
	dlg->m_WaveVolume = t->WaveVolume;
	dlg->m_MidiVolume = t->MidiVolume;
	dlg->m_CDAVolume = t->CDAVolume;
	dlg->m_GeneralVolume = t->GeneralVolume;
	...
	dlg->m_CDADriveLetter = t->CDADrive;
	if((t->CDADrive >= 'D') && (t->CDADrive <= 'Z')) OutTrace("DEBUG: get CDADrive=%c:\n", t->CDADrive);

	// assign dlg->m_CDADriveIndex
	for (int i=0; AvailableCDDrives[i]; i++){
		if(AvailableCDDrives[i] == dlg->m_CDADriveLetter) {
			OutTrace("matched ordinal=%d cd=%c:\n", i);
			dlg->m_CDADriveId = i;
			break;
		}
	}
	...
	switch(t->dflags & CPUVENDORMASK){
		case 0: break;
		case CPUFAKEDXWNDCPU: dlg->m_CPUVendorId = 1; break;
		case CPUGENUINEINTEL: dlg->m_CPUVendorId = 2; break;
		case CPUAUTHENTICAMD: dlg->m_CPUVendorId = 3; break;
		case CPUCYRIXINSTEAD: dlg->m_CPUVendorId = 4; break;
	}
}

static char *StripRoot(char *s)
{
	static char tmp[MAX_PATH+2];
	if(!gStripRoot) return s;
	int rootLen = strlen(gProgramRootFolder);
	if(rootLen && !strncmp(s, gProgramRootFolder, rootLen)) {
		strncpy(tmp, ":", MAX_PATH);
		strncat(tmp, &s[rootLen], MAX_PATH);
		return tmp;
	}
	else return s;
}
...
static void SaveConfigItem(TARGETMAP *TargetMap, PRIVATEMAP *PrivateMap, int i, char *InitPath)
{
	...
	WritePrivateProfileString("target", key, StripRoot(TargetMap->path), InitPath);
	// -------
	sprintf_s(key, sizeof(key), "startfolder%i", i);
	WritePrivateProfileString("target", key, StripRoot(PrivateMap->startfolder), InitPath);
	// -------
	sprintf_s(key, sizeof(key), "launchpath%i", i);
	char *wrap = (char *)malloc(strlen(PrivateMap->launchpath) + 4); // +2 for quotes, +1 for terminator
	sprintf(wrap, "\"%s\"", StripRoot(PrivateMap->launchpath));
	WritePrivateProfileString("target", key, wrap, InitPath);
	free(wrap);
	// -------
	sprintf_s(key, sizeof(key), "cmdline%i", i);
	WritePrivateProfileString("target", key, PrivateMap->cmdline, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "msshims%i", i);
	WritePrivateProfileString("target", key, PrivateMap->msshims, InitPath);
	// -------
	...
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "flagv%i", i);
	sprintf_s(val, sizeof(val), "%i", TargetMap->flags17);
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "flagw%i", i);
	sprintf_s(val, sizeof(val), "%i", TargetMap->flags18);
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "flagx%i", i);
	sprintf_s(val, sizeof(val), "%i", TargetMap->flags19);
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "flagy%i", i);
	sprintf_s(val, sizeof(val), "%i", TargetMap->flags20);
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
	...
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "tflagb%i", i);
	sprintf_s(val, sizeof(val), "%i", TargetMap->tflags2);
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
	...
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "cdadrive%i", i);
	sprintf_s(val, sizeof(val), "%c:", TargetMap->CDADrive);
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "wavevol%i", i);
	sprintf_s(val, sizeof(val), "%i", TargetMap->WaveVolume);
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "midivol%i", i);
	sprintf_s(val, sizeof(val), "%i", TargetMap->MidiVolume);
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "cdvol%i", i);
	sprintf_s(val, sizeof(val), "%i", TargetMap->CDAVolume);
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "gvol%i", i);
	sprintf_s(val, sizeof(val), "%i", TargetMap->GeneralVolume);
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
	...
	sprintf_s(val, sizeof(val), "%d", TargetMap->Locale);
	WritePrivateProfileString("target", key, val, InitPath);
	// -------
...
static void SaveExportStamp(char *InitPath)
{
	...
	WritePrivateProfileString("stamp", "version", GetDllVersion(), InitPath);
...
static void ClearTarget(int i, char *InitPath)
{
	...
	char *keys[]={
		...
		"flagt", "flagu", "flagv", "flagw", "flagx", "flagy", "tflag", "initx", "cmdline","msshims",
...
static void ExpandRoot(char *s)
{
	if(s[0] == ':') {
		char tmp[MAX_PATH];
		strncpy(tmp, gProgramRootFolder, MAX_PATH);
		strncat(tmp, &s[1], MAX_PATH);
		strcpy(s, tmp);
	}
}

static int LoadLogFile(TARGETMAP *TargetMap, PRIVATEMAP *PrivateMap, int i, char *LogPath)
{
	MessageBox(0, "Load log file", "dxwnd", 0);
	char line[2048];
	int f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12, f13, f14, f15, f16, f17, f18, f19, f20;
	FILE *logfile = fopen(LogPath, "r");
	if(!logfile) return -1;
	fgets(line, 2048, logfile); // header
	MessageBox(0, line, "debug", 0);
	if(line[0] != '[') return -1;
	fgets(line, 2048, logfile); // flags - textual
	MessageBox(0, line, "debug", 0);
	if(line[0] != '[') return -1;
	fgets(line, 2048, logfile); // flags - numeric
	MessageBox(0, line, "debug", 0);
	if(line[0] != '[') return -1;
	if(sscanf(line, "[ %d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d:%d ]",
		&f1, &f2, &f3, &f4, &f5, &f6, &f7, &f8, &f9, &f10, &f11, &f12, &f13, &f14, &f15, &f16, &f17, &f18, &f19, &f20)
		!= 20) {
			MessageBox(0, "Warning: bad flags count", "DxWnd", 0);
		//return -1; v2.06.06 do the import anyway
	}
	...
	TargetMap->flags8 = f8;
	TargetMap->flags9 = f9;
	TargetMap->flags10 = f10;
	TargetMap->flags11 = f11;
	TargetMap->flags12 = f12;
	TargetMap->flags13 = f13;
	TargetMap->flags14 = f14;
	TargetMap->flags15 = f15;
	TargetMap->flags16 = f16;
	TargetMap->flags17 = f17;
	TargetMap->flags18 = f18;
	TargetMap->flags19 = f19;
	TargetMap->flags20 = f20;
	fclose(logfile); // v2.06.06 - unlock the log file
	return 0;
}
...
static int LoadConfigItem(TARGETMAP *TargetMap, PRIVATEMAP *PrivateMap, int i, char *InitPath)
{
	...
	// -------
	sprintf_s(key, sizeof(key), "flagv%i", i);
	TargetMap->flags17= GetPrivateProfileInt("target", key, 0, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "flagw%i", i);
	TargetMap->flags18= GetPrivateProfileInt("target", key, 0, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "flagx%i", i);
	TargetMap->flags19= GetPrivateProfileInt("target", key, 0, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "flagy%i", i);
	TargetMap->flags20= GetPrivateProfileInt("target", key, 0, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "tflag%i", i);
	TargetMap->tflags = GetPrivateProfileInt("target", key, 0, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "tflagb%i", i);
	TargetMap->tflags2 = GetPrivateProfileInt("target", key, 0, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "dflag%i", i);
	...
	// -------
	sprintf_s(key, sizeof(key), "cdadrive%i", i);
	GetPrivateProfileString("target", key, "?:", sBuf, 32, InitPath);
	char driveLetter = sBuf[0];
	if((driveLetter <= 'C') || (driveLetter > 'Z')) driveLetter = '?';
	TargetMap->CDADrive = driveLetter;
	// -------
	sprintf_s(key, sizeof(key), "wavevol%i", i);
	TargetMap->WaveVolume = GetPrivateProfileInt("target", key, 0, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "midivol%i", i);
	TargetMap->MidiVolume = GetPrivateProfileInt("target", key, 0, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "wavevol%i", i);
	TargetMap->WaveVolume = GetPrivateProfileInt("target", key, 0, InitPath);
	// -------
	...
	// -------
	sprintf_s(key, sizeof(key), "cdvol%i", i);
	TargetMap->CDAVolume = GetPrivateProfileInt("target", key, 0, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "gvol%i", i);
	TargetMap->GeneralVolume = GetPrivateProfileInt("target", key, 100, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "codepage%i", i);
	TargetMap->CodePage = GetPrivateProfileInt("target", key, 0, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "lang%i", i);
	TargetMap->Locale = GetPrivateProfileInt("target", key, 0, InitPath);
	// -------
	sprintf_s(key, sizeof(key), "country%i", i);
	...
	if(strlen(sBuf)){
		...
		if(Icon) {
			...
			// try to renew the icon
			//PrivateMap->updated = TRUE;
			Icon = ic->Extract(TargetMap->path);
			if(Icon){
				PrivateMap->Icon = ic->Extract(TargetMap->path);
				PrivateMap->IconString = ic->Serialize(Icon);
				if(PrivateMap->IconString){ 
					sprintf_s(key, sizeof(key), "icon%i", i);
					WritePrivateProfileString("target", key, PrivateMap->IconString.GetBuffer(), InitPath);
				} else {
...
static int SetTargetIcon(TARGETMAP tm)
{
	...
	if (tm.flags3 & HOOKENABLED) return (
			(tm.flags2 & STARTDEBUG)||
			(tm.flags7 & INJECTSUSPENDED)||
			(tm.flags12 & INJECTPROXY) ||
			(tm.flags15 & INJECTAPC) ||
			(tm.flags19 & STARTDETOURS)
		) ? 2 : 1;
	return 0;
}
...
CDxwndhostView::CDxwndhostView()
{
	...
	SystemParametersInfo(SPI_GETMOUSESPEED, 0, (PVOID)iInitialMouseSpeed, 0);
	// log values
	...
	OutTrace("Init InitialMouseSpeed=%d\n", iInitialMouseSpeed);
...
CDxwndhostView::~CDxwndhostView()
{
	...
	RecoverGammaRamp();
	//RecoverKayboardLayout(); -- to be fixed
}
...
void CDxwndhostView::OnKeyDown(UINT c, UINT nRepCnt, UINT Flags)
{
	...
	switch(c){
		case 'K': // v2.06.00 Ctrl+K shortcut kills the current task
...
void CDxwndhostView::OnExit()
{
	...
	RestoreMouseSpeed(iInitialMouseSpeed);
}
...
void CDxwndhostView::UpdateHotKeys()
{
	...
	for(i=0; HKeys[i].iLabelResourceId; i++){
	...
	lastHot = i;
	for(i=0; SKeys[i].iLabelResourceId; i++){
		dwKey = GetPrivateProfileInt("keymapping", SKeys[i].sIniLabel, 0, gInitPath);
		if(dwKey) {
			UnregisterHotKey(this->GetSafeHwnd(), i + lastHot);
			if(!RegisterHotKey(this->GetSafeHwnd(), i + lastHot, MOD_CONTROL, dwKey)) VKeyError(SKeys[i].sIniLabel);
		}
	}
}
...
void CDxwndhostView::AppendItemToList(TARGETMAP *t, PRIVATEMAP *p, int i, LV_ITEM *listitem)
{
	...
	if(p->Icon){
		if(!gGrayIcons || (FileExists(t->path) && (t->flags3 & HOOKENABLED)))
...
DWORD WINAPI ToolbarMon(LPVOID lpParameter)
{
	OutTrace("Toolbar monitor starting ...\n");
	while(TRUE){
		Sleep(1000);
		HANDLE hMutex=OpenSemaphoreA(NULL, FALSE, "Global\\DxWnd TOOLBAR");
		if(hMutex){
			//OutTrace("OpenSemaphore succeeded\n");
			CloseHandle(hMutex);
			continue;
		}
		else {
			//OutTrace("OpenSemaphore failed err=%d\n", GetLastError());
			if(!IsToolbarWndVisible()){
				OutTrace("Showing toolbar ...\n");
...
void CheckRunningTask(TARGETMAP *tm, PRIVATEMAP *pm)
{
	PROCESSENTRY32 processInfo;
	DWORD (WINAPI *lpfGetModuleFileName)( HANDLE, HMODULE, LPTSTR, DWORD ) = NULL;
	char msgBuf[1024];
	HMODULE hInstLib = LoadLibraryA("PSAPI.DLL");
	if(hInstLib) lpfGetModuleFileName =(DWORD (WINAPI *)(HANDLE, HMODULE, LPTSTR, DWORD )) GetProcAddress(hInstLib, "GetModuleFileNameExA");
	if(!lpfGetModuleFileName) return; // skip 
	processInfo.dwSize = sizeof(processInfo);
	HANDLE processesSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
	char *processName = tm->path;
	char *p;
	while(p = strchr(processName, '\\')) processName=p+1;
	if (processesSnapshot != INVALID_HANDLE_VALUE){
		/* Find out if example.exe is still running: */
		BOOL bFound = FALSE;
		Process32First(processesSnapshot, &processInfo);
		OutTrace("compare first: %s - %s\n", processName, processInfo.szExeFile);
		if(_stricmp(processInfo.szExeFile, processName)==0){
			bFound = TRUE;
		}
		if(!bFound) while (Process32Next(processesSnapshot, &processInfo)){
			OutTrace("compare next: %s - %s\n", processName, processInfo.szExeFile);
			if(_stricmp(processInfo.szExeFile, processName)==0){
				bFound = TRUE;
				break;
			}
		...
		if(bFound){
			// now repeat the check against the full pathname!
			HANDLE processHandle = NULL;
			TCHAR filename[MAX_PATH];
			processHandle = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, processInfo.th32ProcessID);
			if (processHandle != NULL) {
				if ((*lpfGetModuleFileName)(processHandle, NULL, filename, MAX_PATH)) {
					if(_stricmp(filename, tm->path)==0){
						sprintf(msgBuf, "Active target \"%s\" found.\nDisabled.", pm->title);
						MessageBox(0, msgBuf, "DxWnd", 0);
						tm->flags3 &= ~HOOKENABLED;
						tm->flags15 |= HOOKRECOVER;
					}
				}
...
void CDxwndhostView::OnInitialUpdate()
{
	...
	StatusMap.FogFactor = 1.0f; // initialize to 1.0, maybe to save/restore from dxwnd.ini???
	StatusMap.LightFactor = 1.0f; // initialize to 1.0, maybe to save/restore from dxwnd.ini???
	OutTrace("joystick initial deadzone = %d%%\n", StatusMap.VJoyDeadZone);

	// initialize defined entries
	for(i = 0; i < MAXTARGETS; i ++){
		TARGETMAP *t = &TargetMaps[i];
		PRIVATEMAP *p = &PrivateMaps[i];
		if (!LoadConfigItem(t, p, i, gInitPath)) break;
		// use the special flag HOOKRECOVER to recover the HOOKENABLED state of a program temporarily disabled
		if (t->flags15 & HOOKRECOVER) t->flags3 |= HOOKENABLED; 
		// temporarily disable programs that are found as running when DxWnd starts
		if(gCheckRunning && (t->flags3 & HOOKENABLED)) CheckRunningTask(t, p);
		AppendItemToList(t, p, i, &listitem);
		listctrl.InsertItem(&listitem);
	}
	...
	// detect if Taskbar Window is visible
	gIsToolbarWndVisible = IsToolbarWndVisible();
	OutTrace("Toolbar window is %s\n", gIsToolbarWndVisible ? "visible" : "hidden");
	if(gIsToolbarWndVisible) ::CreateThread(NULL, 0, ToolbarMon, (LPVOID)this, 0, NULL);
}
...
void CDxwndhostView::OnExport()
{
	...
	// clear invalid characters for file pathnames
	strcpy(titlePath, PrivateMaps[i].title);
	size_t badCharIndex;
	while((badCharIndex = strcspn(titlePath, "<>:\"/\\|?*")) < strlen(titlePath)) strcpy(&titlePath[badCharIndex], &titlePath[badCharIndex + 1]);
	strcat_s(path, MAX_PATH, titlePath);
	...
	if( dlg.DoModal() == IDOK) {
		DWORD TFlags, TFlags2, DFlags, DFlags2;
		TARGETMAP *TargetMap;
		...
		// v2.05.92: clear debug/trace flags conditionally
		debugFlags = GetPrivateProfileInt("window", "debugflags", 0, gInitPath);
		if(!debugFlags){
			TFlags = TargetMap->tflags;
			TFlags2 = TargetMap->tflags2;
			DFlags = TargetMap->dflags;
			DFlags2 = TargetMap->dflags2;
			TargetMap->tflags = 0;
			TargetMap->tflags2 = 0;
			TargetMap->dflags = 0;
			TargetMap->dflags2 = 0;
		...
		relPath = GetPrivateProfileInt("window", "relpath", 1, gInitPath);
		if(TargetMap->path[0] == '?') relPath = FALSE;
		if(TargetMap->path[0] == '!') relPath = FALSE;
		if(relPath){
			char oldpath[MAX_PATH];
			char newpath[MAX_PATH];
			strcpy(oldpath, TargetMap->path);
			char *p = oldpath;
			char *lastSep = oldpath;
			while(p = strchr(p, '\\')) {
				OutTrace("%s\n", p);
				lastSep = p;
				p++;
			...
			strcpy(newpath, "?\\");
				OutTrace("newpath=%s\n", newpath);
			strcat(newpath, lastSep+1);
				OutTrace("newpath=%s\n", newpath);
			strcpy(TargetMap->path, newpath);
			SaveConfigItem(&TargetMaps[i], &PrivateMaps[i], 0, path);
			strcpy(TargetMap->path, oldpath);
		}
		else {
			SaveConfigItem(&TargetMaps[i], &PrivateMaps[i], 0, path);
		}
		...
		if(!debugFlags){
			...
			TargetMap->tflags2 = TFlags2;
			TargetMap->dflags = DFlags;
			TargetMap->dflags2 = DFlags2;
		}
...
BOOL CDxwndhostView::OnImport(CString sFilePath)
{
	...
	if(LoadConfigItem(&TargetMaps[i], &PrivateMaps[i], 0, (char *)sFilePath.GetString())){
		...
		AppendItemToList(&TargetMaps[i], &PrivateMaps[i], i, &listitem);
		listctrl.InsertItem(&listitem);
		OutTrace("Import file=%s index=%d\n", sFilePath, i);
	}
	Resize();
...
void CDxwndhostView::OnModify() 
{
	...
	OutTrace("CDxwndhostView::OnModify\n");
	if(!listctrl.GetSelectedCount()) return;
	pos = listctrl.GetFirstSelectedItemPosition();
	i = listctrl.GetNextSelectedItem(pos);
	// make the tab activated only when necessary ...
	gbWineConfig = (TargetMaps[i].flags19 & ISWIN16EXECUTABLE) ? TRUE : FALSE;
	SetDlgFromPrivateMap(&PrivateMaps[i], &dlg);
	SetDlgFromTarget(&TargetMaps[i], &dlg);
	// v2.04.74: item numeration
	dlg.m_ItemIndex = i+1;
	// update the CD drives list
	InitializeCDDrives();
	if(dlg.DoModal() == IDOK && dlg.m_FilePath.GetLength()){
		CHAR prefix = dlg.m_FilePath[0];
		if(((prefix == '!') || (prefix == '*')) && (dlg.m_FilePath.GetLength() == 1)) return;
...
void CDxwndhostView::OnDisable() 
{
	...
	// toggle
	if(t->flags3 & HOOKENABLED) 
		t->flags3 &= ~HOOKENABLED;
	else
		t->flags3 |= HOOKENABLED;

	//TargetMaps[i].m_HookEnabled = ~TargetMaps[i].m_HookEnabled;
	p->updated = TRUE;

	// v2.05.05: autosave 
	if(gAutoSave){
		SaveConfigItem(t, p, i, gInitPath);
	}
	else {
		this->isUpdated=TRUE;
	}
...
void CDxwndhostView::OnViewLog() 
{
	...
	pos = listctrl.GetFirstSelectedItemPosition();
	i = listctrl.GetNextSelectedItem(pos);
	if(TargetMaps[i].tflags & OUTONTEMPFOLDER){
		char path[MAX_PATH];
		GetTempPathA(MAX_PATH, path);
		FileFolder = (CString)path;
	}
	else {
		if(PrivateMaps[i].startfolder[0]){
			FileFolder = PrivateMaps[i].startfolder;
		}
		else {
			FileFolder = TargetMaps[i].path;
			len=FileFolder.ReverseFind('\\');	
			if (len==0) return;
			FileFolder.Truncate(len);
		}
	}
	//MessageBox(FileFolder, "FileFolder", 0);
	FilePattern = FileFolder;
	FilePattern.Append("\\dxwnd*.log");
	FilePath = FileFolder;
	FilePath.Append("\\dxwnd.log");

	// find first file
	WIN32_FIND_DATA FindFileData;
	HANDLE ffh = FindFirstFileA(FilePattern.GetBuffer(), &FindFileData);
	if(ffh) {
		// at least one log file is present
	}
	else {
		MessageBox("Can't find DxWnd log files", "DxWnd", 0);
		return;
	}

	ret = FindNextFileA(ffh, &FindFileData);
	if(ret) {
		//MessageBox((CString)FindFileData.cFileName, "next path", 0);
		// 2 log files or more ...
		FindClose(ffh);
		// let the dialog box do the job!
		const TCHAR szFilter[] = _T("log files (*.log)|*.log|");
		CFileDialog dlg(TRUE, _T("log"), FilePath, OFN_READONLY, szFilter, 0, 0); 
		dlg.m_ofn.lpstrTitle = "Open DxWnd log file";
		dlg.m_ofn.lpstrInitialDir = FileFolder;
		if(dlg.DoModal() == IDOK){
...
void CDxwndhostView::OnBossKey() 
{
	DXWNDSTATUS DxWndStatus;
	BossKeyToggle = !BossKeyToggle;
	if(BossKeyToggle){
...
void CDxwndhostView::OnWindowHighlight() 
{
	DXWNDSTATUS DxWndStatus;
	HWND TargethWnd;
	if (GetHookStatus(&DxWndStatus) == DXW_RUNNING){
	...
	fwi.dwTimeout = 0; // flash period in mSec, 0 = default caret flash period
	fwi.hwnd = TargethWnd;
	fwi.uCount = 3;
	::FlashWindowEx(&fwi);
}
...
void CDxwndhostView::OnProcessKill(BOOL bAll)
{
	...
	if(tm->flags19 & ISWIN16EXECUTABLE){
		lpProcName = "otvdmw.exe";
	}
	else {
		strnncpy(FilePath, tm->path, MAX_PATH);
		lpProcName=FilePath;
		while (lpNext=strchr(lpProcName,'\\')) lpProcName=lpNext+1;
	}
	OutTrace("killing %s%s\n" , bAll ? "(ALL) " : "", lpProcName);
...
void CDxwndhostView::OnAdd(char *sInitialPath, int profile) 
{
	...
	if(profile == -1){
		...
		// ghogho
		TARGETMAP *tmCopy = &TargetMaps[profile];
		PRIVATEMAP *pmCopy = &PrivateMaps[profile];
		memcpy(tm, tmCopy, sizeof(TARGETMAP));
		memcpy(pm, pmCopy, sizeof(PRIVATEMAP));
		SetDlgFromTarget(tm, dlg);
	}

	if(sInitialPath) dlg->m_FilePath = CString(sInitialPath);
	else {
		if(gStripRoot) dlg->m_FilePath = gProgramRootFolder;
	}

	while(dlg->DoModal() == IDOK) { 
		// v2.06.09: warn & repeat
		if(dlg->m_FilePath.GetLength() == 0){
			MessageBox("bad null target path", "error", 0);
			continue;
		}
		if((dlg->m_FilePath.GetLength() == 1)) {
			CHAR prefix = dlg->m_FilePath[0];
			if((prefix == '*') || (prefix == '?')) {
				MessageBox("bad target path", "error", 0);
				return; 
			}
		}
...
void CDxwndhostView::OnCopyPath() 
{
	int i;
	POSITION pos;
	CListCtrl& listctrl = GetListCtrl();
	...
	//MessageBox(FilePath, "Copy Path", 0);

	if (::OpenClipboard(0)) {
        EmptyClipboard();
        int size_m = sizeof(char) * (strlen(FilePath) + 1);
        HGLOBAL hClipboardData = GlobalAlloc(GMEM_DDESHARE, size_m);
        char* pchData;
        pchData = (char*)GlobalLock(hClipboardData);
        //wcscpy(pchData, strData);
        strcpy_s(pchData, size_m, FilePath);
...
void CDxwndhostView::OnPastePath() 
{
	int i;
	POSITION pos;
	CListCtrl& listctrl = GetListCtrl();
	...
	if (!pszText) return;
	// Save text in a string 
	strncpy(NewPath, pszText, MAX_PATH);
	// Release the lock
	GlobalUnlock(hData);
	...
	CString msg = 
		"Do you want to replace the current path \n\"" + 
		CString(FilePath) + 
		"\"\nwith the new path\n\"" + 
		NewPath +
		"\"?";
	if(MessageBox(msg, "DxWnd", MB_YESNO) != IDYES) return;

	strncpy(TargetMaps[i].path, NewPath, MAX_PATH);
	PrivateMaps[i].updated = TRUE;
	...
	if(gAutoSave){
		...
		// only the main section is updated, no registry or fake drives
		this->isUpdated=TRUE;
	}
...
void CDxwndhostView::ClearLogsInFolder(CString folder)
{
	...
	for(int i=1; i<100; i++){
		filePath = folder;
		filePath.AppendFormat("\\dxwnd(%d).log", i);
		int ret = _unlink(filePath);
		if(ret == ENOENT) break;
		if(ret == EACCES){
			MessageBox("Can't delete file " + filePath, "DxWnd", 0);
			break;
		}
	}
}
...
{	
	...
	while (TRUE) {
		...
		if(DxStatus==DXW_RUNNING){
			...
			sprintf_s(sMsg, 1024, "DxWnd %s\nHook status: %s", GetDllVersion(), Status);
...
void CDxwndhostView::OnIdentifyMonitors()
{
...
#define NUMBER_HEIGHT 30
	...
	char label[20];
	HFONT hfont = CreateFontA(200, 120, 0, 0, 10, 0, 0, 0, 0, 0, 0, 0, 0, "Arial");
	int monitorNumber = 1;
	for(int iDisplayDevice=0; ;iDisplayDevice++){
		...
		OutTrace("monitor %d found\n", iDisplayDevice);

		memset(&devMode, 0, sizeof(DEVMODEA));
		devMode.dmSize = sizeof(DEVMODEA);
		EnumDisplaySettings(DisplayDevice.DeviceName, ENUM_CURRENT_SETTINGS, &devMode);

		OutTrace("position=(%d, %d) size=(%d x %d)\n", devMode.dmPosition.x, devMode.dmPosition.y, devMode.dmPelsWidth, devMode.dmPelsHeight);
		if((devMode.dmPelsWidth > 0) && (devMode.dmPelsHeight > 0)){
			SIZE size;
			hfont = (HFONT)SelectObject(dc, hfont);
			//sprintf(label,"%d", monitorNumber++);
			sprintf(label,"%d", iDisplayDevice+1);
			GetTextExtentPoint32A(dc, label, strlen(label), &size);
			textPos.x = devMode.dmPosition.x + ((devMode.dmPelsWidth - size.cx) / 2);
			textPos.y = devMode.dmPosition.y + ((devMode.dmPelsHeight - size.cy) / 2);
			TextOutA(dc, textPos.x, textPos.y, label, strlen(label));
			hfont = (HFONT)SelectObject(dc, hfont);
		}
	}
...
void CDxwndhostView::OnGlobalSettings() 
{
	...
	//pDlg->ShowWindow(SW_SHOW);
	BOOL prevStripRoot = gStripRoot;
	CGlobalSettings *pDlg = new(CGlobalSettings);
	pDlg->DoModal();
	// if StripRoot mode has changed, set everything as updated
	if (prevStripRoot != gStripRoot) {
		this->isUpdated=TRUE;
		for(int i = 0; i < MAXTARGETS; i ++){
...
void CDxwndhostView::OnView3DEffects()
{
	C3DEffectsDialog *pDlg = new C3DEffectsDialog();
	BOOL ret = pDlg->Create(C3DEffectsDialog::IDD, this); 
	pDlg->ShowWindow(SW_SHOW);
}
...
void CDxwndhostView::OnRButtonDown(UINT nFlags, CPoint point) 
{
	...
	if((iItem == -1) || (nHitFlags & LVHT_NOWHERE)){
		...
		popup.CheckMenuItem(ID_PENABLED, 
			(TargetMaps[iItem].flags3 & HOOKENABLED) ? MF_CHECKED : MF_UNCHECKED);
	...
	case ID_PDUPLICATE:
		OnDuplicate();
		break;
	case ID_EDITLIST_COPYPATH:
		OnCopyPath();
		break;
	case ID_EDITLIST_PASTEPATH:
		OnPastePath();
		break;
	case ID_PEXPLORE:
		OnExplore();
	...
	case ID_TOOLS_RECOVERGAMMARAMP:
		OnRecoverGammaRamp();
		break;
	case ID_RECOVER_SYSTEMCURSORS:
		OnRecoverSystemCursors();
		break;
	case ID_MOVE_TOP:
		OnMoveTop();
		break;
...
DWORD RecoverDisplaySettings(LPVOID arg)
{
	OutTrace("RecoverDisplaySettings: START\n");
	Sleep(2000);
	while(TRUE){
		Sleep(500);
		//OutTrace("RecoverDisplaySettings: status=%d\n", GetHookInfo()->Status);
		if(GetHookStatus(NULL) != DXW_RUNNING){
			OutTrace("RecoverDisplaySettings: RECOVER\n");
			LONG res = ChangeDisplaySettingsExA(NULL, NULL, NULL, 0, 0);
			if(res) OutTrace("ChangeDisplaySettingsExA: ERROR res=%d @%d\n", res, __LINE__);
			break;
		}
	}
	return 0;
...
void ClearShimsExt(char *path, HKEY root, char *rootName, char *key)
{
	HKEY hk;
	char sMessage[500];
	DWORD lKeyLength = 0L;
	...
	ret=RegOpenKeyEx(root, key, 0, KEY_ALL_ACCESS | KEY_WOW64_64KEY, &hk);
	OutTrace("searching in \"%s\\%s\" ret=%d\n", rootName, key, ret);
	if(ret){
		if(ret != ERROR_FILE_NOT_FOUND){
			if(ret != ERROR_ACCESS_DENIED){
				sprintf(sMessage, "error %ld in RegOpenKeyEx %s\\%s", ret, rootName, key);
				OutTrace("%s\n", sMessage);
				MessageBox(NULL, sMessage, "DxWnd", MB_OK);
			}
			else {
				sprintf(sMessage, "access denied in RegOpenKeyEx %s\\%s\n"
					"Try running DxWnd with administrator's rights", rootName, key);
				OutTrace("%s\n", sMessage);
				MessageBox(NULL, sMessage, "DxWnd", MB_OK);
			}
		}
		...
		ret=RegQueryValueEx(hk, path, NULL, &lType, NULL, &lKeyLength);
		OutTrace("searching in \"%s\\%s\" for \"%s\" ret=%d\n", rootName, key, path, ret);
		if(ret && (ret != ERROR_FILE_NOT_FOUND)){
			if(ret != ERROR_ACCESS_DENIED){ 
				sprintf(sMessage, "error %ld in RegQueryValue hk=%lx \"%s\"", ret, hk, path);
				OutTrace("%s\n", sMessage);
				MessageBox(NULL, sMessage, "DxWnd", MB_OK);
			}
			else {
				sprintf(sMessage, "access denied in RegQueryValue hk=%lx \"%s\"\n"
					"Try running DxWnd with administrator's rights", hk, path);
				OutTrace("%s\n", sMessage);
				MessageBox(NULL, sMessage, "DxWnd", MB_OK);
			}
		}
		if(lKeyLength > 0L) {
			sprintf(sMessage, "Found compatibility flag in %s\\%s", rootName, key);
			OutTrace("%s\n", sMessage);
			MessageBox(NULL, sMessage, "DxWnd", MB_OK);
			ret = RegDeleteValue(hk, path);
			OutTrace("RegDeleteValue(%#x, \"%s\") ret=%d\n", hk, path, ret);
			if(ret) {
				if(ret != ERROR_ACCESS_DENIED){ 
					sprintf(sMessage, "error %ld in RegDeleteValue hk=%lx \"%s\"", ret, hk, path);
					OutTrace("%s\n", sMessage);
					MessageBox(NULL, sMessage, "DxWnd", MB_OK);
				}
				else {
					sprintf(sMessage, "access denied in RegDeleteValue hk=%lx \"%s\"\n" 
						"Try running DxWnd with administrator's rights", hk, path);
					OutTrace("%s\n", sMessage);
					MessageBox(NULL, sMessage, "DxWnd", MB_OK);
				}
...
void ClearShims(char *path)
{
	OutTrace("clearshims path=\"%s\"\n", path);
	ClearShimsExt(path, HKEY_CURRENT_USER, "HKEY_CURRENT_USER", "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\AppCompatFlags\\Layers");
	ClearShimsExt(path, HKEY_LOCAL_MACHINE, "HKEY_LOCAL_MACHINE", "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\AppCompatFlags\\Layers");
	ClearShimsExt(path, HKEY_CURRENT_USER, "HKEY_CURRENT_USER", "Software\\Microsoft\\Windows NT\\CurrentVersion\\AppCompatFlags\\Compatibility Assistant\\Store");
	ClearShimsExt(path, HKEY_LOCAL_MACHINE, "HKEY_LOCAL_MACHINE", "Software\\Microsoft\\Windows NT\\CurrentVersion\\AppCompatFlags\\Compatibility Assistant\\Store");
}

void RunTarget(int runmode, int i, TARGETMAP *tm, PRIVATEMAP *pm, TARGETMAP *TargetMaps)
{
	STARTUPINFO sinfo;
	...
	HookMode_Type HookMode;

	if (tm->flags2 & STARTDEBUG) HookMode = HOOK_DEBUG;
	else if (tm->flags7 & INJECTSUSPENDED) HookMode = HOOK_SUSPENDED;
	else if (tm->flags12 & INJECTPROXY) HookMode = HOOK_PROXY;
	else if (tm->flags15 & INJECTAPC) HookMode = HOOK_INJECTAPC;
	else if (tm->flags19 & STARTDETOURS) HookMode = HOOK_DETOURS;
	//else if (tm->flags15 & INJECTAPC2) HookMode = HOOK_INJECTAPC2;
	else HookMode = HOOK_WINDOW;
	...
	// v2.05.77 - make path and launchpath independent ...
	//sRunTargetPath = (strlen(pm->launchpath)>0) ? pm->launchpath : tm->path;
	sRunTargetPath = tm->path;

	// can't use restricted map when hooking multiple father-son processes ...
	// v2.05.69: unless you use extended hook!
	// t.b.d. should this be decided by a flag?
	bRestrictedMap = !(tm->flags5 & (INJECTSON|DEBUGSON));
	if(tm->flags13 & SHAREDHOOK) bRestrictedMap = TRUE;
	...
	if(tm->flags19 & ISWIN16EXECUTABLE){
		...
		TARGETMAP *t = &RestrictedMaps[0];
		char newTarget[MAX_PATH+1];
		_snprintf(newTarget, MAX_PATH, "%swinevdm\\otvdmw.exe", GetDxWndPath());
		OutTrace("newTarget=%s\n", newTarget);
		strcpy(t->path, newTarget);
		sRunTargetPath = t->path;
		_snprintf(newTarget, MAX_PATH, "otvdmw.exe \"%s\"", tm->path);
		OutTrace("newArgs=%s\n", newTarget);
		strcpy(pm->cmdline, newTarget);
		strcpy_s(folderpath, sizeof(folderpath), tm->path);
		strcpy_s(exepath, sizeof(exepath), tm->path);
		PathRemoveFileSpec(folderpath);	
		OutTrace("folderpath=%s\n", folderpath);
		strcpy_s(exepath, sizeof(exepath), sRunTargetPath);
		OutTrace("exepath=%s\n", exepath);
		//------- otvdmw settings
		char otvdmIniPath[MAX_PATH];
		_snprintf(otvdmIniPath, MAX_PATH, "%swinevdm\\otvdm.ini", GetDxWndPath());
		WritePrivateProfileString("otvdm", "DIBPalette", (tm->flags19 & DIBPALETTE) ? "1" : "0", otvdmIniPath);
	}
	else {
		if(!(pm->startfolder[0])){
			// empty (default) running folder
			strcpy_s(folderpath, sizeof(folderpath), sRunTargetPath);
			strcpy_s(exepath, sizeof(exepath), sRunTargetPath);
			PathRemoveFileSpec(folderpath);
		}else{
			// non empty running folder
			strcpy_s(folderpath, sizeof(folderpath), pm->startfolder);
			strcpy_s(exepath, sizeof(exepath), sRunTargetPath);
			if((tm->flags10 & FAKECDDRIVE) && (strlen(folderpath) == 2)){
				if ((toupper(tm->FakeCDDrive) == toupper(folderpath[0])) &&
					(folderpath[1] == ':')){
					strcpy_s(folderpath, sizeof(folderpath), pm->FakeCDPath);
					if(folderpath[0]=='?'){
						char sCurrentPath[MAX_PATH+1];
						char sTail[MAX_PATH+1];
						strcpy(sTail, &folderpath[1]);
						strcpy_s(sCurrentPath, sizeof(sCurrentPath), sRunTargetPath);
						PathRemoveFileSpec(sCurrentPath);
						sprintf_s(folderpath, MAX_PATH, "%s\\%s", sCurrentPath, sTail);
					}
					if(GetHookInfo()->CDIndex > 0) {
						char tail[6];
						sprintf_s(tail, 6, "%02d", GetHookInfo()->CDIndex + 1);
						strcat_s(folderpath, sizeof(folderpath), tail);
					}
					OutTrace("expanding fake CD %c: index=%d to path=\"%s\"\n", tm->FakeCDDrive, GetHookInfo()->CDIndex, folderpath);
				}
			}
		}
	}

	if(strlen(pm->launchpath)>0){
		OutTrace("running launcher=%s\n", pm->launchpath);
		STARTUPINFO sinfo;
		PROCESS_INFORMATION pinfo;
		ZeroMemory(&pinfo, sizeof(pinfo));
		ZeroMemory(&sinfo, sizeof(sinfo));
		sinfo.cb = sizeof(sinfo);
		CreateProcess(NULL, pm->launchpath, 
		...
		// beware: in HOOK_WINDOW and HOOK_PROXY modes the task is supposed to be run by the launcher
		if(HookMode == HOOK_WINDOW) return;
	}
	...
	// apply shims from flags or shims string
	if(strlen(pm->msshims)>0){
		char env[MAX_PATH+40];
		if(tm->flags & SETDPIAWARE){
			sprintf(env, "__COMPAT_LAYER=HIGHDPIAWARE %s",  pm->msshims);
		}
		else {
			sprintf(env, "__COMPAT_LAYER=%s", pm->msshims);
		}
		_putenv(env);
	}
	else {
	...
	// for wine logs
	BOOL WineDebug = GetPrivateProfileInt("window", "winedebug", 0, gInitPath);
	if(tm->flags19 & ISWIN16EXECUTABLE) {
		OutTrace("gInitPath=%s\n", gInitPath);
		OutTrace("WineVDM DEBUG %s\n", WineDebug ? "ON" : "OFF");
		if(WineDebug) {
			_putenv("WINEDEBUG=+relay,+int21,+msg");
		}
		else {
			_putenv("WINEDEBUG=");
		}
	...
	if(tm->dflags2 & DISABLEWINHOOK){
		MessageBox(0, "DEBUG: disabling window hook", "DxWnd", 0);
		EndHook();
	}

	if(tm->flags19 & SETRESOLUTION){
		DEVMODEA dm;
		LONG res;
		if(!EnumDisplaySettingsA(NULL, ENUM_CURRENT_SETTINGS, &dm))
			OutTrace("EnumDisplaySettingsA: ERROR err=%d @%d\n", GetLastError(), __LINE__);
		else {
			dm.dmFields = DM_PELSWIDTH | DM_PELSHEIGHT;
			dm.dmPelsWidth = tm->resw;
			dm.dmPelsHeight = tm->resh;
			res = ChangeDisplaySettingsExA(NULL, &dm, NULL, CDS_FULLSCREEN, NULL);
			if(res) 
				OutTrace("ChangeDisplaySettingsExA: ERROR res=%d @%d\n", res, __LINE__);
		}
	}

	// v2.04.04: fix - STARTDEBUG and INJECTSUSPENDED and STARTDETOURS must take place only when HOOKENABLED
	if ((HookMode == HOOK_DEBUG) && 
		(tm->flags3 & HOOKENABLED) &&
		bHookEnabled){
		// Debugger mode
		OutTrace("debugger mode\n");
		if(runmode==RUN_SUSPENDED){
			MessageBox(NULL, "Suspended mode not available for debug hook", "Warning", MB_ICONWARNING|MB_OK);
		}
		else{
	...
	if ((HookMode == HOOK_DETOURS) &&
		(tm->flags3 & HOOKENABLED) &&
		bHookEnabled){
		// Inject mode
		OutTrace("injectDetours mode=%d(%s)\n", 
			runmode, (runmode==RUN_SUSPENDED)?"SUSPENDED":((runmode==RUN_UNHOOKED)?"UNHOOKED":"DEFAULT"));
		InjectDetours(exepath, folderpath, (runmode == RUN_SUSPENDED), bMustCommit, tm, pm); 
	}
	else
	if ((HookMode == HOOK_PROXY) && 
		(tm->flags3 & HOOKENABLED) &&
		bHookEnabled){
		// Proxy mode
		...
		if(runmode==RUN_SUSPENDED){
			MessageBox(NULL, "Suspended mode not available for proxy hook", "Warning", MB_ICONWARNING|MB_OK);
		}
		else{
			ThreadInfo_Type *pThreadInfo;
			pThreadInfo = (ThreadInfo_Type *)malloc(sizeof(ThreadInfo_Type));
	...
	if ((HookMode == HOOK_INJECTAPC) &&
		(tm->flags3 & HOOKENABLED) &&
		bHookEnabled){
		// Inject mode
		OutTrace("injectAPC mode=%d(%s)\n", 
			runmode, (runmode==RUN_SUSPENDED)?"SUSPENDED":((runmode==RUN_UNHOOKED)?"UNHOOKED":"DEFAULT"));
		HMODULE injlib = LoadLibraryA("injectAPC.dll");
		if(!injlib) MessageBox(NULL, "InjectAPC is not available", "DxWnd", 0);
		else {
			typedef void (*InjectAPC_Type)(char *, char *, BOOL, BOOL, char *);
			typedef void (*OutTrace_Type)(const char *, ...);
			typedef void (*InjectSetLogger_Type)(OutTrace_Type);
			InjectAPC_Type pInjectAPC = (InjectAPC_Type)GetProcAddress(injlib, "InjectAPC");
			InjectSetLogger_Type pInjectSetLogger = (InjectSetLogger_Type)GetProcAddress(injlib, "InjectSetLogger");
			if(pInjectSetLogger) (*pInjectSetLogger)(OutTrace);
			(*pInjectAPC)(exepath, folderpath, (runmode == RUN_SUSPENDED), bMustCommit, pm->cmdline); 
			FreeLibrary(injlib);
		}
	}
#ifdef ADDINJECTACP
	else
	if ((HookMode == HOOK_INJECTAPC2) &&
		(tm->flags3 & HOOKENABLED) &&
		bHookEnabled){
		// Inject mode
		OutTrace("injectAPC2 mode=%d(%s)\n", 
			runmode, (runmode==RUN_SUSPENDED)?"SUSPENDED":((runmode==RUN_UNHOOKED)?"UNHOOKED":"DEFAULT"));
		InjectAPC2(exepath, folderpath, (runmode == RUN_SUSPENDED), bMustCommit, tm, pm); 
	}
#endif // ADDINJECTACP
	else if (HookMode == HOOK_WINDOW) {
		// windowshook mode
		OutTrace("setwindowshook mode\n");
		if(runmode==RUN_SUSPENDED){
			MessageBox(NULL, "Suspended mode useless for windows hook", "Warning", MB_ICONWARNING|MB_OK);
		}
		else{
			ThreadInfo_Type *pThreadInfo;
			pThreadInfo = (ThreadInfo_Type *)malloc(sizeof(ThreadInfo_Type));
	...
	if(bPanicMode){
		...
		int choice = MessageBox(NULL, sMessage, "DxWnd AUTOLOG MODE", MB_YESNO | MB_ICONEXCLAMATION);
		if(choice == IDYES) {
			ShellExecute(NULL, "open", FilePath, NULL, NULL, SW_SHOW);
		}
	}
	...
	if(tm->dflags2 & DISABLEWINHOOK){
		MessageBox(0, "DEBUG: enable window hook", "DxWnd", 0);
		StartHook();
	}

	// wait & recover
	if(bRestrictedMap) CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)RecoverTargetMaps, (LPVOID)TargetMaps, 0, NULL);
	if(tm->flags19 & SETRESOLUTION) CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)RecoverDisplaySettings, (LPVOID)NULL, 0, NULL);
}
...
void CDxwndhostView::OnRun(int runmode, int i) 
{
	...
	if((len > 2) && (tm->path[0] == '?')) {
		...
		if((len>2) && (tm->path[1] == '\\')) strcpy(target, &tm->path[2]);
		else strcpy(target, &tm->path[1]);
		sprintf_s(message, 1024, 
			"Incomplete entry\n"
			"Select the file to match the \"%s\" target.",
			target);
		int choice = MessageBox(message, "DxWnd", MB_OKCANCEL | MB_ICONEXCLAMATION);
		if (choice == IDCANCEL) return;
		// update the Path field ...

		char path[MAX_PATH];
		CFileDialog dlg( TRUE, "*.*", path, 0, "Program (*.exe)|*.exe|All files (*.*)|*.*||",  this);
		char rootFolder[MAX_PATH];
		GetPrivateProfileString("window", "root", "C:\\Games", rootFolder, MAX_PATH, gInitPath);
		dlg.m_ofn.lpstrInitialDir = rootFolder;
		sprintf_s(message, 1024, "Select the \"%s\" target.", target);
		dlg.m_ofn.lpstrTitle = message;
		dlg.m_ofn.lpstrFile = target;
		int ret = dlg.DoModal();
		if(ret==IDOK){ 
			strncpy(path, dlg.GetPathName(), MAX_PATH);  
			strncpy(tm->path, path, MAX_PATH); 
			if(gAutoSave) {
				SaveConfigItem(&TargetMaps[i], &PrivateMaps[i], i, gInitPath);
			}
			else {
	...
	RunTarget(runmode, i, tm, pm, TargetMaps);
...
void CDxwndhostView::OnRecoverSystemCursors()
{
	//MessageBox("RecoverSystemCursors", "DxWnd", 0);
	RestoreMouseSpeed(iInitialMouseSpeed);
	RestoreSystemCursors();
	SaveSystemCursors();
}
...
void CDxwndhostView::OnImportFromLog()
{
	char ConfigPath[MAX_PATH+1] = {0};
	int i, res;
	POSITION pos;
	...
	char sMessage[256];
	sprintf(sMessage, "Import configuration flags for \"%s\"?", PrivateMaps[i].title);
	res=MessageBox(sMessage, "DxWnd", MB_YESNO | MB_ICONQUESTION);
	if(res!=IDYES) return;

	CFileDialog FileDlg( TRUE, "*.log", ConfigPath, OFN_OVERWRITEPROMPT, NULL, this);
	FileDlg.m_ofn.lpstrFilter = "DxWnd log (*.log)\0*.log\0All files (*.*)\0*.*\0\0";
	FileDlg.m_ofn.lpstrTitle = "Open DxWnd log file";

	if(FileDlg.DoModal() != IDOK) return;
	if(LoadLogFile(t, p, 0, FileDlg.GetPathName().GetBuffer())) {
		MessageBox("Bad syntax, operation aborted", "DxWnd", MB_ICONEXCLAMATION);
		return;
	}
...
void CDxwndhostView::OnDropFiles(HDROP dropInfo)
{
	...
	if(nFilesDropped > 0){
		...
		if(!_strnicmp(p, ".dxw", 4)){
			...
			RECT rectCheck;
			CListCtrl& listctrl = GetListCtrl();
			DragQueryPoint(dropInfo, &point);
			for(item=0; item<listctrl.GetItemCount(); item++){
				listctrl.GetItemRect(item , &rectCheck, LVIR_BOUNDS);
				rectCheck.right+=1;
				if(PtInRect(&rectCheck, point)) break;
			};
			//sprintf(msg, "ItemFromPoint returns %d", item);
			//MessageBox(msg, "test", 0);
			if(item < listctrl.GetItemCount()) {
				sprintf(msg, "Adding item with \"%s\" profile",
					listctrl.GetItemText(item, 0));
				MessageBox(msg, "DxWnd", 0);
				OutTrace("%s\n", msg);
				this->OnAdd(sFile.GetBuffer(), item);
			}
			else {
				strcpy(msg, "Adding item with default profile");
				MessageBox(msg, "DxWnd", 0);
				OutTrace("%s\n", msg);
				this->OnAdd(sFile.GetBuffer(), -1);
			}
		}
		sFile.ReleaseBuffer();
	}
...
void CDxwndhostView::OnProxyInstall(char *dllfile) 
{
	...
	if ((TargetMaps[i].flags & HANDLEEXCEPTIONS) ||
		...
		(TargetMaps[i].flags17 & FAKEPENTIUM3) ||
