...
BOOL gMustDie = FALSE;
BOOL gStripRoot = FALSE;
BOOL gCheckRunning = FALSE;
BOOL gIsToolbarWndVisible = FALSE;
int iProgIndex;
DWORD SysColors[32];
char gInitialWorkingDir[MAX_PATH+1];
char gProgramRootFolder[MAX_PATH+1];
WORD GammaRamp[256*3];
char gKeyboardLayoutName[KL_NAMELENGTH];
HKL  ghkl;
...
BOOL CDxwndhostApp::InitInstance()
{
	...
	gStripRoot = GetPrivateProfileInt("window", "striproot", 0, gInitPath); // automatically strips the common root folder from every path, replacing it with ':'
	gCheckRunning = GetPrivateProfileInt("window", "checkrunning", 0, gInitPath); // at start, checks active programs and disables hook for those already running
	GetPrivateProfileString("window", "root", "C:\\Games", gProgramRootFolder, MAX_PATH, gInitPath);
	...
	gKeyboardLayoutName[0]=0;
	GetKeyboardLayoutNameA(gKeyboardLayoutName);
	SystemParametersInfoA(SPI_GETDEFAULTINPUTLANG, 0, (PVOID)&ghkl, NULL);
	if(ghkl == NULL) OutTrace("SystemParametersInfoA(SPI_GETDEFAULTINPUTLANG) failed err=%d\n", GetLastError());
	OutTrace("initial keyboard layout=%s hkl=%x\n", gKeyboardLayoutName, ghkl);
