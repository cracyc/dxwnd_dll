...
void CheckSafeDiscVersion(char *path)
{
	...
	while(TRUE) {
		...
		if(*(DWORD *)(opcode+16)==0x00003E79){
			...
			MessageBox(NULL, sMessage, "DxWnd hint", MB_OK|MB_ICONWARNING|MB_TOPMOST);
			break;
		}
...
void CheckSafeDiscVersion(char *path)
{
	...
	DWORD dwVersion, dwSubversion, dwRevision;
	FILE *exe;
	unsigned char fbuffer[128*1024]; // 128 KBytes
	int len;

	exe=fopen(path, "rb");
	if(!exe) return;
	len = fread(fbuffer, 1, sizeof(fbuffer), exe);
	fclose(exe);
	if(len < SAFEDISKSEQLEN) return;
	...
	while(TRUE) {
		...
		opcode ++;
		if(opcode + SAFEDISKSEQLEN > fbuffer + len ) break;
	}
}
#endif