...
DWORD CDib::CreateEmpty(int w, int h, int bpp, RGBQUAD color)
{
	DWORD dwPixSize;
	DWORD dwPalSize = 0;
	...
	bmiHeader.biSizeImage = 0; // can be 0 when biCompression == BI_RGB
	...
	// Go read the bits.
	m_pBits = new BYTE[ dwPixSize + 0x200]; // GHO fix: you need some more space who knows why...? 
	if (m_pBits == 0)
		return 0;
	RGBQUAD *pPixel = (RGBQUAD *)m_pBits;
	for(int y=0; y<h; y++)
		for(int x=0; x<w; x++)
			*(pPixel++) = color;
