...
BOOL isWindowizeEnabled;
BOOL isEarlyHookEnabled;
...
char *sMachineArchitecture(WORD Machine)
{
    ...
    case 0x0:    s="All "; break;
    case 0x14d:  s="Intel i860"; break;
    case 0x14c:  s="Intel i386,i486,i586"; break;
    case 0x200:  s="Intel Itanium processor"; break;
    case 0x8664: s="AMD x64"; break;
    case 0x162:  s="MIPS R3000"; break;
    case 0x166:  s="MIPS R4000"; break;
    case 0x183:  s="DEC Alpha AXP"; break;
    }
	return s;
}

BOOL CheckPE32(CTargetDlg *t)
{
    DWORD FileSize, PeHeaderAddress, Signature;
    FILE*fp=NULL;
	char *path = t->m_FilePath.GetBuffer();
    errno_t err = fopen_s(&fp, path, "rb");
	if(fp == NULL) return FALSE;

	DWORD dwBinaryType;
	if(t->m_IsWin16 || t->m_IsDOS) return TRUE;
	if(GetBinaryTypeA(path, &dwBinaryType)){
		int choice;
		switch (dwBinaryType) {
			case SCS_WOW_BINARY:
				choice = MessageBox(0, "Win16 application\nDo you want to run it through otvdm?", "DxWnd", MB_YESNO);
				if(choice == IDYES) {
					t->m_IsWin16 = TRUE;
					return TRUE;
				}
				...
				break;
			case SCS_DOS_BINARY:
				choice = MessageBox(0, "DOS application\nDo you want to run it through DOSBox?", "DxWnd", MB_YESNO);
				if(choice == IDYES) {
					t->m_IsDOS = TRUE;
					return TRUE;
				}
...
void CTabProgram::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_EARLYHOOK, cTarget->m_EarlyHook);
	...
	DDX_Check(pDX, IDC_FORCEWINDOWING, cTarget->m_ForceWindowing);
	...
	DDX_Check(pDX, IDC_DISABLEGHOSTING, cTarget->m_DisableGhosting);
	...
	if(strlen(cTarget->m_FilePath) > 0) {
		CHAR prefix = cTarget->m_FilePath[0];
		if((prefix != '!') && (prefix != '*')){
			CheckPE32(cTarget);
			if(!cTarget->m_PossiblyProtected && !IsWinXP()){
				CheckSafeDiscVersion(cTarget->m_FilePath.GetBuffer());
				cTarget->m_PossiblyProtected = TRUE;
			}
		}
	}

	CButton *CheckBox;
	CheckBox = (CButton *) GetDlgItem(IDC_EARLYHOOK);
	isEarlyHookEnabled = CheckBox->GetState() & BST_CHECKED;
	CheckBox = (CButton *) GetDlgItem(IDC_WINDOWIZE);
	isWindowizeEnabled = CheckBox->GetState() & BST_CHECKED;
}
...
BEGIN_MESSAGE_MAP(CTabProgram, CDialog)
	...
	ON_BN_CLICKED(IDC_EARLYHOOK, &CTabProgram::OnBnClickedEarlyhook)
	ON_BN_CLICKED(IDC_WINDOWIZE, &CTabProgram::OnBnClickedWindowize)
END_MESSAGE_MAP()
...
void CTabProgram::OnBnClickedEarlyhook()
{
	//CTargetDlg *cTarget = ((CTargetDlg *)(this->GetParent()->GetParent()));
	//cTarget->m_EarlyHook = !cTarget->m_EarlyHook;
	CButton *CheckBox = (CButton *) GetDlgItem(IDC_EARLYHOOK);
	isEarlyHookEnabled = CheckBox->GetState() & BST_CHECKED;
}

void CTabProgram::OnBnClickedWindowize()
{
	CButton *CheckBox = (CButton *) GetDlgItem(IDC_WINDOWIZE);
	isWindowizeEnabled = CheckBox->GetState() & BST_CHECKED;
}
