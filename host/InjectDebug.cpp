...
DWORD WINAPI InjectDebug(void *p)
{
	...
	DWORD flags = DEBUG_ONLY_THIS_PROCESS | CREATE_SUSPENDED;
	if(ThInfo->TM.tflags2 & OUTSTDIO){
	...
	if(!CreateProcess(sRunTargetPath, lpCommandLine, 
		NULL, NULL, TRUE, flags, NULL, path, &sinfo, &pinfo)){
	...
	if((ThInfo->TM.tflags2 & OUTSTDIO) && h) CloseHandle(h);
