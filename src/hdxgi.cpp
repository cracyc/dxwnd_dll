...
HRESULT WINAPI extDXGIQueryInterfaceI(void *, REFIID, void **);
HRESULT WINAPI extDXGIQueryInterfaceO(void *, REFIID, void **);

DXGIQueryInterface_Type pDXGIQueryInterfaceF, pDXGIQueryInterfaceA, pDXGIQueryInterfaceI, pDXGIQueryInterfaceO;
...
HRESULT WINAPI extDXGIGetParent(void *, REFIID, void **);
#ifdef TRACEALL
...
DXGIGetParent_Type pDXGIGetParentA, pDXGIGetParentF, pDXGIGetParent;
#ifdef TRACEALL
...
FARPROC Remap_DXGI_ProcAddress(LPCSTR proc, HMODULE hModule)
{
	...
	// v2.05.84 fix: some calls can be missing on certain platforms, like Win7
	// so it is necessary to use RemapLibraryOpt instead of RemapLibraryEx
	if (addr=RemapLibraryOpt(proc, hModule, DXGIHooks)) return addr;
	return NULL;
}

void HookDXGIUnknownF(void **obj)
{
	ApiName("dxwnd.HookDXGIUnknownF");
	OutTraceD3D("%s\n", ApiRef);
...
{
	ApiName("dxwnd.HookDXGIUnknownA");
	OutTraceD3D("%s\n", ApiRef);
	SetHook((void *)(**(DWORD **)obj +   0), extDXGIQueryInterfaceA, (void **)&pDXGIQueryInterfaceA, "QueryInterface(DXGI)");
}

void HookDXGIUnknownI(void **obj)
{
	ApiName("dxwnd.HookDXGIUnknownI");
	OutTraceD3D("%s\n", ApiRef);
	SetHook((void *)(**(DWORD **)obj +   0), extDXGIQueryInterfaceI, (void **)&pDXGIQueryInterfaceI, "QueryInterface(DXGI)");
}

void HookDXGIObject(void **obj)
{
	ApiName("dxwnd.HookDXGIObject");
	OutTraceD3D("%s\n", ApiRef);
	SetHook((void *)(**(DWORD **)obj +   0), extDXGIQueryInterfaceO, (void **)&pDXGIQueryInterfaceO, "QueryInterface(DXGI)");
	SetHook((void *)(**(DWORD **)obj +  24), extDXGIGetParent, (void **)&pDXGIGetParent, "GetParent(DXGI)");
}
...
void HookDXGIObjectA(void **ppObject)
{
	...
	ApiName("dxwnd.HookDXGIObjectA");
	OutTraceD3D("%s\n", ApiRef);
	SetHook((void *)(**(DWORD **)ppObject +   0), extDXGIQueryInterfaceA, (void **)&pDXGIQueryInterfaceA, "QueryInterface(DXGI)");
#ifdef TRACEALL
...
{
	ApiName("dxwnd.HookDXGISwapChain");
	OutTraceD3D("%s\n", ApiRef);
...
HRESULT WINAPI extDXGIQueryInterface(DXGIQueryInterface_Type pDXGIQueryInterface, void *obj, REFIID riid, void** ppvObj)
{
	...
	switch(riid.Data1){
		...
		case 0x50c83a1c:
			...
			break;
		case 0x25483823:
			OutTraceD3D("> IID_IDXGIFactory3\n");
			HookDXGIFactory(ppvObj); 
			break;
		case 0x1BC6EA02:
			OutTraceD3D("> IID_IDXGIFactory4\n");
			HookDXGIFactory(ppvObj); 
			break;
		case 0x7ABB6563:
			OutTraceD3D("> IID_IDXGIAdapterInternal2\n");
			HookDXGIUnknownI(ppvObj); 
			//HookDXGIObjectA(ppvObj);
			//HookDXGIAdapter(ppvObj); 
			break;
		default:
...
HRESULT WINAPI extDXGIQueryInterfaceI(void *obj, REFIID riid, void** ppvObj)
{ return extDXGIQueryInterface(pDXGIQueryInterfaceI, obj, riid, ppvObj); }
HRESULT WINAPI extDXGIQueryInterfaceO(void *obj, REFIID riid, void** ppvObj)
{ return extDXGIQueryInterface(pDXGIQueryInterfaceO, obj, riid, ppvObj); }
...
{ return intCreateDXGIFactory("dxgi.CreateDXGIFactory", pCreateDXGIFactory, riid, ppFactory); }
HRESULT WINAPI extCreateDXGIFactory1(REFIID riid, void **ppFactory)
{ return intCreateDXGIFactory("dxgi.CreateDXGIFactory1", pCreateDXGIFactory1, riid, ppFactory); }
...
{
	HRESULT res;
	ApiName("dxgi.CreateDXGIFactory2");
...
HRESULT WINAPI extDXGICreateSwapChain(void *obj, IUnknown *pDevice, DXGI_SWAP_CHAIN_DESC *pDesc, IDXGISwapChain **ppSwapChain)
{
	...
	if(dxw.Windowize) {
		// v2.06.08 fix: declare the main window handle (needed for mouse clipping)
		dxw.SethWnd(pDesc->OutputWindow);
...
HRESULT WINAPI extDXGIGetParent(void *obj, REFIID riid, void **ppParent)
{
	HRESULT res;
	ApiName("DXGIFactory::GetParent");
	OutTraceD3D("%s: obj=%#x riid=%s\n", ApiRef, obj, sRIID(riid));
	res = (*pDXGIGetParent)(obj, riid, ppParent);
	if (res) {
		OutErrorD3D("%s: ERROR err=%#x(%s)\n", ApiRef, res, ExplainDDError(res));
		return res;
	} 
...
HRESULT WINAPI extDXGISetFullscreenState(void *obj, BOOL Fullscreen, IDXGIOutput *pTarget)
{
	...
	if(dxw.Windowize && Fullscreen) {
		...
		_if (res) OutErrorD3D("%s: ignore ERROR err=%#x(%s)\n", ApiRef, res, ExplainDDError(res));
		return DD_OK;
	}
