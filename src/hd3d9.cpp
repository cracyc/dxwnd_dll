...
void D3D9TextureHandling(void *arg, int Level)
{
	...
	switch(Desc.Type){
		...
		case D3DRTYPE_TEXTURE:
			break;
		default:
			OutDebugD3D("%s: SKIP type=%#x usage=%#x\n", ApiRef, Desc.Type);
			return;
			break;
	}
...
HRESULT dxGetFrontBuffer9(void *lpd3dd, UINT iSwapChain, LPDIRECTDRAWSURFACE xdest)
{
	...
	if(res) {
		OutErrorD3D("%s: CreateOffscreenPlainSurface ERROR res=%#x(%s) @%d\n", ApiRef, res, ExplainDDError(res), __LINE__);
		return res;
	}
	...
	if(res) {
		OutErrorD3D("%s: GetFrontBufferData ERROR res=%#x(%s) @%d\n", ApiRef, res, ExplainDDError(res), __LINE__);
	...
	OutDebugD3D("%s: dest   rect=(%d,%d)-(%d,%d)\n", ApiRef, dstrect.left, dstrect.top, dstrect.right, dstrect.bottom);
	res = lpDevice->UpdateSurface(lpSurface, &srcrect, lpStretchedSurface, NULL);
	if(res) {
		OutErrorD3D("%s: UpdateSurface ERROR res=%#x(%s) @%d\n", ApiRef, res, ExplainDDError(res), __LINE__);
		lpStretchedSurface->Release();
		lpSurface->Release();
		return res;
	}
	...
	lpStretchedSurface->Release();
	if(res) {
		OutErrorD3D("%s: StretchRect ERROR res=%#x(%s) @%d\n", ApiRef, res, ExplainDDError(res), __LINE__);
		return res;
	}
