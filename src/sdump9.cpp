...
#define dxerror(call) if(res) OutTrace("dx8SurfaceDump %s error %#x(%s) @%d\n", call, res, ExplainDDError(res), __LINE__)
...
void dx9SurfaceDump(int prog, LPDIRECTDRAWSURFACE lpdds)
{
	HRESULT res;
	...
	LPDIRECT3DSURFACE9 lpd9 = (LPDIRECT3DSURFACE9)lpdds;
    res = lpd9->GetDesc(&SurfaceDesc);
	dxerror("GetDesc");
	if(res) return;
	res = lpd9->LockRect(&LockedRect, NULL, D3DLOCK_READONLY);
	dxerror("LockRect");
	if(res) return;
	...
	while (TRUE) {
		...
		OutTrace("SurfaceDump(%d): dxversion=9 lpdds=%#x hash=%08.8X size=(%dx%d) bits=%d format=%#x(%s)\n", 
			prog, lpdds, hash,
			w, h, bpp,
