...
#ifdef HOOKDIQUERYINTERFACES
HRESULT WINAPI extDIQueryInterfaceA1(void *, REFIID, LPVOID *);
HRESULT WINAPI extDIQueryInterfaceA2(void *, REFIID, LPVOID *);
HRESULT WINAPI extDIQueryInterfaceA7(void *, REFIID, LPVOID *);
HRESULT WINAPI extDIQueryInterfaceA8(void *, REFIID, LPVOID *);
HRESULT WINAPI extDIQueryInterfaceW1(void *, REFIID, LPVOID *);
HRESULT WINAPI extDIQueryInterfaceW2(void *, REFIID, LPVOID *);
HRESULT WINAPI extDIQueryInterfaceW7(void *, REFIID, LPVOID *);
HRESULT WINAPI extDIQueryInterfaceW8(void *, REFIID, LPVOID *);
#endif // HOOKDIQUERYINTERFACES
...
#ifdef HOOKDIQUERYINTERFACES
QueryInterface_Type pDIQueryInterfaceA1, pDIQueryInterfaceA2, pDIQueryInterfaceA7, pDIQueryInterfaceA8;
QueryInterface_Type pDIQueryInterfaceW1, pDIQueryInterfaceW2, pDIQueryInterfaceW7, pDIQueryInterfaceW8;
#endif // HOOKDIQUERYINTERFACES
...
void HookDirectInput(HMODULE module)
{
	...
	if(!pDirectInputCreateA && !pDirectInputCreateW && !pDirectInputCreateEx){
		...
		if(!hinst) {
			OutErrorIN("LoadLibrary dinput.dll ERROR err=%d @%d\n", GetLastError(), __LINE__);
			return;
		}
...
void HookDirectInput8(HMODULE module)
{
	...
	if(!pDirectInput8Create){
		...
		if(!hinst) {
			OutErrorIN("LoadLibrary dinput8.dll ERROR err=%d @%d\n", GetLastError(), __LINE__);
			return;
		}
...
void HookDirectInputSession(void **lplpdi, int iVersion, BOOL isAscii)	
{
	if(isAscii){
		switch(iVersion){
			case 1:
#ifdef HOOKDIQUERYINTERFACES
				SetHook((void *)(**(DWORD **)lplpdi), extDIQueryInterfaceA1, (void **)&pDIQueryInterfaceA1, "QueryInterface(I)");
#endif // HOOKDIQUERYINTERFACES
			...
			case 2:
#ifdef HOOKDIQUERYINTERFACES
				SetHook((void *)(**(DWORD **)lplpdi), extDIQueryInterfaceA2, (void **)&pDIQueryInterfaceA2, "QueryInterface(I)");
#endif // HOOKDIQUERYINTERFACES
			...
			case 7:
#ifdef HOOKDIQUERYINTERFACES
				SetHook((void *)(**(DWORD **)lplpdi), extDIQueryInterfaceA7, (void **)&pDIQueryInterfaceA7, "QueryInterface(I)");
#endif // HOOKDIQUERYINTERFACES
			...
			case 8:
#ifdef HOOKDIQUERYINTERFACES
				SetHook((void *)(**(DWORD **)lplpdi), extDIQueryInterfaceA8, (void **)&pDIQueryInterfaceA8, "QueryInterface(I)");
#endif // HOOKDIQUERYINTERFACES
		...
		switch(iVersion){
			case 1:
#ifdef HOOKDIQUERYINTERFACES
				SetHook((void *)(**(DWORD **)lplpdi), extDIQueryInterfaceW1, (void **)&pDIQueryInterfaceW1, "QueryInterface(I)");
#endif // HOOKDIQUERYINTERFACES
			...
			case 2:
#ifdef HOOKDIQUERYINTERFACES
				SetHook((void *)(**(DWORD **)lplpdi), extDIQueryInterfaceW2, (void **)&pDIQueryInterfaceW2, "QueryInterface(I)");
#endif // HOOKDIQUERYINTERFACES
			...
			case 7:
#ifdef HOOKDIQUERYINTERFACES
				SetHook((void *)(**(DWORD **)lplpdi), extDIQueryInterfaceW7, (void **)&pDIQueryInterfaceW7, "QueryInterface(I)");
#endif // HOOKDIQUERYINTERFACES
			...
			case 8:
#ifdef HOOKDIQUERYINTERFACES
				SetHook((void *)(**(DWORD **)lplpdi), extDIQueryInterfaceW8, (void **)&pDIQueryInterfaceW8, "QueryInterface(I)");
#endif // HOOKDIQUERYINTERFACES
...
char *sGUIDProperty(REFGUID p)
{
	...
	DWORD prop = (DWORD)&p; 
	switch(prop){
		case &DIPROP_BUFFERSIZE: s="BUFFERSIZE"; break;
		case &DIPROP_AXISMODE: s="AXISMODE"; break;
		case &DIPROP_GRANULARITY: s="GRANULARITY"; break;
		case &DIPROP_RANGE: s="RANGE"; break;
		case &DIPROP_DEADZONE: s="DEADZONE"; break;
		case &DIPROP_SATURATION: s="SATURATION"; break;
		case &DIPROP_FFGAIN: s="FFGAIN"; break;
		case &DIPROP_FFLOAD: s="FFLOAD"; break;
		case &DIPROP_AUTOCENTER: s="AUTOCENTER"; break;
		case &DIPROP_CALIBRATIONMODE: s="CALIBRATIONMODE"; break;
		case &DIPROP_CALIBRATION: s="CALIBRATION"; break;
		case &DIPROP_GUIDANDPATH: s="GUIDANDPATH"; break;
		case &DIPROP_INSTANCENAME: s="INSTANCENAME"; break;
		case &DIPROP_PRODUCTNAME: s="PRODUCTNAME"; break;
		case &DIPROP_JOYSTICKID: s="JOYSTICKID"; break;
		case &DIPROP_GETPORTDISPLAYNAME: s="GETPORTDISPLAYNAME"; break;
		case &DIPROP_PHYSICALRANGE: s="PHYSICALRANGE"; break;
		case &DIPROP_LOGICALRANGE: s="LOGICALRANGE"; break;
		case &DIPROP_KEYNAME: s="KEYNAME"; break;
		case &DIPROP_CPOINTS: s="CPOINTS"; break;
		case &DIPROP_APPDATA: s="APPDATA"; break;
		case &DIPROP_SCANCODE: s="SCANCODE"; break;
		case &DIPROP_VIDPID: s="VIDPID"; break;
		case &DIPROP_USERNAME: s="USERNAME"; break;
		case &DIPROP_TYPENAME: s="TYPENAME"; break;
		default: s="unknown";
	}
	return s;
}
...
char *sDataFormatFlag(DWORD f)
{
	...
	char *s;
	switch(f & 0xFF){
		case DIDFT_ALL:			s = "ALL"; break;
		case DIDFT_ABSAXIS:		s = "ABSAXIS"; break;
		case DIDFT_RELAXIS:		s = "RELAXIS"; break;
		case DIDFT_AXIS:		s = "AXIS"; break;
		case DIDFT_PSHBUTTON:	s = "PSHBUTTON"; break;
		case DIDFT_TGLBUTTON:	s = "TGLBUTTON"; break;
		case DIDFT_BUTTON:		s = "BUTTON"; break;
		case DIDFT_POV:			s = "POV"; break;
		case DIDFT_COLLECTION:	s = "COLLECTION"; break;	// ???
		case DIDFT_NODATA:		s = "NODATA"; break;		// ???
		default:				s = "???"; break;
	}
	strcpy_s(sBuf, 120, s);
...
char *sODFFlag(DWORD f)
{
	char *s;
	switch(f){
		case DIDOI_ASPECTACCEL: s="ACCEL"; break;
		case DIDOI_ASPECTFORCE: s="FORCE"; break;
		case DIDOI_ASPECTPOSITION: s="POSITION"; break;
		case DIDOI_ASPECTVELOCITY: s="VELOCITY"; break;
		case 0: s="ANY"; break;
...
char *sHow(DWORD h)
{
	char *s;
	switch(h){
		case DIPH_BYOFFSET: s = "BYOFFSET"; break;
		case DIPH_BYID: s = "BYID"; break;
		case DIPH_BYUSAGE: s = "BYUSAGE"; break;
		default: s = "unknown"; break;
	}
	return s;
}
...
{
	HRESULT res;
	ApiName("dinput.DirectInputCreateA");
	...
	if(res) {
		if ((res == OLDDIRECTINPUTVERSION) && (dwversion == 0x800) && OLDDIVERSION){
			OutErrorIN("%s: ERROR err=OLDDIRECTINPUTVERSION version=800 try to call DirectInput8 interface\n", ApiRef);
			if(!pDirectInput8Create) {
				HMODULE hDI8;
		...
		if ((res == DDERR_INVALIDPARAMS) && (hinst == NULL)) {
			OutErrorIN("%s: ERROR err=INVALIDPARAMS try to provide hinst\n", ApiRef);
			res = (*pDirectInputCreateA)(GetModuleHandle(NULL), dwversion, lplpdi, NULL); 
		}
	}
	if(res) {
		OutErrorIN("%s: ERROR err=%#x(%s)\n", ApiRef, res, ExplainDDError(res));
	}
	else {
...
{
	HRESULT res;
	ApiName("dinput.DirectInputCreateW");
	...
	if(res) {
		...
		if ((res == DDERR_INVALIDPARAMS) && (hinst == NULL)) {
			OutErrorIN("%s: ERROR err=INVALIDPARAMS try to provide hinst\n", ApiRef);
			res = (*pDirectInputCreateW)(GetModuleHandle(NULL), dwversion, lplpdi, NULL); 
		}
	}
	if(res) {
		OutErrorIN("%s: ERROR err=%#x(%s)\n", ApiRef, res, ExplainDDError(res));
...
{
	HRESULT res;
	ApiName("dinput.DirectInputCreateEx");
...
HRESULT WINAPI intDIQueryInterface(ApiArg, QueryInterface_Type pDIQueryInterface, void * lpdi, REFIID riid, LPVOID *obp)
{
	HRESULT res;
	int iObjectType;
	int iObjectVersion;
	...
	if(obp == NULL){
		OutTrace("%s: NULL obp ret=E_INVALIDARG\n", ApiRef);
		return E_INVALIDARG;
	}
	...
	switch(riid.Data1){
		case 0x89521360:		//DirectInputA
			iObjectType = TYPE_OBJECT_DIRECTINPUT; iObjectVersion = 1; isAscii = ISASCII;
			break;
		case 0x89521361:		//DirectInputW
			iObjectType = TYPE_OBJECT_DIRECTINPUT; iObjectVersion = 1; isAscii = ISWIDEC;
			break;
		case 0x5944E662:		//DirectInput2A
			iObjectType = TYPE_OBJECT_DIRECTINPUT; iObjectVersion = 2; isAscii = ISASCII;
			break;
		case 0x5944E663:		//DirectInput2W
			iObjectType = TYPE_OBJECT_DIRECTINPUT; iObjectVersion = 2; isAscii = ISWIDEC;
			break;
		case 0x9A4CB684:		//IDirectInput7A
			iObjectType = TYPE_OBJECT_DIRECTINPUT; iObjectVersion = 7; isAscii = ISASCII;
			break;
		case 0x9A4CB685:		//IDirectInput7W
			iObjectType = TYPE_OBJECT_DIRECTINPUT; iObjectVersion = 7; isAscii = ISWIDEC;
			break;
		case 0x5944E680:		// IID_IDirectInputDeviceA
			iObjectType = TYPE_OBJECT_DIDEVICE; iObjectVersion = 1; isAscii = ISASCII;
			break;
		case 0x5944E681:		// IID_IDirectInputDeviceW
			iObjectType = TYPE_OBJECT_DIDEVICE; iObjectVersion = 1; isAscii = ISWIDEC;
			break;
		case 0x5944E682:		// IID_IDirectInputDevice2A
			iObjectType = TYPE_OBJECT_DIDEVICE; iObjectVersion = 2; isAscii = ISASCII;
			break;
		case 0x5944E683:		// IID_IDirectInputDevice2W
			iObjectType = TYPE_OBJECT_DIDEVICE; iObjectVersion = 2; isAscii = ISWIDEC;
			break;
		case 0x57D7C6BC:		// IID_IDirectInputDevice7A
			iObjectType = TYPE_OBJECT_DIDEVICE; iObjectVersion = 7; isAscii = ISASCII;
			break;
		case 0x57D7C6BD:		// IID_IDirectInputDevice7W
			iObjectType = TYPE_OBJECT_DIDEVICE; iObjectVersion = 7; isAscii = ISWIDEC;
			break;
...
{ return intDIQueryInterface("IDirectInputDevice::QueryInterface", pDIQueryInterface, lpdi, riid, obp); }
#ifdef HOOKDIQUERYINTERFACES
HRESULT WINAPI extDIQueryInterfaceA1(void * lpdi, REFIID riid, LPVOID *obp)
{ return intDIQueryInterface("IDirectInputA::QueryInterface", pDIQueryInterfaceA1, lpdi, riid, obp); }
HRESULT WINAPI extDIQueryInterfaceA2(void * lpdi, REFIID riid, LPVOID *obp)
{ return intDIQueryInterface("IDirectInput2A::QueryInterface", pDIQueryInterfaceA2, lpdi, riid, obp); }
HRESULT WINAPI extDIQueryInterfaceA7(void * lpdi, REFIID riid, LPVOID *obp)
{ return intDIQueryInterface("IDirectInput7A::QueryInterface", pDIQueryInterfaceA7, lpdi, riid, obp); }
HRESULT WINAPI extDIQueryInterfaceA8(void * lpdi, REFIID riid, LPVOID *obp)
{ return intDIQueryInterface("IDirectInput8A::QueryInterface", pDIQueryInterfaceA8, lpdi, riid, obp); }
HRESULT WINAPI extDIQueryInterfaceW1(void * lpdi, REFIID riid, LPVOID *obp)
{ return intDIQueryInterface("IDirectInputW::QueryInterface", pDIQueryInterfaceW1, lpdi, riid, obp); }
HRESULT WINAPI extDIQueryInterfaceW2(void * lpdi, REFIID riid, LPVOID *obp)
{ return intDIQueryInterface("IDirectInput2W::QueryInterface", pDIQueryInterfaceW2, lpdi, riid, obp); }
HRESULT WINAPI extDIQueryInterfaceW7(void * lpdi, REFIID riid, LPVOID *obp)
{ return intDIQueryInterface("IDirectInput7W::QueryInterface", pDIQueryInterfaceW7, lpdi, riid, obp); }
HRESULT WINAPI extDIQueryInterfaceW8(void * lpdi, REFIID riid, LPVOID *obp)
{ return intDIQueryInterface("IDirectInput8W::QueryInterface", pDIQueryInterfaceW8, lpdi, riid, obp); }
#endif // HOOKDIQUERYINTERFACES

HRESULT WINAPI extDirectInput8Create(HINSTANCE hinst, DWORD dwversion, REFIID riidltf, LPVOID *ppvout, LPUNKNOWN pu)
{
	HRESULT res;
	ApiName("dinput8.DirectInput8Create");
...
HRESULT WINAPI extDICreateDeviceExA(LPDIRECTINPUTA lpdi, REFGUID rguid, REFIID riid, LPVOID *pvout, LPUNKNOWN pu)
{
	...
	if((dxw.dwFlags6 & VIRTUALJOYSTICK) && (rguid.Data1 == IFakeJoyInstance.Data1)){
		...
		IFakeDirectInputDeviceA *FakeJoy = new(IFakeDirectInputDeviceA);
		switch(riid.Data1){
			case 0x5944E680: FakeJoy->IVersion = 1; break;
			case 0x5944E682: FakeJoy->IVersion = 2; break;
			case 0x57D7C6BC: FakeJoy->IVersion = 7; break;
			default:		 FakeJoy->IVersion = 7; break;
		}
	...
	switch(riid.Data1){
		case 0x5944E680: iObjectVersion = 1; break;
		case 0x5944E682: iObjectVersion = 2; break;
		case 0x57D7C6BC: iObjectVersion = 7; break;
		default:		 iObjectVersion = 7; break;
	}
...
HRESULT WINAPI extDICreateDeviceExW(LPDIRECTINPUTW lpdi, REFGUID rguid, REFIID riid, LPVOID *pvout, LPUNKNOWN pu)
{
	...
	if((dxw.dwFlags6 & VIRTUALJOYSTICK) && (rguid.Data1 == IFakeJoyInstance.Data1)){
		...
		IFakeDirectInputDeviceW *FakeJoy = new(IFakeDirectInputDeviceW);
		switch(riid.Data1){
			case 0x5944E681: FakeJoy->IVersion = 1; break;
			case 0x5944E683: FakeJoy->IVersion = 2; break;
			case 0x57D7C6BD: FakeJoy->IVersion = 7; break;
			default:		 FakeJoy->IVersion = 7; break;
		}
	...
	switch(riid.Data1){
		case 0x5944E681: iObjectVersion = 1; break;
		case 0x5944E683: iObjectVersion = 2; break;
		case 0x57D7C6BD: iObjectVersion = 7; break;
		default:		 iObjectVersion = 7; break;
	}
...
// v2.05.68: fixed bugged handling of returned data elements. Adapted logic for variable DIDEVICEOBJECTDATA
// structure for IDirectInput1-7 vs. IDirectInput8 cases

typedef struct DIDEVICEOBJECTDATA7 {
    DWORD       dwOfs;
    DWORD       dwData;
    DWORD       dwTimeStamp;
    DWORD       dwSequence;
} DIDEVICEOBJECTDATA7, *LPDIDEVICEOBJECTDATA7;
...
HRESULT WINAPI extGetDeviceData(LPDIRECTINPUTDEVICE lpdid, DWORD cbdata, LPVOID rgdod, LPDWORD pdwinout, DWORD dwflags)
{
	...
	if((RECOVERDATAFORMAT) && (res == DIERR_NOTBUFFERED)){
		OutErrorIN("%s: recovering DIERR_NOTBUFFERED\n", ApiRef); 
		DIPROPDWORD p;
		res = (*pUnacquire)(lpdid);
		OutErrorIN("%s: Unacquire res=%#x(%s)\n", ApiRef, res, ExplainDDError(res)); 
		...
		OutErrorIN("%s: SetProperty res=%#x(%s)\n", ApiRef, res, ExplainDDError(res)); 
		res = (*pAcquire)(lpdid);
		OutErrorIN("%s: Acquire res=%#x(%s)\n", ApiRef, res, ExplainDDError(res)); 
		in  = (int)*pdwinout;
		res = (*pGetDeviceData)(lpdid, cbdata, rgdod, pdwinout, dwflags);
		out = (int)*pdwinout;
		out = (in >= out) ? out : in;
	...
	if (IsDebugIN && (res == DI_OK)){
		LPDIDEVICEOBJECTDATA7 lpdod = (LPDIDEVICEOBJECTDATA7)rgdod;
		OutTrace("%s: inout=%d\n", ApiRef, *pdwinout);
		tmp = (BYTE *)rgdod;
		if (out > 0) for(int i=0; i<out; i++){
			LPDIDEVICEOBJECTDATA p = (LPDIDEVICEOBJECTDATA)tmp;
			if(cbdata == sizeof(LPDIDEVICEOBJECTDATA7))
				OutTrace("> dod[%d]: ofs=%d data=%#x seq=%d time=%#x\n", 
					i, p->dwOfs, p->dwData, p->dwSequence, p->dwTimeStamp);
			else // IDirectInput8
				OutTrace("> dod[%d]: ofs=%d data=%#x seq=%d time=%#x appdata=%#x\n", 
					i, p->dwOfs, p->dwData, p->dwSequence, p->dwTimeStamp, p->uAppData);
			tmp += cbdata;
		}
	}
#endif // DXW_NOTRACES
	if (((res == DIERR_INPUTLOST) || (res == DIERR_NOTACQUIRED)) && RECOVERINPUTLOST){
		OutErrorIN("%s: recovering DIERR_INPUTLOST\n", ApiRef); 
		if(lpdid == lpDIDSysMouse) {
			res = (*pDISetCooperativeLevel)(lpdid, dxw.GethWnd(), DISCL_NONEXCLUSIVE | DISCL_FOREGROUND);
			_if(res) OutErrorIN("%s: SetCooperativeLevel ERROR: err=%#x(%s)\n", ApiRef, res, ExplainDDError(res)); 
			if(dxw.dwFlags1 & CLIPCURSOR) dxw.SetClipCursor();
		}
		res = (*pAcquire)(lpdid);
		_if(res) OutErrorIN("%s: Acquire ERROR: err=%#x(%s)\n", ApiRef, res, ExplainDDError(res)); 
		res = (*pGetDeviceData)(lpdid, cbdata, rgdod, pdwinout, dwflags);
	}
	...
	if(lpdid == lpDIDSysMouse){
		if(dxw.dwFlags4 & RELEASEMOUSE) {
			...
			if ((curr.x < client.left) || (curr.y < client.top) || (curr.x > client.right) || (curr.y > client.bottom)){
				*pdwinout = 0;
				GetHookInfo()->DInputX=(short)-1;
				GetHookInfo()->DInputY=(short)-1;
				return DI_OK;
			}
		}
		...
		if(dxw.bDInputAbs){
			...
			GetHookInfo()->DInputX=(short)(p.x);
			GetHookInfo()->DInputY=(short)(p.y);
		}
		else{
			if((dxw.dwFlags6 & EMULATERELMOUSE) && dxw.bActive){
				...
				//OutDebugIN("%s: RELATIVE setpos=(%d,%d)\n", ApiRef, p.x, p.y);
				if(!(*pSetCursorPos)(p.x, p.y)) OutTrace("%s: RELATIVE setpos ERROR err=%d\n", ApiRef, GetLastError());
				GetHookInfo()->DInputX=(short)(p.x);
				GetHookInfo()->DInputY=(short)(p.y);
			}
			else if((dxw.dwFlags18 & PACKMOUSEDATA) && (out > 2)){
				// v2.06.03: packed mouse movements
				int totalX, totalY;
				totalX = totalY = 0;
				for(i = 0; i < out; i ++){
					if(((LPDIDEVICEOBJECTDATA)tmp)->dwOfs == DIMOFS_X) totalX += ((LPDIDEVICEOBJECTDATA)tmp)->dwData;
					if(((LPDIDEVICEOBJECTDATA)tmp)->dwOfs == DIMOFS_Y) totalY += ((LPDIDEVICEOBJECTDATA)tmp)->dwData;
					tmp += cbdata;
				}
				tmp = (BYTE *)rgdod;
				((LPDIDEVICEOBJECTDATA)tmp)->dwOfs = DIMOFS_X;
				((LPDIDEVICEOBJECTDATA)tmp)->dwData = totalX;
				tmp += cbdata;
				((LPDIDEVICEOBJECTDATA)tmp)->dwOfs = DIMOFS_Y;
				((LPDIDEVICEOBJECTDATA)tmp)->dwData = totalY;
				OutTraceIN("%s: PACKED %d movements to 2 (x,y)=(%d,%d)\n", ApiRef, out, totalX, totalY);
				*pdwinout = 2;
			}
			if(dxw.dwFlags6 & SCALERELMOUSE){
				...
				for(i = 0; i < out; i ++){
					if(((LPDIDEVICEOBJECTDATA)tmp)->dwOfs == DIMOFS_X){
						...
						((LPDIDEVICEOBJECTDATA7)tmp)->dwData = (DWORD)p.x;
					}
					if(((LPDIDEVICEOBJECTDATA)tmp)->dwOfs == DIMOFS_Y){
						POINT p;
						p.y = (LONG)((LPDIDEVICEOBJECTDATA7)tmp)->dwData;
						p.x = 0;
						dxw.ScaleRelMouse(ApiRef, &p);
						((LPDIDEVICEOBJECTDATA7)tmp)->dwData = (DWORD)p.y;
					}
					tmp += cbdata;
				}
				GetHookInfo()->DInputX=(short)(p.x);
...
HRESULT WINAPI extGetDeviceState(LPDIRECTINPUTDEVICE lpdid, DWORD cbdata, LPDIMOUSESTATE lpvdata)
{
	...
	if (((res == DIERR_INPUTLOST) || (res == DIERR_NOTACQUIRED)) && RECOVERINPUTLOST){
		OutErrorIN("%s: did=%#x recovering DIERR_INPUTLOST\n", ApiRef, lpdid); 
		if(lpdid == lpDIDSysMouse) {
			res = (*pDISetCooperativeLevel)(lpdid, dxw.GethWnd(), DISCL_NONEXCLUSIVE | DISCL_FOREGROUND);
			_if(res) OutErrorIN("%s: SetCooperativeLevel ERROR: err=%#x(%s)\n", ApiRef, res, ExplainDDError(res)); 
			if(dxw.dwFlags1 & CLIPCURSOR) dxw.SetClipCursor();
	...
	if(	cbdata == sizeof(DIMOUSESTATE) || cbdata == sizeof(DIMOUSESTATE2)){
		...
		if(dxw.bDInputAbs){ // absolute position
			...
			if(dxw.dwFlags6 & EMULATERELMOUSE){
				...
				if(!(*pSetCursorPos)(iCursorX, iCursorY)){
					OutTraceIN("%s: SetCursorPos ERROR pos=(%d,%d) err=%d\n", 
						ApiRef, iCursorX, iCursorY, GetLastError());
				}
			}
			if(dxw.dwFlags6 & SCALERELMOUSE){
				POINT p;
...
char *ExplainDataFormatFlags(DWORD f)
{
	...
	s="unknown";
	switch(f){
		case DIDF_ABSAXIS: s="DIDF_ABSAXIS"; break;
		case DIDF_RELAXIS: s="DIDF_RELAXIS"; break;
		default: s="unknown"; break;
	}
	return s;
}
...
HRESULT WINAPI extSetDataFormat(LPDIRECTINPUTDEVICE lpdid, LPCDIDATAFORMAT lpdf)
{
	...
	_if(res) OutErrorIN("%s: ERROR err=%#x(%s)\n", ApiRef, res, ExplainDDError(res));
	return res;
} 
...
HRESULT WINAPI extDISetCooperativeLevel(LPDIRECTINPUTDEVICE lpdid, HWND hwnd, DWORD dwflags)
{
	...
	OutTraceIN("%s: did=%#x(%s) hwnd=%#x flags=%#x(%s)\n", 
		ApiRef, lpdid, sDevice(lpdid), hwnd, dwflags, ExplainDICooperativeFlags(dwflags, sFlags, 128));
	...
	if((lpdid == lpDIDKeyboard) && (dxw.dwFlags7 & SHAREDKEYBOARD)) {
		...
		OutTraceIN("%s: SYSKEYBOARD flags=%#x(%s)\n", ApiRef, dwflags, ExplainDICooperativeFlags(dwflags, sFlags, 128));
	}
	// v2.06.01: fix for "Polska Goola"
	if( dxw.Windowize && 
		(lpdid == lpDIDSysMouse) && 
		(dxw.dwFlags17 & SHAREDMOUSE) && 
		(dwflags & DISCL_EXCLUSIVE)){
		dwflags &= ~DISCL_EXCLUSIVE;
		dwflags |= DISCL_NONEXCLUSIVE;
		OutTraceIN("%s: SHAREDMOUSE flags=%#x(%s)\n", ApiRef, dwflags, ExplainDICooperativeFlags(dwflags, sFlags, 128));
	}

	res = (*pDISetCooperativeLevel)(lpdid, hwnd, dwflags);

	// v2.06.00: fix needed for "Gangsters 2"
	// The mode DISCL_NONEXCLUSIVE | DISCL_FOREGROUND is no good for this game (unsupported) and
	// maybe in general. So, better use the original mode and try to use alternate modes only in 
	// case of failure. Here we try (DISCL_NONEXCLUSIVE | DISCL_FOREGROUND) first and in case of 
	// further error (DISCL_NONEXCLUSIVE | DISCL_BACKGROUND) but the list could get longer ...
	if(lpdid == lpDIDSysMouse) {
		if(res) {
			OutErrorIN("%s: ERROR err=%#x(%s)\n", ApiRef, res, ExplainDDError(res));
			dwflags = (DISCL_NONEXCLUSIVE | DISCL_FOREGROUND);
...
#define DI8DEVTYPE_TYPEMASK 0x1F
...
HRESULT WINAPI extDeviceProxy(LPCDIDEVICEINSTANCE dev, LPVOID arg)
{
	...
	// v2.05.98: fix on HIDEJOYSTICKS criteria
	if((dxw.dwFlags9 & HIDEJOYSTICKS) && ((dev->dwDevType & DI8DEVTYPE_TYPEMASK) == DIDEVTYPE_JOYSTICK)) {
		OutTraceIN("%s: skip JOYSTICK device devtype=%#x\n", ApiRef, dev->dwDevType);
		return TRUE; // skip DIDEVTYPE_JOYSTICK
	}

	if((dxw.dwFlags9 & HIDEJOYSTICKS) && ((dev->dwDevType & DI8DEVTYPE_TYPEMASK) >= DI8DEVTYPE_JOYSTICK)) {
...
char *sEnumFlags(DWORD c)
{
	...
	unsigned int l;
	strcpy(eb,"DIEDFL_");
	if (c & DIEDFL_ATTACHEDONLY) strcat(eb, "ATTACHEDONLY+"); else strcat(eb, "ALLDEVICES+");
...
HRESULT WINAPI extDIEnumDevicesA(int iDIVersion, DIEnumDevicesA_Type pDIEnumDevices, void *lpdi, DWORD dwDevType, LPDIENUMDEVICESCALLBACKA lpCallback, LPVOID pvRef, DWORD dwFlags)
{
	...
	if(dxw.dwFlags6 & VIRTUALJOYSTICK) {
		...
		// v2.05.92 fix: enumerate DIEDFL_FORCEFEEDBACK only when effects are enabled
		if((dwFlags & DIEDFL_FORCEFEEDBACK) && !(dxw.dwFlags16 & JOYSTICKEFFECTS)) vJoy = FALSE;
...
void ToggleAcquiredDevices(BOOL flag)
{
	...
	__try {
		OutTraceDW("ToggleAcquiredDevices: flag=%#x\n", flag);
		if(flag && pAcquire){
			SetCapture(dxw.GethWnd());
...
static 	void RemapUSKey(DWORD ofs, char *name)
{
	char *p = NULL;
	switch(ofs){
		case   1: p = "Escape";			break;
		case  14: p = "Backspace";		break;
		case  15: p = "Tab";			break;
		case  28: p = "Enter";			break;
		case  29: p = "Left Ctrl";		break;
		case  42: p = "Left Shift";		break;
		case  54: p = "Right Shift";	break;
		case  55: p = "Numpad *";		break;
		case  56: p = "Left Alt";		break;
		case  57: p = "Space";			break;
		case  58: p = "CapsLock";		break;
		case  69: p = "NumLock";		break;
		case  70: p = "ScrollLock";		break;
		case  71: p = "Numpad 7";		break;
		case  72: p = "Numpad 8";		break;
		case  73: p = "Numpad 9";		break;
		case  74: p = "Numpad -";		break;
		case  75: p = "Numpad 4";		break;
		case  76: p = "Numpad 5";		break;
		case  77: p = "Numpad 6";		break;
		case  78: p = "Numpad +";		break;
		case  79: p = "Numpad 1";		break;
		case  80: p = "Numpad 2";		break;
		case  81: p = "Numpad 3";		break;
		case  82: p = "Numpad 0";		break;
		case  83: p = "Numpad .";		break;
		case  86: p = "OEM Key 102";	break;
		case 115: p = "Non-US / ?";		break;
		case 126: p = "Non-US Numpad .";break;
		case 144: p = "Prev Track";		break;
		case 153: p = "Next Track";		break;
		case 156: p = "Numpad Enter";	break;
		case 157: p = "Right CTRL";		break;
		case 160: p = "Mute";			break;
		case 161: p = "Calculator";		break;
		case 162: p = "Play/Pause";		break;
		case 164: p = "Media Stop";		break;
		case 174: p = "Volume -";		break;
		case 176: p = "Volume +";		break;
		case 178: p = "Web/Home";		break;
		case 181: p = "Numpad /";		break;
		case 183: p = "SysRq";			break;
		case 184: p = "Right Alt";		break;
		case 197: p = "Pause";			break;
		case 199: p = "Home";			break;
		case 200: p = "Up Arrow";		break;
		case 201: p = "PgUp";			break;
		case 203: p = "Left Arrow";		break;
		case 205: p = "Right Arrow";	break;
		case 207: p = "End";			break;
		case 208: p = "Down Arrow";		break;
		case 209: p = "PgDn";			break;
		case 210: p = "Insert";			break;
		case 211: p = "Delete";			break;
		case 219: p = "Left Win";		break;
		case 220: p = "Right Win";		break;
		case 221: p = "AppMenu";		break;
		case 222: p = "Power";			break;
		case 223: p = "Sleep";			break;
		case 227: p = "Wake";			break;
		case 229: p = "Search";			break;
		case 230: p = "Favorites";		break;
		case 231: p = "Refresh";		break;
		case 232: p = "Web Stop";		break;
		case 233: p = "Forward";		break;
		case 234: p = "Back";			break;
		case 235: p = "My Computer";	break;
		case 236: p = "Mail";			break;
		case 237: p = "Media";			break;
	}
	if(p) {
...
HRESULT WINAPI extEnumObjectsProxy(LPCDIDEVICEOBJECTINSTANCE dev, LPVOID arg)
{
	...
	switch (dev->dwSize) {
		...
		default: 
			p="UNKNOWN"; 
			OutErrorIN("%s: unknown struct size=%d\n", ApiRef, dev->dwSize);
			break;
	}
...
HRESULT WINAPI extSetProperty(LPDIRECTINPUTDEVICE lpdid, REFGUID rguidProp, LPCDIPROPHEADER lpdph)
{
	...
	if(res != DI_OK) {
		OutErrorIN("%s: ERROR ret=%#x(%s)\n", ApiRef, res, ExplainDDError(res));
	} else {
		OutTraceIN("%s: res=OK\n", ApiRef);
	}

	// v2.05.65: fix to bypass an error if the device is acquired. Fixes "Tytus Romek I A'Tomek".
...
HRESULT WINAPI extGetCapabilities(LPDIRECTINPUTDEVICE lpdid, LPDIDEVCAPS lpdidc)
{
	...
	if((dxw.dwFlags6 & VIRTUALJOYSTICK) && (lpdid == gFakeJoy)){
		MessageBox(0, "IDirectInputDevice:GetCapabilities on fake joystick", "DxWnd", 0);
		return 0;
	}

	res = (*pGetCapabilities)(lpdid, lpdidc);
	if(res){
		OutErrorIN("%s: ERROR lpdid=%#x ret=%#x(%s)\n", ApiRef, lpdid, res, ExplainDDError(res));
	}
	else{
		OutTraceIN("%s: lpdid=%#x "
