...
void DDrawScreenShot(int dxversion, int hash)
{
	...
	// v2.05.78: try both ways just in case ...
	if(dxw.Renderer->flags & DXWRF_USEBACKBUFFER){
		// to do: SDL renderer has a better way ...
		s = dxwss.GetBackBufferSurface();
		if(!s) s = dxwss.GetPrimarySurface();
	}
	else{
		s = dxwss.GetPrimarySurface();
		if (!s) s = dxwss.GetBackBufferSurface();
	}
	if(!s) {
		OutTraceE("%s: ScreenShot: no surface!\n", ApiRef);
		return;
	}
	...
	while (TRUE) { // fake loop
		OutTraceDW("%s: lpdds=%#x BitCount=%d size=(%dx%d)\n", 
			ApiRef, s, ddsd.ddpfPixelFormat.dwRGBBitCount, ddsd.dwWidth, ddsd.dwHeight);
		w = ddsd.dwWidth;
		h = ddsd.dwHeight;
		if(ddsd.ddpfPixelFormat.dwRGBBitCount == 0) {
			OutTraceE("%s: SKIP 0BPP texture\n", ApiRef);
			break;
		}
...
#ifndef DXW_NOTRACES
	if (res) OutTraceE("%s: Unlock ERROR lpdds=%#x res=%#x(%s) @%d\n", 
