...
DWORD WINAPI extGetFileVersionInfoSizeA(LPCSTR, LPDWORD);
...
static HookEntryEx_Type VersionHooks[]={
	...
	{HOOK_IAT_CANDIDATE, 0, "GetFileVersionInfoSizeA", (FARPROC)NULL, (FARPROC *)&pGetFileVersionInfoSizeA, (FARPROC)extGetFileVersionInfoSizeA},
...
{
	OutTraceDW("HookVersion: hmod=%#x\n", module);
	if((dxw.dwFlags11 & CUSTOMLOCALE) || (dxw.dwFlags10 & (FAKECDDRIVE|FAKEHDDRIVE)))
		HookLibraryEx(module, VersionHooks, libname);
}
...
BOOL WINAPI extGetFileVersionInfoA(LPCSTR lptstrFilename, DWORD dwHandle, DWORD dwLen, LPVOID lpData)
{
	...
	if(dxw.dwFlags10 & (FAKECDDRIVE|FAKEHDDRIVE)){
		lptstrFilename = dxwTranslatePathA(lptstrFilename, NULL);
		OutTraceDW("%s: mapping path=\"%s\"\n", ApiRef, lptstrFilename);
	}
...
DWORD WINAPI extGetFileVersionInfoSizeA(LPCSTR lptstrFilename, LPDWORD lpdwHandle)
{
	DWORD ret;
	ApiName("GetFileVersionInfoSizeA");
	OutTraceSYS("%s: fname=\"%s\" lphandle=%#x\n", 
		ApiRef, lptstrFilename, lpdwHandle);
	...
	ret = (*pGetFileVersionInfoSizeA)(lptstrFilename, lpdwHandle);
	OutTraceDW("%s: ret=%d\n", ApiRef, ret);
	return ret;
}
	...
	if(ret && (dxw.dwFlags11 & CUSTOMLOCALE) && (pBlock == gFakeLocaleData)){
		...
		if(!strncmp(lpSubBlock, "\\VarFileInfo\\Translation", strlen("\\VarFileInfo\\Translation"))){
			...
			if(*puLen == 4){
				...
				if(dxw.Locale) langcodepage[0] = (WORD)dxw.Locale;
