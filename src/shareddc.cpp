...
//#define D3D9TRY
#define SHAREDDCDEBUG TRUE
#if SHAREDDCDEBUG
//#define _Warn(s) {char cap[80]; sprintf(cap, "Warn @%d", __LINE__); MessageBox(NULL, (s), cap, MB_OK);}
#define _Warn(s) {OutTrace("Warn: %s\n", s);}
#else
#define _Warn(s) 
#endif
...
extern ExtSelectClipRgn_Type pExtSelectClipRgn;
extern GetClipRgn_Type pGetClipRgn;

/* to check for correct use of pointers / wrappers:
GetViewportOrgEx
SetViewportOrgEx
IsWindowVisible
OffsetRect
...
dxwSDC::dxwSDC()
{
	OutDebugDW("dxwSDC::dxwSDC: Initialize\n");
	PrimaryDC = NULL;
	lpDDSPrimary = NULL;
...
dxwSDC::~dxwSDC()
{
	//OutDebugDW("dxwSDC::~dxwSDC: Destroy\n");
}
...
HDC dxwSDC::GetPrimaryDC(HDC hdc, HDC hdcsrc)
{
	...
	OutDebugDW("dxwSDC::GetPrimaryDC: hdc=%#x\n", hdc);
	...
	if (lpDDSPrimary) { 
		...
		while((PrimaryDC == NULL) && lpDDSPrimary) { 
			OutDebugDW("dxwSDC::GetPrimaryDC: found primary surface with no DC, unref lpdds=%#x\n", lpDDSPrimary);
		...
		if (!PrimaryDC) {
			_Warn("No primary DC");
			OutDebugDW("dxwSDC::GetPrimaryDC: no ddraw primary DC\n");
			return NULL;
		}
		// avoid double Getdc on same hdc and lock
		// if(PrimaryDC == hdc) (*pReleaseDC)(lpDDSPrimary, PrimaryDC);
		OutDebugDW("dxwSDC::GetPrimaryDC: ddraw PrimaryDC=%#x\n", PrimaryDC);
		...
		if (!PrimaryDC) {
			_Warn("No window DC");
			OutDebugDW("dxwSDC::GetPrimaryDC: no windows DC\n");
			return NULL;
		}
		OutDebugDW("dxwSDC::GetPrimaryDC: gdi PrimaryDC=%#x\n", PrimaryDC);
	...
	if(hdc != LastHDC) do {
		...
		OutTrace("WindowFromDC(hdc)=%#x hdc=%#x\n", CurrenthWnd, hdc);
		...
		if(client.right && client.bottom){
			...
			ScreenWidth = dxw.GetScreenWidth();
			ScreenHeight = dxw.GetScreenHeight();
		}

		OutDebugDW("dxwSDC::GetPrimaryDC: VirtualHDC INITIALIZE last size=(%dx%d) current size=(%dx%d)\n", 
	...
	if(CurrenthWnd && CurrenthWnd!=dxw.GethWnd()){
		...
		VirtualOffset.x = 0;
		VirtualOffset.y = 0;
		OutDebugDW("dxwSDC::GetPrimaryDC: same window\n");
	}
	...
	// v2.04.30: gsky916 leakage discovery
    //(*pGetDCOrgEx)((*pGDIGetDC)(dxw.GethWnd()), &mainwin); // replaced by following 3 LOC
    HDC winhdc = (*pGDIGetDC)(dxw.GethWnd());
    (*pGetDCOrgEx)(winhdc, &mainwin);
    (*pGDIReleaseDC)(dxw.GethWnd(), winhdc);
    // v2.04.30: end of gsky916 fix
	origin.x -= mainwin.x;
	origin.y -= mainwin.y;
	OutDebugDW("dxwSDC::GetPrimaryDC: origin=(%d,%d)\n", origin.x, origin.y);
...
BOOL dxwSDC::PutPrimaryDC(HDC hdc, BOOL UpdateScreen, int XDest, int YDest, int nDestWidth, int nDestHeight, BOOL fpsControl)
{
	...
	if (IsDebugDW){
		...
		OutTrace("dxwSDC::PutPrimaryDC: hdc=%#x type=%s update=%#x fpsc=%d %s\n", hdc, sType, UpdateScreen, fpsControl, sRect);
	}
#endif // DXW_NOTRACES
	...
	if(UpdateScreen){
		switch(VirtualSurfaceType){
			case VIRTUAL_ON_DDRAW: 
				...
				// v2.04.30: gsky916 fix - avoid double FPS handling since it is performed already within GDI32/USER32 hookers
				// v2.05.80: fix: do FPS control unless asked differently. For instance, in "BeachHead 2000" the screen updates
				// for Text output should not be delayed nor skipped!
				if(!fpsControl){
					dxw.SuspendFPS();
					dxw.ScreenRefresh(TRUE); 
					dxw.ResumeFPS();
				}
				else {
					dxw.ScreenRefresh(FALSE);
				}
				...
				if((dxw.dwFlags8 & SHAREDDCHYBRID) && CurrentHDCSrc && (WindowFromDC(CurrentHDC)!=dxw.GethWnd())){
					...
					OutDebugDW("dxwSDC::PutPrimaryDC: StretchBlt over ddraw\n");
					nXDest= XDest;
					nYDest= YDest;
					nWDest= nDestWidth;
					nHDest= nDestHeight;
			...
			case VIRTUAL_ON_WINDOW:
				...
				ret=TRUE;
				if(PrimaryDC){
					extern BOOL dxwStretchBlt(HDC, int, int, int, int, HDC, int, int, int, int, DWORD);
					if (dxw.dwFlags15 & FORCEHALFTONETINY)
						ret=dxwStretchBlt(PrimaryDC, RealArea.left, RealArea.top, RealArea.right, RealArea.bottom, VirtualHDC, VirtualArea.left, VirtualArea.top, VirtualArea.right, VirtualArea.bottom, SRCCOPY);
					else
						ret=(*pGDIStretchBlt)(PrimaryDC, RealArea.left, RealArea.top, RealArea.right, RealArea.bottom, VirtualHDC, VirtualArea.left, VirtualArea.top, VirtualArea.right, VirtualArea.bottom, SRCCOPY);
				}
	...
	OutDebugDW("dxwSDC::PutPrimaryDC: hdc=%#x PrimaryDC=%#x ret=%#x\n", hdc, PrimaryDC, ret);
	return ret;
}

BOOL dxwSDC::PutPrimaryDC(HDC hdc, BOOL UpdateScreen, int XDest, int YDest, int nDestWidth, int nDestHeight)
{
	return PutPrimaryDC(hdc, UpdateScreen, 0, 0, LastScreenWidth, LastScreenHeight, TRUE);
}
...
BOOL dxwSDC::PutPrimaryDC(HDC hdc, BOOL UpdateScreen, BOOL fpsControl)
{
	return PutPrimaryDC(hdc, UpdateScreen, 0, 0, LastScreenWidth, LastScreenHeight, fpsControl);
}
...
void dxwSDC::copyDcAttributes(HDC destDC, HDC origDc, POINT origin)
{
	...
	(*pSetTextColor)(destDC, GetTextColor(origDc));

	OutDebugDW("dxwSDC::copyDcAttributes: orig=(%d,%d)\n", origin.x, origin.y);
#ifndef DXW_NOTRACES
...
#else
	(*pSetWindowOrgEx)(destDC, -origin.x, -origin.y, NULL);
#endif // DXW_NOTRACES
...
void dxwSDC::setClippingRegion(HDC compatDc, HDC origDc, POINT origin)
{
	...
	if (1 == (*pGetClipRgn)(origDc, origClipRgn))
	{
		OutDebugDW("dxwSDC::setClippingRegion: GetClipRgn==1\n");
		OffsetRgn(origClipRgn, origin.x, origin.y);
		(*pExtSelectClipRgn)(compatDc, origClipRgn, RGN_AND);
		if(IsDebugDW){
			RECT RgnBox;
			GetRgnBox(origClipRgn, &RgnBox); // for logging only
			OutDebugDW("dxwSDC::setClippingRegion: OrigRgnBox=(%d,%d)-(%d,%d)\n", RgnBox.left, RgnBox.top, RgnBox.right, RgnBox.bottom);
		}
	}
	...
	if(dxw.dwFlags7 & FIXCLIPPERAREA){
		...
		if (!isEmptyClipRgn){
			OutDebugDW("dxwSDC::setClippingRegion: isEmptyClipRgn FALSE\n");
			...
			if (hwnd && (!dxw.IsDesktop(hwnd))){
				...
				OutDebugDW("dxwSDC::setClippingRegion: compatdc=%#x origin=(%d,%d) ancestor=%#x\n", 
