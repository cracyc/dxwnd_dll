...
typedef HSMACKBLIT (WINAPI *SmackBlitClose_Type)(u32);
...
typedef void	(WINAPI *SmackBufferNewPalette_Type)(HSMACKBUF, void *, unsigned int);

SmackOpen_Type pSmackOpen;
SmackClose_Type pSmackClose, pSmackBufferClose, pSmackBufferClear;
...
SmackBlitClose_Type pSmackBlitClose;
...
SmackBufferNewPalette_Type pSmackBufferNewPalette;
...
void WINAPI extSmackBufferClear(Smack *);
...
HSMACKBLIT WINAPI extSmackBlitClose(u32);
...
void WINAPI extSmackBufferNewPalette(HSMACKBUF, void *, unsigned int);

#if 0
// new calls from https://github.com/TheNitesWhoSay/lawine/blob/master/lawine/misc/smack.c#L33
...
static HookEntryEx_Type Hooks31N[]={
	...
	{HOOK_IAT_CANDIDATE, 19, "_SmackBufferNewPalette@12", (FARPROC)NULL, (FARPROC *)&pSmackBufferNewPalette, (FARPROC)extSmackBufferNewPalette},
#endif
	{HOOK_IAT_CANDIDATE, 0, 0, NULL, 0, 0} // terminator
};
...
static HookEntryEx_Type Hooks32G[]={
	...
	{HOOK_IAT_CANDIDATE,  0, "_SmackBlitClose@4", (FARPROC)NULL, (FARPROC *)&pSmackBlitClose, (FARPROC)extSmackBlitClose},
	{HOOK_IAT_CANDIDATE, 20, "_SmackBufferOpen@24", (FARPROC)NULL, (FARPROC *)&pSmackBufferOpen, (FARPROC)extSmackBufferOpen},
	{HOOK_IAT_CANDIDATE, 14, "_SmackBufferClose@4", (FARPROC)NULL, (FARPROC *)&pSmackBufferClose, (FARPROC)extSmackBufferClose},
	{HOOK_IAT_CANDIDATE, 48, "_SmackSoundUseDirectSound@4", (FARPROC)NULL, (FARPROC *)&pSmackSoundUseDirectSound, (FARPROC)extSmackSoundUseDirectSound},
	{HOOK_IAT_CANDIDATE, 12, "_SmackBufferBlit@32", (FARPROC)NULL, (FARPROC *)&pSmackBufferBlit, (FARPROC)extSmackBufferBlit},
...
#ifdef SMACK_TRACEALL
	{HOOK_IAT_CANDIDATE, 0x00, "_SmackBufferClear", (FARPROC)NULL, (FARPROC *)&pSmackBufferClear, (FARPROC)extSmackBufferClear}, // ???
#endif
	{HOOK_IAT_CANDIDATE, 0, 0, NULL, 0, 0} // terminator
};
...
Smack * WINAPI extSmackOpen(const char *SmackFile, UINT32 flags, INT32 extrabuf)
{
	...
	if(IsTraceDW){
		if((DWORD)SmackFile & 0xFFFF0000){
			OutTraceDW("%s: SmackFile=\"%s\" flags=%#x(%s) extrabuf=%#x\n", 
				ApiRef, SmackFile, flags, ExplainSmackFlags(flags), extrabuf);
		}
		else {
			OutTraceDW("%s: SmackFile=0x%04.4X flags=%#x(%s) extrabuf=%#x\n", 
				ApiRef, SmackFile, flags, ExplainSmackFlags(flags), extrabuf);
		}
	}
	...
	// v2.06.09: added fake devices handling
	if(dxw.dwFlags10 & (FAKEHDDRIVE | FAKECDDRIVE)) {
		DWORD mapping;
		SmackFile = dxwTranslatePathA(SmackFile, &mapping);
		if(mapping != DXW_NO_FAKE){
			OutTraceDW("%s: REMAPPED SmackFile=\"%s\"\n", ApiRef, SmackFile);
		}
	}
...
Smack * WINAPI extSmackColorRemapWithTrans(Smack *h, const void *remappal, u32 numcolors, u32 paltype, u32 transindex)
{
	...
	DumpSmack(h);
	// BYPASS the call to avoid resolution changes
	//return (*pSmackColorRemapWithTrans)(h, remappal, numcolors, paltype, transindex);
	return h;
}
...
void WINAPI extSmackBufferClear(Smack *h)
{
	OutTraceDW("SmackBufferClear: h=%#x\n", h);
	DumpSmack(h);
	(*pSmackBufferClear)(h);
}
#endif
...
HSMACKBLIT WINAPI extSmackBlitOpen(u32 flags)
{
	...
	OutTraceDW("SmackBlitOpen: ret(hSmackBlit)=%#x\n", ret);
	return ret;
}
#endif // SMACK_TRACEALL

#ifdef SMACK_TRACEALL
HSMACKBLIT WINAPI extSmackBlitClose(u32 hSmackBlit)
{
	HSMACKBLIT ret;
	OutTraceDW("SmackBlitClose: hSmackBlit=%#x\n", hSmackBlit);
	ret = (*pSmackBlitClose)(hSmackBlit);
	OutTraceDW("SmackBlitClose: ret=%#x(%s)\n", ret, ret ? "OK" : "ERR");
	return ret;
}
#endif // SMACK_TRACEALL
...
void WINAPI extSmackBufferNewPalette(HSMACKBUF hsb, void *f1, unsigned int f2)
{
	OutTraceDW("SmackBufferNewPalette: h=%#x f1=%#x f2=%d\n", hsb, f1, f2);
	return (*pSmackBufferNewPalette)(hsb, f1, f2);
}
