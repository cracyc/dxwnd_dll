...
void gShowHideTaskBar(BOOL bHide /*=FALSE*/)
{
	...
	if(GetVersionEx(&osinfo)) {
		isWinXP = ((osinfo.dwMajorVersion == 5) && (osinfo.dwMinorVersion == 1));
		isWin7 = ((osinfo.dwMajorVersion == 6) && (osinfo.dwMinorVersion == 1));
	}
	OutTraceDW("gShowHideTaskBar: hide=%d isWinXP=%d isWin7=%d\n", bHide, isWinXP, isWin7);
	...
	if(isWinXP || isWin7) {
		...
		iBottom = rectWorkArea.bottom;
		(*pGetWindowRect)(pWnd, &rectTaskBar);
	}
	if( bHide ){
		// creates the semaphore to prevent DxWnd recovery of the Windows toolbar
		hMutex=CreateSemaphore(NULL, 0, 1, "Global\\DxWnd TOOLBAR");
		if(hMutex) {
			OutTraceDW("HideTaskBar: CreateSemaphore handle=%#x\n", hMutex);
		}
		else {
			OutTraceE("HideTaskBar: CreateSemaphore ERROR err=%d\n", GetLastError());
		}
		...
		if(isWin7) (*pShowWindow)(pStart, SW_HIDE);
	}
	else{
		// Code to Show the System Task Bar
		if(isWinXP || isWin7) {
			rectWorkArea.bottom = iBottom;
			(*pSystemParametersInfoA)(SPI_SETWORKAREA, 0, (LPVOID)&rectWorkArea, 0);
		}
		(*pShowWindow)(pWnd, SW_SHOW);
		if(isWin7) (*pShowWindow)(pStart, SW_SHOW);

		OutTraceDW("HideTaskBar: release handle=%#x\n", hMutex);
		CloseHandle(hMutex);
	}
}
...
LRESULT CALLBACK dw_Hider_Message_Handler(HWND hwnd, UINT umsg, WPARAM wparam, LPARAM lparam)
{
	...
	// ExplainMsg("HIDER", TRUE, hwnd, umsg, wparam, lparam);
	...
	if(hwnd == wHider){
		...
		switch(umsg){
			...
			case WM_WINDOWPOSCHANGED:
				...
				break;
			case WM_ACTIVATEAPP:
				// v2.05.68: tentative handling of sync between main win status and wHider
				(*pShowWindow)(wHider, wparam ?  SW_SHOW : SW_HIDE);
				break;
			default:
				break;
		}
...
void dxwCore::HideDesktop(HWND hwnd)
{
	...
	// v2.05.77 fix: considering HIDETASKBAR in the choice of the blacked area size.
	GetMonitorWorkarea(&wDesktop, (Coordinates != DXW_DESKTOP_FULL) && !(dxw.dwFlags6 & HIDETASKBAR));
	...
	HWND hParent = (*pGetDesktopWindow)(); 
	if(!wHider) {
		// v2.05.78: WS_EX_TOOLWINDOW to delete the hider icon in the Windows toolbar
		wHider=(*pCreateWindowExA)(WS_EX_TOOLWINDOW, "dxwnd:hider", "hider", 0, 0, 0, 0, 0, hParent, NULL, hinst, NULL);
		if(!wHider) {
