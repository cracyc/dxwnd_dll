...
extern void HookDirect3D10(HMODULE);
extern void HookDirect3D10_1(HMODULE);
...
extern void HookBink2W32(HMODULE);
...
extern void HookVB6(HMODULE);
extern void HookDPlayX(HMODULE);
extern void	HookFlash(HMODULE);
extern void	HookShell32(HMODULE);
extern void HookUXTheme(HMODULE);
extern void HookLz32(HMODULE);
extern void HookD3dxof(HMODULE);
...
extern FARPROC Remap_DPlayX_ProcAddress(LPCSTR, HMODULE);
extern FARPROC Remap_Shell32_ProcAddress(LPCSTR, HMODULE);
extern FARPROC Remap_UXTheme_ProcAddress(LPCSTR, HMODULE);
extern FARPROC Remap_Lz32_ProcAddress(LPCSTR, HMODULE);
extern FARPROC Remap_D3dxof_ProcAddress(LPCSTR, HMODULE);
...
typedef enum {
	HOOK_IAT_CANDIDATE = 0,		// the entry was not hooked yet and should be IAT-patched
	HOOK_HOT_CANDIDATE,			// the entry was not hooked yet and should be IAT-patched or hot-patched if requested
	HOOK_HOT_REQUIRED,			// the entry was not hooked yet and should be hot-patched 
	HOOK_IAT_LINKED,			// the entry was hooked with IAT-patch
	HOOK_HOT_LINKED,			// the entry was hooked with hot-patch
	HOOK_HOT_FORBIDDEN			// the entry was not hooked yet but must be IAT-patched in any case, no HOT-patch is allowed
} HookEntry_Status;
...
extern FARPROC RemapLibraryOpt(LPCSTR, HMODULE, HookEntryEx_Type *);
