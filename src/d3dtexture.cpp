...
unsigned int HashBuffer(BYTE *buf, int len)
{
...
char *ExplainD3DSurfaceFormat(DWORD dwFormat)
{
	...
	if(dwFormat & 0xFF00){
		memcpy(sFormat, &dwFormat, sizeof(DWORD));
		for(int i=0; i<4; i++) if((sFormat[i]<' ') || (sFormat[i]>'Z')) sFormat[i]='.';
		sFormat[4]=0;
		s = sFormat;
	}
	return s;
...
int dxGetFormatColorDepth(D3DFORMAT Format)
{
	int iColorDepth = 0;
	switch (Format){
...
#define FormatColorBytes(Format) (dxGetFormatColorDepth(Format) >> 3)
...
void dxSetDDSPixelFormat(D3DFORMAT Format, DDS_PIXELFORMAT *pf)
{
	switch (Format){
		case D3DFMT_X8R8G8B8: 
		case D3DFMT_A8R8G8B8: 
...
void D3DTextureDump(D3DSURFACE_DESC Desc, D3DLOCKED_RECT LockedRect)
{
	...
	if(!hash) {
		OutTraceDW("%s: hash=NULL\n", ApiRef); 
		return; // almost certainly, an empty black surface!
	}	

	OutDebugDW("%s: hash=%08.8X size=(%dx%d) format=%#x(%s)\n", 
		ApiRef, hash, Desc.Width, Desc.Height, Desc.Format, ExplainD3DSurfaceFormat(Desc.Format));
		...
		switch(iTextureFileFormat){
			case FORMAT_RAW:
				...
				if(fwrite((BYTE *)LockedRect.pBits, TextureSize(Desc, LockedRect), 1, hf)!=1)
						OutTraceE("%s: fwrite ERROR err=%d\n", ApiRef, GetLastError());
#else
			...
			case FORMAT_DDS: {
				...
				dxSetDDSPixelFormat(Desc.Format, &ddsh.ddspf);
#ifndef DXW_NOTRACES
				...
				if(fwrite((BYTE *)LockedRect.pBits, TextureSize(Desc, LockedRect), 1, hf)!=1)
						OutTraceE("%s: fwrite ERROR err=%d\n", ApiRef, GetLastError());
#else
				fwrite((BYTE *)&ddsh, sizeof(ddsh), 1, hf);
				fwrite((BYTE *)LockedRect.pBits, TextureSize(Desc, LockedRect), 1, hf);
#endif // DXW_NOTRACES
				}
				break;
...
void D3DTextureHack(D3DSURFACE_DESC Desc, D3DLOCKED_RECT LockedRect)
{
	...
	while(TRUE){ // fake loop
...
#else
				fread(magic, 4, 1, hf);
				fread((BYTE *)&ddsh, sizeof(ddsh), 1, hf);
				fread((BYTE *)LockedRect.pBits, TextureSize(Desc, LockedRect), 1, hf);
#endif // DXW_NOTRACES
			}
			break;
		...
		case FORMAT_BMP:
			...
			int iBitCount = dxGetFormatColorDepth(Desc.Format);
			if(iBitCount == 0){
				OutTraceDW("%s: unsupported format=%#x\n", ApiRef, Desc.Format); 
				break;
			}
				...
				switch (Desc.Format){
					...
					case D3DFMT_DXT1:
						...
						if(pbi.bV4Height < 0){
							...
							for(int y=0; y<(int)Desc.Height; y+=4){
								for(int x=0; x<(int)Desc.Width; x+=4){
									OutTraceDW("%s: Compressing line=%d row=%d\n", ApiRef, y, x);
									stb_compress_dxt_block(p, fb, FALSE, STB_DXT_NORMAL);
									p += (16 * sizeof(DWORD) / 8);
									OutTraceDW("%s: Compression done\n", ApiRef);
									fb += 4 * 2;
								}
							}
							free(fb);
...
{
	ApiName("D3DTextureHighlight");
	OutDebugDW("%s: size=(%dx%d) format=%d(%s)\n", 
		ApiRef, Desc.Width, Desc.Height, Desc.Format, ExplainD3DSurfaceFormat(Desc.Format));

	if(!dxw.VerifyTextureLimits(Desc.Width, Desc.Height)){
		OutTraceOGL("%s: SKIP texture\n", ApiRef);
		return;
	}		
	...
	switch (Desc.Format){
		...
		case D3DFMT_X4R4G4B4: 
		case D3DFMT_A4R4G4B4:
			if(!mask16) mask16 = 0x0FFF;
			// fall through
		case D3DFMT_R5G6B5:
			if(!mask16) mask16 = 0xFFFF;
			// fall through
		case D3DFMT_X1R5G5B5:
		case D3DFMT_A1R5G5B5:
			if(!mask16) mask16 = 0x8FFF;
			// fall through
			{
				WORD *p;
				WORD color;
				color=(WORD)(rand() & mask16);
				for(UINT y=0; y<Desc.Height; y++){
					p = (WORD *)LockedRect.pBits + ((y * LockedRect.Pitch) >> 1);
					for(UINT x=0; x<Desc.Width; x++) *(p+x) = color;
				}
