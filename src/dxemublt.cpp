	...
	LPVOID lpsurface, LPRECT lpsrcrect, LONG srcpitch, DWORD srcGBitMask)
{
	...
	dest = dest0;
	src8 = src08;
	OutTrace("%s: InterlaceLines=%d\n", ApiRef, InterlaceLines);
	if(InterlaceLines == 1){
		for(y = 0; y < h; y += 2){
			for(x = 0; x < w; x ++){
				*(dest ++) = PaletteEntries[*(src8 ++)];
	...
	LPVOID lpsurface, LPRECT lpsrcrect, LONG srcpitch, DWORD srcGBitMask)
{
	...
	OutTrace("%s: SCALE2X *** dest=%#x src=%#x\n", ApiRef, dest, tmpbuf);
	...
	LPVOID lpsurface, LPRECT lpsrcrect, LONG srcpitch, DWORD srcGBitMask)
{
	...
	OutTrace("%s: SCALE3X *** dest=%#x src=%#x\n", ApiRef, dest, tmpbuf);
	...
	LPVOID lpsurface, LPRECT lpsrcrect, LONG srcpitch, DWORD srcGBitMask)
{
	...
	OutTrace("%s: SCALE2K *** dest=%#x src=%#x\n", ApiRef, dest, tmpbuf);
	(*pScale)(202, dest, destpitch, tmpbuf, w*sizeof(DWORD), 4, w, h);
	free(tmpbuf);
}
	...
	LPVOID lpsurface, LPRECT lpsrcrect, LONG srcpitch, DWORD srcGBitMask)
{
	...
	OutTrace("%s: SCALE3K *** dest=%#x src=%#x\n", ApiRef, dest, tmpbuf);
	(*pScale)(303, dest, destpitch, tmpbuf, w*sizeof(DWORD), 4, w, h);
	free(tmpbuf);
}
	...
	LPVOID lpsurface, LPRECT lpsrcrect, LONG srcpitch, DWORD srcGBitMask)
{
	...
	OutTrace("%s: SCALE4K *** dest=%#x src=%#x\n", ApiRef, dest, tmpbuf);
...
{
	ApiName("EmuBlt_Null");
	char *sMsg="undefined color depth";
	OutTraceE("%s: %s\n", ApiRef, sMsg);
...
#define colorFactor 4

static void Dither32(Pixel *p, unsigned int w, unsigned int h)
{
#define pixels(x, y) p[y*w + x]
	unsigned int x, y;
	...
	for(y = 0; y < h - 1; y ++){
		for(x = 1; x < w - 1; x ++){
			...
			pixels(x,y).red   =  NewR;
			pixels(x,y).green =  NewG;
			pixels(x,y).blue  =  NewB;

			int ErrorR = pixel.red   -   NewR;
			int ErrorG = pixel.green -   NewG;
			int ErrorB = pixel.blue  -   NewB;
/*
			pixels(x+1,y).red   = pixels(x+1,y).red   + ((ErrorR * 7) / 16);
			pixels(x+1,y).green = pixels(x+1,y).green + ((ErrorG * 7) / 16);
			pixels(x+1,y).blue  = pixels(x+1,y).blue  + ((ErrorB *  7) / 16);

			pixels(x-1,y+1).red    = pixels(x-1,y+1).red   + ((ErrorR * 3) / 16);
			pixels(x-1,y+1).green  = pixels(x-1,y+1).green + ((ErrorG * 3) / 16);
			pixels(x-1,y+1).blue   = pixels(x-1,y+1).blue  + ((ErrorB * 3) / 16);

			pixels(x,y+1).red    = pixels(x,y+1).red   + ((ErrorR * 5) / 16);
			pixels(x,y+1).green  = pixels(x,y+1).green + ((ErrorG * 5) / 16);
			pixels(x,y+1).blue   = pixels(x,y+1).blue  + ((ErrorB * 5) / 16);

			pixels(x+1,y+1).red    = pixels(x+1,y+1).red   + (ErrorR / 16);
			pixels(x+1,y+1).green  = pixels(x+1,y+1).green + (ErrorG / 16);
			pixels(x+1,y+1).blue   = pixels(x+1,y+1).blue  + (ErrorB / 16);
*/
			pixels(x+1,y).red   = pixels(x+1,y).red   + ((ErrorR * 7) >> 4);
			pixels(x+1,y).green = pixels(x+1,y).green + ((ErrorG * 7) >> 4);
			pixels(x+1,y).blue  = pixels(x+1,y).blue  + ((ErrorB *  7) >> 4);

			pixels(x-1,y+1).red    = pixels(x-1,y+1).red   + ((ErrorR * 3) >> 4);
			pixels(x-1,y+1).green  = pixels(x-1,y+1).green + ((ErrorG * 3) >> 4);
			pixels(x-1,y+1).blue   = pixels(x-1,y+1).blue  + ((ErrorB * 3) >> 4);

			pixels(x,y+1).red    = pixels(x,y+1).red   + ((ErrorR * 5) >> 4);
			pixels(x,y+1).green  = pixels(x,y+1).green + ((ErrorG * 5) >> 4);
			pixels(x,y+1).blue   = pixels(x,y+1).blue  + ((ErrorB * 5) >> 4);

			pixels(x+1,y+1).red    = pixels(x+1,y+1).red   + (ErrorR >> 4);
			pixels(x+1,y+1).green  = pixels(x+1,y+1).green + (ErrorG >> 4);
			pixels(x+1,y+1).blue   = pixels(x+1,y+1).blue  + (ErrorB >> 4);
		}
	}
}

static void WINAPI Dither_8_to_32(
	LPVOID lpdest, LPRECT lpdestrect, LONG destpitch, DWORD dstGBitMask,
	LPVOID lpsurface, LPRECT lpsrcrect, LONG srcpitch, DWORD srcGBitMask)
{
	EmuBlt_8_to_32(lpdest, lpdestrect, destpitch, dstGBitMask, lpsurface, lpsrcrect, srcpitch, srcGBitMask);
	Dither32((Pixel *)lpdest, lpdestrect->right, lpdestrect->bottom);
}

static void WINAPI Dither_16_to_32(
	LPVOID lpdest, LPRECT lpdestrect, LONG destpitch, DWORD dstGBitMask,
	LPVOID lpsurface, LPRECT lpsrcrect, LONG srcpitch, DWORD srcGBitMask)
{
	EmuBlt_16_to_32(lpdest, lpdestrect, destpitch, dstGBitMask, lpsurface, lpsrcrect, srcpitch, srcGBitMask);
	Dither32((Pixel *)lpdest, lpdestrect->right, lpdestrect->bottom);
}

static void WINAPI Dither_32_to_32(
	LPVOID lpdest, LPRECT lpdestrect, LONG destpitch, DWORD dstGBitMask,
	LPVOID lpsurface, LPRECT lpsrcrect, LONG srcpitch, DWORD srcGBitMask)
{
	EmuBlt_32_to_32(lpdest, lpdestrect, destpitch, dstGBitMask, lpsurface, lpsrcrect, srcpitch, srcGBitMask);
	Dither32((Pixel *)lpdest, lpdestrect->right, lpdestrect->bottom);
}
...
void SetBltTransformations()
{
	...
	if(pColorConversion != ColorConversionEmulated) {
		OutTraceDW("%s: colors=%d->%d renderer=%d(\"%s\")\n",
			ApiRef,
	...
	switch (dxw.ActualPixelFormat.dwRGBBitCount){
		case 32:
			...
			switch(dxw.VirtualPixelFormat.dwRGBBitCount){
				case 8:
					switch(dxw.FilterId){
						case DXW_FILTER_NONE: pEmuBlt=EmuBlt_8_to_32; break;
						case DXW_FILTER_BILX2: pEmuBlt=BilinearBlt_8_to_32; break;
						case DXW_FILTER_HQX2: pEmuBlt=HQ_X2_Blt_8_to_32; break;
						case DXW_FILTER_HQX3: pEmuBlt=HQ_X3_Blt_8_to_32; break;
						case DXW_FILTER_HQX4: pEmuBlt=HQ_X4_Blt_8_to_32; break;
						case DXW_FILTER_DEINTERLACE: pEmuBlt=Deinterlace_8_to_32; break;
						case DXW_FILTER_INTERLACE: pEmuBlt=Interlace_8_to_32; break;
						case DXW_FILTER_PIX2: pEmuBlt=PIX_X2_Blt_8_to_32; break;
						case DXW_FILTER_PIX3: pEmuBlt=PIX_X3_Blt_8_to_32; break;
						case DXW_FILTER_PIX4: pEmuBlt=PIX_X4_Blt_8_to_32; break;
						case DXW_FILTER_SCALE2X: pEmuBlt=Scale2X_Blt_8_to_32; break;
						case DXW_FILTER_SCALE3X: pEmuBlt=Scale3X_Blt_8_to_32; break;
						case DXW_FILTER_SCALE4X: pEmuBlt=Scale4X_Blt_8_to_32; break;
						case DXW_FILTER_SCALE2K: pEmuBlt=Scale2K_Blt_8_to_32; break;
						case DXW_FILTER_SCALE3K: pEmuBlt=Scale3K_Blt_8_to_32; break;
						case DXW_FILTER_SCALE4K: pEmuBlt=Scale4K_Blt_8_to_32; break;
						case DXW_FILTER_DITHER: pEmuBlt=Dither_8_to_32; break;
						default:pEmuBlt=EmuBlt_8_to_32; bUnsupported = TRUE; break;
					}
					break;
				case 16: 
					switch(dxw.FilterId){
						case DXW_FILTER_NONE: pEmuBlt=EmuBlt_16_to_32; break;
						case DXW_FILTER_BILX2: pEmuBlt=BilinearBlt_16_to_32; break;
						case DXW_FILTER_HQX2: pEmuBlt=HQ_X2_Blt_16_to_32; break;
						case DXW_FILTER_HQX3: pEmuBlt=HQ_X3_Blt_16_to_32; break;
						case DXW_FILTER_HQX4: pEmuBlt=HQ_X4_Blt_16_to_32; break;
						case DXW_FILTER_DEINTERLACE: pEmuBlt=Deinterlace_16_to_32; break;
						case DXW_FILTER_INTERLACE: pEmuBlt=Interlace_16_to_32; break;
						case DXW_FILTER_PIX2: pEmuBlt=PIX_X2_Blt_16_to_32; break;
						case DXW_FILTER_PIX3: pEmuBlt=PIX_X3_Blt_16_to_32; break;
						case DXW_FILTER_PIX4: pEmuBlt=PIX_X4_Blt_16_to_32; break;
						case DXW_FILTER_SCALE2X: pEmuBlt=Scale2X_Blt_16_to_32; break;
						case DXW_FILTER_SCALE3X: pEmuBlt=Scale3X_Blt_16_to_32; break;
						case DXW_FILTER_SCALE4X: pEmuBlt=Scale4X_Blt_16_to_32; break;
						case DXW_FILTER_SCALE2K: pEmuBlt=Scale2K_Blt_16_to_32; break;
						case DXW_FILTER_SCALE3K: pEmuBlt=Scale3K_Blt_16_to_32; break;
						case DXW_FILTER_SCALE4K: pEmuBlt=Scale4K_Blt_16_to_32; break;
						case DXW_FILTER_DITHER: pEmuBlt=Dither_16_to_32; break;
						default: pEmuBlt=EmuBlt_16_to_32; bUnsupported = TRUE; break;
					}
					break;
				case 24: 
				...
				case 32: 
					switch(dxw.FilterId){
						case DXW_FILTER_NONE: pEmuBlt=EmuBlt_32_to_32; break;
						case DXW_FILTER_BILX2: pEmuBlt=BilinearBlt_32_to_32; break;
						case DXW_FILTER_HQX2: pEmuBlt=HQ_X2_Blt_32_to_32; break;
						case DXW_FILTER_HQX3: pEmuBlt=HQ_X3_Blt_32_to_32; break;
						case DXW_FILTER_HQX4: pEmuBlt=HQ_X4_Blt_32_to_32; break;
						case DXW_FILTER_INTERLACE: pEmuBlt=Interlace_32_to_32; break;
						case DXW_FILTER_PIX2: pEmuBlt=PIX_X2_Blt_32_to_32; break;
						case DXW_FILTER_PIX3: pEmuBlt=PIX_X3_Blt_32_to_32; break;
						case DXW_FILTER_PIX4: pEmuBlt=PIX_X4_Blt_32_to_32; break;
						case DXW_FILTER_SCALE2X: pEmuBlt=Scale2X_Blt_32_to_32; break;
						case DXW_FILTER_SCALE3X: pEmuBlt=Scale3X_Blt_32_to_32; break;
						case DXW_FILTER_SCALE4X: pEmuBlt=Scale4X_Blt_32_to_32; break;
						case DXW_FILTER_SCALE2K: pEmuBlt=Scale2K_Blt_32_to_32; break;
						case DXW_FILTER_SCALE3K: pEmuBlt=Scale3K_Blt_32_to_32; break;
						case DXW_FILTER_SCALE4K: pEmuBlt=Scale4K_Blt_32_to_32; break;
						case DXW_FILTER_DITHER: pEmuBlt=Dither_32_to_32; break;
						default: pEmuBlt=EmuBlt_32_to_32; bUnsupported = TRUE; break;
					}
					break;
				default:
					bUnsupported = TRUE;
		...
		case 16:
			...
			switch(dxw.VirtualPixelFormat.dwRGBBitCount){
				case 8:
					switch(dxw.FilterId){
						case DXW_FILTER_NONE: pEmuBlt=EmuBlt_8_to_16; break;
						case DXW_FILTER_BILX2: pEmuBlt=BilinearBlt_8_to_16; break;
						...
						case DXW_FILTER_INTERLACE: pEmuBlt=Interlace_8_to_16; break;
						case DXW_FILTER_PIX2: pEmuBlt=PIX_X2_Blt_8_to_16; break;
						case DXW_FILTER_PIX3: pEmuBlt=PIX_X3_Blt_8_to_16; break;
						case DXW_FILTER_PIX4: pEmuBlt=PIX_X4_Blt_8_to_16; break;
						case DXW_FILTER_SCALE2X: pEmuBlt=Scale2X_Blt_8_to_16; break;
						case DXW_FILTER_SCALE3X: pEmuBlt=Scale3X_Blt_8_to_16; break;
						case DXW_FILTER_SCALE4X: pEmuBlt=Scale4X_Blt_8_to_16; break;
						default: pEmuBlt=EmuBlt_8_to_16; bUnsupported = TRUE; break;
					}
					break;
				case 16:
					switch(dxw.FilterId){
						case DXW_FILTER_NONE: pEmuBlt=EmuBlt_16_to_16; break;
						case DXW_FILTER_BILX2: pEmuBlt=BilinearBlt_16_to_16; break;
						...
						case DXW_FILTER_PIX2: pEmuBlt=PIX_X2_Blt_16_to_16; break;
						case DXW_FILTER_PIX3: pEmuBlt=PIX_X3_Blt_16_to_16; break;
						case DXW_FILTER_PIX4: pEmuBlt=PIX_X4_Blt_16_to_16; break;
						case DXW_FILTER_SCALE2X: pEmuBlt=Scale2X_Blt_16_to_16; break;
						case DXW_FILTER_SCALE3X: pEmuBlt=Scale3X_Blt_16_to_16; break;
						case DXW_FILTER_SCALE4X: pEmuBlt=Scale4X_Blt_16_to_16; break;
						case DXW_FILTER_SCALE2K: pEmuBlt=Scale2K_Blt_16_to_16; break;
						case DXW_FILTER_SCALE3K: pEmuBlt=Scale3K_Blt_16_to_16; break;
						case DXW_FILTER_SCALE4K: pEmuBlt=Scale4K_Blt_16_to_16; break;
						default: pEmuBlt=EmuBlt_16_to_16; bUnsupported = TRUE; break;
					}
					break;
				case 24: 
	...
	OutTraceDW("%s: colors=%d->%d renderer=%d(\"%s\") filter=%d(%s) scaling=(%d,%d) supported=%s\n",
		ApiRef,
		dxw.VirtualPixelFormat.dwRGBBitCount,
		dxw.ActualPixelFormat.dwRGBBitCount,
		dxw.Renderer->id, dxw.Renderer->name,
