...
// v2.04.79: fix - added logic to avoid MAXROLEKOUNT limitaton when the surface is already on
// the current surface stack: in this case there is no addition but a simple replacement that
// doesn't need boudary or limitation checking.
...
static char *sRole(USHORT role)
{
	...
	switch (role){
		...
		case SURFACE_ROLE_OVERLAY: s="(OVERLAY)"; break;
		case SURFACE_ROLE_OVERLAYBACK: s="(OVERBACK)"; break;
...
static void DumpSurfaceList(SurfaceDB_Type *SurfaceDB)
{
	...
	for (int i=0;i<DDSQLEN;i++) {
		...
		OutTrace("--- SURFACELIST DUMP: i=%d lpssd=%#x%s ref=%d vers=%d caps=%#x(%s)\n", i, 
			...
			SurfaceDB[i].dwCaps, ExplainDDSCaps(SurfaceDB[i].dwCaps, sCaps, 256));
	}
}
#endif
...
void dxwSStack::ClearSurfaceList()
{
	...
	for (i=0; i<DDSQLEN; i++) {
		...
		switch(SurfaceDB[i].uRole){
			...
			case SURFACE_ROLE_3DREF: lpDDS3DRef = SurfaceDB[i].lpdds; break;
		}
	}
...
{
	int i;
	// v2.04.35: when unref, clear cached references
	// clearing lpDDSPrimary avoids the risk of error during auto-refresh operations - fixes "M1 Tank Platoon 2"
	if(ps == lpDDSPrimary) lpDDSPrimary = NULL;
	if(ps == lpDDSBackBuffer) lpDDSBackBuffer = NULL;
	if(ps == lpDDSZBuffer) lpDDSZBuffer = NULL;
	if(ps == lpDDS3DRef) lpDDS3DRef = NULL;
	// look for entry
	for (i=0;i<DDSQLEN;i++) 
...
void dxwSStack::PushSurface(LPDIRECTDRAWSURFACE ps, USHORT role, USHORT version, DWORD dwCaps, DWORD codec)
{
	int i, iMax;
	SurfaceDB_Type *e;
	int iRoleKount = 0;
	...
	for (i=0;i<DDSQLEN;i++) {
		e = &SurfaceDB[i];
		if (e->uRole == role) {
			iRoleKount++;	// count how many matching entries
			iMax = i; // v2.05.58: set index to last matching entry (same role!!)
		}
		if (e->lpdds == ps) { // got matching entry
			bTarget = TRUE;
			break;
		}
		if (e->lpdds == (DWORD)0) break; // got end of the list
	}
#ifdef DXW_SURFACE_STACK_TRACING
	OutTraceSDB(">>> SURFACELIST MARK: role=%d iRoleKount=%d\n", role, iRoleKount);
#endif
	if(!bTarget){
		if(iRoleKount >= MAXROLEKOUNT){
			//max allowed entries per role type reached, do not add but overwrite ....
			i = iMax;
			e = &SurfaceDB[i];
#ifdef DXW_SURFACE_STACK_TRACING
			OutTraceSDB(">>> SURFACELIST MARK: OVERRIDE lpdds=%#x%s vers=%d\n", e->lpdds, sRole(e->uRole), e->uVersion);
#endif
		}
		if(i == DDSQLEN) {
	...
	// else override old entry with same lpdds value
	e->lpdds=ps;
	e->uRole = role;
	e->uRef = TRUE;
	e->uVersion = version;
	...
	switch(e->uRole){
		...
		case SURFACE_ROLE_3DREF: lpDDS3DRef = e->lpdds; break;
		case SURFACE_ROLE_OVERLAY: lpDDSOverlay = e->lpdds; break;
		case SURFACE_ROLE_OVERLAYBACK: lpDDSOverlayBack = e->lpdds; break;
	}
#ifdef DXW_SURFACE_STACK_TRACING
	DumpSurfaceList(SurfaceDB);
#endif
...
{
	PushSurface(ps, SURFACE_ROLE_PRIMARY, (USHORT)version, dwCaps, 0);
}

void dxwSStack::PushBackBufferSurface(LPDIRECTDRAWSURFACE ps, int version, DWORD dwCaps)
{
	PushSurface(ps, SURFACE_ROLE_BACKBUFFER, (USHORT)version, dwCaps, 0);
}

void dxwSStack::PushZBufferSurface(LPDIRECTDRAWSURFACE ps, int version, DWORD dwCaps)
{
	PushSurface(ps, SURFACE_ROLE_ZBUFFER, (USHORT)version, dwCaps, 0);
}

void dxwSStack::Push3DRefSurface(LPDIRECTDRAWSURFACE ps, int version, DWORD dwCaps)
{
	PushSurface(ps, SURFACE_ROLE_3DREF, (USHORT)version, dwCaps, 0);
}

void dxwSStack::PushOverlaySurface(LPDIRECTDRAWSURFACE ps, int version, DWORD dwCaps, DWORD fourcc)
{
	PushSurface(ps, SURFACE_ROLE_OVERLAY, (USHORT)version, dwCaps, fourcc);
}

void dxwSStack::PushOverlayBackSurface(LPDIRECTDRAWSURFACE ps, int version, DWORD dwCaps, DWORD fourcc)
{
	PushSurface(ps, SURFACE_ROLE_OVERLAYBACK, (USHORT)version, dwCaps, fourcc);
}
...
DWORD dxwSStack::DuplicateSurface(LPDIRECTDRAWSURFACE psfrom, LPDIRECTDRAWSURFACE psto, int version)
{
...
#endif
	// search for source or empty slot and set surface role
	for (i=0;i<DDSQLEN;i++) {
		e=&SurfaceDB[i];
		if (e->lpdds==psfrom) {
			role = e->uRole;
	...
	// search for a possible existing target
	bTarget = FALSE;
	for (i=0;i<DDSQLEN;i++) {
		e=&SurfaceDB[i];
		if (e->lpdds==(DWORD)0) break;
		if (e->lpdds==psto) {
			bTarget = TRUE;
#ifdef DXW_SURFACE_STACK_TRACING
			OutTraceSDB("--- REPLACE ENTRY@%d: to=%#x\n", i, psto);
#endif
			break;
		}
	}

	// search again, but considering role count as well
	for (i=0;i<DDSQLEN;i++) {
		e=&SurfaceDB[i];
		if (e->uRole == role) iRoleKount++;	// count how many matching entries
		if ((e->lpdds==psfrom) || (e->lpdds==(DWORD)0) || (iRoleKount==MAXROLEKOUNT)) break; // got matching entry or end of the list or max count per role
	}
	// search for destination or empty slot
	if((iRoleKount < MAXROLEKOUNT) || bTarget) {
		for (i=0;i<DDSQLEN;i++) {
			e=&SurfaceDB[i];
		...
		//max allowed entries per role type reached, do not add but overwrite ....
#ifdef DXW_SURFACE_STACK_TRACING
		OutTraceSDB(">>> SURFACELIST DUPL: OVERRIDE lpdds=%#x%s vers=%d\n", e->lpdds, sRole(e->uRole), e->uVersion);
#endif
	}
	if(i == DDSQLEN) {
#ifdef DXW_SURFACE_STACK_TRACING
		OutTraceSDB(">>> SURFACELIST FULL\n");
#endif
		for(int j=0;j<DDSQLEN-1;j++) SurfaceDB[j]=SurfaceDB[j+1]; // scale down the whole stack one entry
		e=&SurfaceDB[DDSQLEN-1];
	}
...
BOOL dxwSStack::IsInPrimaryFlipchain(LPDIRECTDRAWSURFACE ps)
{
	int i;
	// treat NULL surface ptr as a non primary
	if(!ps) return FALSE;
	for (i=0;i<DDSQLEN;i++) {
		if (SurfaceDB[i].lpdds==0) return FALSE;
		if (SurfaceDB[i].lpdds==ps) {
			USHORT role = SurfaceDB[i].uRole;
			return (role == SURFACE_ROLE_BACKBUFFER) || (role == SURFACE_ROLE_PRIMARY);
		}
	}
	return FALSE;
}
...
BOOL dxwSStack::IsAnOverlayPrimSurface(LPDIRECTDRAWSURFACE ps)
{
	int i;
	// treat NULL surface ptr as a non primary
	if(!ps) return FALSE;
	for (i=0;i<DDSQLEN;i++) {
		if (SurfaceDB[i].lpdds==0) return FALSE;
		if (SurfaceDB[i].lpdds==ps) return (SurfaceDB[i].uRole == SURFACE_ROLE_OVERLAY);
	}
	return FALSE;
}

BOOL dxwSStack::IsAnOverlayBackSurface(LPDIRECTDRAWSURFACE ps)
{
	int i;
	// treat NULL surface ptr as a non primary
	if(!ps) return FALSE;
	for (i=0;i<DDSQLEN;i++) {
		if (SurfaceDB[i].lpdds==0) return FALSE;
		if (SurfaceDB[i].lpdds==ps) return (SurfaceDB[i].uRole == SURFACE_ROLE_OVERLAYBACK);
	}
	return FALSE;
}

BOOL dxwSStack::IsInOverlayFlipchain(LPDIRECTDRAWSURFACE ps)
{
	int i;
	// treat NULL surface ptr as a non primary
	if(!ps) return FALSE;
	for (i=0;i<DDSQLEN;i++) {
		...
		if (SurfaceDB[i].lpdds==ps) {
			USHORT role = SurfaceDB[i].uRole;
			return (role == SURFACE_ROLE_OVERLAY) || (role == SURFACE_ROLE_OVERLAYBACK);
			}
	}
	return FALSE;
}
...
{
	// v2.04.53: fix for "Rival Realms" that had a cached NULL primary surface
	if(!lpDDSPrimary) lpDDSPrimary = GetSurfaceByRole(SURFACE_ROLE_PRIMARY);
	return lpDDSPrimary;
}

LPDIRECTDRAWSURFACE dxwSStack::GetBackBufferSurface(void)
{	
	if(!lpDDSBackBuffer) lpDDSBackBuffer = GetSurfaceByRole(SURFACE_ROLE_BACKBUFFER);
	return lpDDSBackBuffer;
}

LPDIRECTDRAWSURFACE dxwSStack::GetZBufferSurface(void)
{
	if(!lpDDSZBuffer) lpDDSZBuffer = GetSurfaceByRole(SURFACE_ROLE_ZBUFFER);
	return lpDDSZBuffer;
}

LPDIRECTDRAWSURFACE dxwSStack::GetOverlaySurface(void)
{
	if(!lpDDSOverlay) lpDDSOverlay = GetSurfaceByRole(SURFACE_ROLE_OVERLAY);
	return lpDDSOverlay;
}

LPDIRECTDRAWSURFACE dxwSStack::GetOverlayBackSurface(void)
{
	if(!lpDDSOverlayBack) lpDDSOverlayBack = GetSurfaceByRole(SURFACE_ROLE_OVERLAYBACK);
	return lpDDSOverlayBack;
}
...
#ifdef DXW_SURFACE_STACK_TRACING
			char sCaps[256];
			OutTraceSDB(">>> GETCAPS: lpdds=%#x caps=%#x(%s)\n", ps, SurfaceDB[i].dwCaps, ExplainDDSCaps(SurfaceDB[i].dwCaps, sCaps, 256));
#endif		
...
DWORD dxwSStack::GetFourCC(LPDIRECTDRAWSURFACE ps)
{
	for (int i=0;i<DDSQLEN;i++) {
		if (SurfaceDB[i].lpdds==0) return 0;
		if (SurfaceDB[i].lpdds==ps) {
#ifdef DXW_SURFACE_STACK_TRACING
			OutTraceSDB(">>> GETCAPS: lpdds=%#x FourCC=%#x(%s)\n", ps, SurfaceDB[i].dwFourCC, sFourCC(SurfaceDB[i].dwFourCC));
#endif		
			return SurfaceDB[i].dwFourCC;
		}
	}
	return 0;
}
