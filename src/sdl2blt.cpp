...
HRESULT SDL2SyncBlitter(int dxversion, Blt_Type pBlt, char *api, LPDIRECTDRAWSURFACE lpdds, LPRECT lpdestrect,
	LPDIRECTDRAWSURFACE s, LPRECT lpsrcrect, DWORD dwflags, LPDDBLTFX lpddbltfx, BOOL isFlipping)
{
	...
	if(!bInitialized){
		...
		if(!gSDL_Window){
			OutTraceE("%s: ERROR in SDL_CreateWindowFrom @%d\n", api, __LINE__);
	...
	if(!gSDL_VideoSurface){
		OutTraceE("%s: ERROR in SDL_GetWindowSurface @%d\n", api, __LINE__);
		(*pUnlockMethod(dxversion))(s, NULL);
		return DD_FALSE;
	}
	...
	if(ddsd.ddpfPixelFormat.dwFourCC) pixels = dxwConvertFourCC(&ddsd, ddsd.ddpfPixelFormat.dwRGBBitCount);
	if(!pixels){
	...
	if(!SDL_DDPrimary){
		OutTraceE("%s: ERROR in SDL_CreateRGBSurfaceFrom @%d\n", api, __LINE__);
		(*pUnlockMethod(dxversion))(s, NULL);
		return DD_FALSE;
	}
	...
	if(res=(*pSDL_SoftStretch)(SDL_Converted, 
		...
		OutTraceE("%s: SDL_SoftStretch ERROR err=%#x @%d\n", api, res, __LINE__);
		(*pSDL_FreeSurface)(SDL_DDPrimary);
		return DD_FALSE;
	}
