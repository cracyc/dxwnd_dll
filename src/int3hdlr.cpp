...
{
	ApiName("Int3Handler");
	OutTraceDW("%s: [EAX] SteppingId=%#x Model=%#x Family=%#x ProcType=%#x ExtModel=%#x ExtFamily=%#x\n", ApiRef, 
		(dwEAX>> 0)&0xF, 
		(dwEAX>> 4)&0xF, 
		(dwEAX>> 8)&0xF, 
		(dwEAX>>12)&0x3, 
		(dwEAX>>16)&0xF, 
		(dwEAX>>20)&0xF);
	OutTraceDW("%s: [EBX] BrandIndex=%#x CLFLUSHLineSize=%#x InitialAPICID=%#x\n", ApiRef, 
		(dwEBX>> 0)&0xFF, 
		(dwEBX>> 8)&0xFF, 
		(dwEBX>>24)&0xFF);
	OutTraceDW("Int3Handler: [ECX] SSE3=%#x Monitor=%#x DS-CPL=%#x EST=%#x TM2=%#x CNTX-ID=%#x\n", 
		...
		(dwECX>> 8)&0x01, 
		(dwECX>>10)&0x01); 
	OutTraceDW("%s: [EDX] FPU=%#x VME=%#x DE=%#x PSE=%#x TSC=%#x MSR=%#x PAE=%#x MCE=%#x "
...
static char *sCPUType(DWORD dwEAX)
{
	char *p;
	switch (dwEAX & 0x0FFFFFFF){
		case 0x00: p="Earlier Intel 486";
		case 0x01: p="Later Intel 486, Pentium";
		case 0x02: p="Pentium Pro, Pentium II, Celeron, Pentium 4, Xeon, Pentium M";
		case 0x03: p="Pentium III";
		case 0x05: p="Pentium 4 with Hyper-Threading, Pentium D(8xx)";
		case 0x06: p="Pentium D (9xx)";
		case 0x0A: p="Core Duo, Core 2 Duo, Xeon 3000, 5000 series, Xeon 5200, 5400 series, Atom";
		case 0x0B: p="Nehalem-based proc.";
		case 0x0D: p="Core 2 Duo 8000 series, Ivy Bridge-based proc.";
		case 0x16: p="Skylake-based proc.";
		case 0x17: p="System-On-Chip Vendor";
		default: p="undetected";
	}
	return p;
}
			...
			case 0x90: // Replace RTDSC
				{	
				   ...
				   __asm {
					...
					OutTraceDW("%s: INT3 at=%#x tick=%#x RDTSC=%#x:%#x\n", ApiRef,
			...
			case 0xCC: // Replace CPUID
				{
					...
					OutTraceDW("%s: eax=%#x ebx=%#x ecx=%#x edx=%#x\n", ApiRef, dwEAX, dwEBX, dwECX, dwEDX); 
					switch(dwEAX0){
						case 0x00000000: // CPU Vendor-Id 
							OutTraceDW("%s: vendor=\"%04.4s%04.4s%04.4s\" MaxCPUIDInput=%d\n", 
								ApiRef, (char *)&dwEBX, (char *)&dwEDX, (char *)&dwECX, dwEAX);
							if(dxw.dwDFlags & DUMPCPUID) break;
							if(dxw.dwFlags17 & FAKEPENTIUM3) {
								dwEBX = *(DWORD *)"Genu";
								dwEDX = *(DWORD *)"ineI";
								dwECX = *(DWORD *)"ntel";
								//dwEAX = 1;
							...
							OutTraceDW("%s: FAKE vendor=\"%04.4s%04.4s%04.4s\" MaxCPUIDInput=%d\n", 
								ApiRef, (char *)&dwEBX, (char *)&dwEDX, (char *)&dwECX, dwEAX);
							break;
						case 0x00000001: // CPU features
							LogCPUFeatures(dwEAX, dwEBX, dwECX, dwEDX);
							if(dxw.dwFlags17 & FAKEPENTIUM3) {
								// set processor model bits 7:4 = 0111
								dwEAX &= 0xFFFFFF0F;
								dwEAX |= 0x00000070;
								// set processor family bits 11:8 to 0110
								dwEAX &= 0xFFFFF0FF;
								dwEAX |= 0x00000600;
								LogCPUFeatures(dwEAX, dwEBX, dwECX, dwEDX);
								break;
							...
							if((dxw.dwDFlags & CPUDISABLEMMX) || (dxw.dwFlags5 & DISABLEMMX)) {
								OutTraceDW("%s: disable MMX & PBE capability\n", ApiRef); 
							...
							if(dxw.dwDFlags & CPUDISABLESSE) {
								OutTraceDW("%s: disable SSE capability\n", ApiRef); 
								dwEDX &= ~(0x1 << 25); // SSE = 25th bit
							}
							if(dxw.dwDFlags & CPUDISABLESSE2) {
								OutTraceDW("%s: disable SSE2 capability\n", ApiRef); 
							...
							if(dxw.dwDFlags & CPUDISABLEPBE) {
								...
								OutTraceDW("%s: disable PBE capability\n", ApiRef); 
								dwEDX &= ~(0x1 << 31);
							}
							//dwEAX = 0;
							//dwEBX = 0;
...
#endif
							break;
						case 0x80000000: // CPU type
							OutTraceDW("%s: CPU type=%#x(%s)\n", ApiRef, dwEAX, sCPUType(dwEAX));
							break;
						case 0x80000001: // CPU extended features
							OutTraceDW("%s: [EDX] mmext=%#x mmx=%#x\n", ApiRef,
								(dwEDX>>22)&0x01, 
								(dwEDX>>23)&0x01); 
							if(dxw.dwDFlags & DUMPCPUID) break;
							if((dxw.dwDFlags & CPUDISABLEMMX) || (dxw.dwFlags5 & DISABLEMMX)) {
								OutTraceDW("%s: disable MMX capability\n", ApiRef); 
								dwEDX &= ~(0x1 << 22); // MMXext = 22th bit
								dwEDX &= ~(0x1 << 23); // MMX = 23th bit
								dwEDX &= ~(0x1 << 30); // 3DNow!ext = 30th bit
								dwEDX &= ~(0x1 << 31); // 3DNow! = 31th bit
							}
							OutTraceDW("%s: FAKE eax=%08.8X ebx=%08.8X ecx=%08.8X edx=%0.8X\n", 
								ApiRef, dwEAX, dwEBX, dwECX, dwEDX);
							break;
						default: 
							break; 
