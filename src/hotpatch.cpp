...
void *HotPatch(void *apiproc, const char *apiname, void *hookproc)
{
	...
	if(DoOnce){
		if ((status = MH_Initialize()) != MH_OK) {
			OutTraceE("%s: MH_Initialize FAILED status=%d\n", ApiRef, status);
	...
	OutTraceH("%s: api=%s addr=%#x hook=%#x\n", ApiRef, apiname, apiproc, hookproc);
	
	if(!strcmp(apiname, "GetProcAddress")) return 0; // do not mess with this one!

	if ((status = MH_CreateHook(apiproc, hookproc, reinterpret_cast<void**>(&pProc))) != MH_OK){
		OutTraceE("%s: ERROR MH_CreateHook FAILED apiname=%s status=%d\n", ApiRef, apiname, status);
        return 0;
	}

	OutTraceH("%s: api=%s addr=%#x->%#x hook=%#x\n", ApiRef, apiname, apiproc, pProc, hookproc);
	return pProc;
#else
	DWORD dwPrevProtect;
	BYTE* patch_address;
