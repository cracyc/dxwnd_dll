...
{
	libname = "binkw32.dll";
	OutTraceDW("HookBinkW32; hmodule=%#x\n", hModule); 
...
void HookBink2W32(HMODULE hModule)
{
	libname = "bink2w32.dll";
	OutTraceDW("HookBinkW32; hmodule=%#x\n", hModule); 
	if(!(dxw.dwFlags5 & HOOKBINKW32)) return;
	InitializeBinkInterface();
	HookLibraryEx(hModule, Hooks, libname);
...
static void InitializeBinkInterface()
{
	...
	while(TRUE){ // fake loop
		...
		OutErrorSYS("Smack interface unknown\n");
...
static void GetBinkVersion(HBINK h)
{
	...
	HMODULE hbinkw32 = (*pLoadLibraryA)(libname);
	if(!hbinkw32){
		OutErrorSYS("GetBinkVersion: LoadLibrary failed, err=%d\n", GetLastError());
		return;
	}

	char path[MAX_PATH+1];
	if (GetModuleFileName(hbinkw32, path, sizeof(path)) == 0){
		OutErrorSYS("GetBinkVersion: GetModuleFileName failed, err=%d\n", GetLastError());
		return;
	}
...
static void BinkSkipMovie(ApiArg, HBINK h)
{
	...
	if(!pBinkGoto){
		HINSTANCE hinst;
		hinst = (*pLoadLibraryA)(libname);
		if(!hinst) {
			OutTraceDW("%s: LoadLibrary %s ERROR err=%d\n", ApiRef, libname, GetLastError());
			return;
		}
	...
	switch(_BinkVersion){
		case BINKVERSION0: Frames = ((BINK0 *)h)->Frames; break;
		case BINKVERSION1: Frames = ((BINK1 *)h)->Frames; break;
		default: Frames = 0; break;
	}
