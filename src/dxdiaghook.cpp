...
HRESULT WINAPI extGetChildContainer(void *, LPCWSTR, IDxDiagContainer **);
...
void HookContainer(IDxDiagContainer **ppInstance)
{
	...
	// IID_IDxDiagContainer::GetChildContainer
	SetHook((void *)(**(DWORD **)ppInstance + 20), extGetChildContainer, (void **)&pGetChildContainer, "GetChildContainer(DxDiag)");
...
HRESULT WINAPI extGetRootContainer(void *th, IDxDiagContainer **ppInstance)
{
	...
	if(res == DD_OK) HookContainer(ppInstance);
...
HRESULT WINAPI extGetChildContainer(void *th, LPCWSTR pwszContainer, IDxDiagContainer **ppInstance)
{
	HRESULT res;
	OutTraceDW("DxDiag::GetChildContainer: container=\"%ls\"\n", pwszContainer);
	res=(*pGetChildContainer)(th, pwszContainer, ppInstance);
	if(res == DD_OK) HookContainer(ppInstance);
	OutTraceDW("DxDiag::GetChildContainer res=%#x\n", res);
	return res;
}
...
HRESULT WINAPI extGetProp(void *th, LPCWSTR pwszPropName, VARIANT *pvarProp)
{
	...
	if (!wcsncmp(L"dwDirectXVersionMajor", pwszPropName, sizeof(L"dwDirectXVersionMajor"))){
		DOUBLE versionMajor = pvarProp->dblVal;
		OutTraceDW("DxDiag::GetProp DirectXVersionMajor=%d\n", versionMajor);
		if(dxw.dwFlags13 & DXVERSIONLIE){
			pvarProp->dblVal=9;
	...
	if (!wcsncmp(L"dwDirectXVersionMinor", pwszPropName, sizeof(L"dwDirectXVersionMinor"))){
		DOUBLE versionMinor = pvarProp->dblVal;
	...
	if (!wcsncmp(L"szDirectXVersionLetter", pwszPropName, sizeof(L"szDirectXVersionLetter"))){
		BSTR versionString = pvarProp->bstrVal;
		WCHAR versionLetter = versionString[0];
		OutTraceDW("DxDiag::GetProp DirectXVersionLetter=\"%lc\"\n", versionLetter);
		if(dxw.dwFlags13 & DXVERSIONLIE){
			pvarProp->bstrVal[0]=(WCHAR)'c';
			OutTraceDW("DxDiag::GetProp fixed szDirectXVersionLetter=\"%lc\"\n", pvarProp->bstrVal[0]);
		}
	}
#endif // DXW_NOTRACES
	return res;
