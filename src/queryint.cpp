...
extern void TextureTransp(LPDIRECTDRAWSURFACE, int);
...
HRESULT WINAPI extQueryInterfaceDX(ApiArg, int dxversion, QueryInterface_Type pQueryInterface, void *lpdds, REFIID riid, LPVOID *obp)
{
	HRESULT res;
	BOOL IsPrim;
	BOOL IsBack;
	...
	OutTraceDDRAW("%s: ver=%d lpdds=%#x%s REFIID=%#x(%s) obp=%#x\n",
		ApiRef, dxversion, lpdds, dxwss.ExplainSurfaceRole((LPDIRECTDRAWSURFACE)lpdds), riid.Data1, ExplainGUID((GUID *)&riid), obp);	

	if(obp == NULL){
		OutTrace("%s: NULL obp ret=DDERR_INVALIDPARAMS\n", ApiRef);
	...
	case 0xF2086B20: // IID_IDirect3DRampDevice
		if(dxw.bHintActive) ShowHint(HINT_RAMPDEV);
		iObjectType=TYPE_OBJECT_D3DDEVICE; iObjectVersion=1; break;
	...
	OutTraceDDRAW("%s: Got interface for %s version %d\n", ApiRef, sLabel, iObjectVersion);
	...
	switch(iObjectType){
		...
		case TYPE_OBJECT_DIRECTDRAW:
			if ((dxw.dwFlags7 & LIMITDDRAW) && (iObjectVersion > (int)dxw.MaxDdrawInterface)) {
				if(obp) *obp = NULL;
				OutTraceDW("%s: DDVersion=%d SUPPRESS lpdds=%#x(%s) REFIID=%#x obp=(NULL) ret=E_NOINTERFACE @%d\n",
		...
		case TYPE_OBJECT_DDRAWSURFACE:
			if ((dxw.dwFlags7 & LIMITDDRAW) && (iObjectVersion > (int)dxw.MaxDdrawInterface)) {
				if(obp) *obp = NULL;
				OutTraceDW("%s: DDSVersion=%d SUPPRESS lpdds=%#x(%s) REFIID=%#x obp=(NULL) ret=E_NOINTERFACE @%d\n",
					ApiRef, iObjectVersion, lpdds, IsPrim?"":"(PRIM)", riid.Data1, __LINE__);
				return(E_NOINTERFACE);
			}
			// v2.05.58: Late Release of backbuffer surface. All this only because we didn't understand
	...
	//if(riid == IID_IDirectDrawSurfaceNew){
	if((dxw.dwFlags16 & NOSURFACENEW) && (riid.Data1 == 0x1bab8e96)){
		OutTraceDW("%s: replacing IID_IDirectDrawSurfaceNew with IID_IDirectDrawSurface7\n", ApiRef);
		res = (*pQueryInterface)(lpdds, IID_IDirectDrawSurface7, obp);
	}
	else
	if((dxw.dwFlags16 & FORCED3DREFDEVICE) && 
		(iObjectType == TYPE_OBJECT_D3DDEVICE)){
		OutTraceDW("%s: force IID_IDirect3DRefDevice GUID\n", ApiRef);
		res = (*pQueryInterface)(lpdds, IID_IDirect3DRefDevice, obp);
	}
	else
	if((dxw.dwFlags16 & (RAMP2RGBDEVICE|RAMP2MMXDEVICE)) && 
		(iObjectType == TYPE_OBJECT_D3DDEVICE) && 
		(riid == IID_IDirect3DRampDevice)){
		...
		res = (*pQueryInterface)(lpdds, (dxw.dwFlags16 & RAMP2MMXDEVICE) ? IID_IDirect3DMMXDevice : IID_IDirect3DRGBDevice, obp);
	}
#ifdef GHOSTTEXTURE
	else 
	if((dxw.dwDFlags2 & EXPERIMENTAL8) && (iObjectType == TYPE_OBJECT_TEXTURE)) {
		dxw.dwDFlags2 &= ~EXPERIMENTAL8;
		extern LPDIRECTDRAWSURFACE ReplaceTexture(int, LPDIRECTDRAWSURFACE);
		lpdds = ReplaceTexture(dxversion, (LPDIRECTDRAWSURFACE)lpdds);
		res = (*pQueryInterface)(lpdds, riid, obp);
		dxw.dwDFlags2 |= EXPERIMENTAL8;
	}
#endif
	else
	if((iObjectType == TYPE_OBJECT_TEXTURE) && (dxw.dwFlags5 & TEXTURETRANSP)) {
		TextureTransp((LPDIRECTDRAWSURFACE)lpdds, dxversion);
		res = (*pQueryInterface)(lpdds, riid, obp);
	}
	else {
		res = (*pQueryInterface)(lpdds, riid, obp);
	...
	if(res == 0x887602e9){
		...
		res = (*pQueryInterface)(lpDDSEmu_Back, riid, obp);
	}

	// v2.05.94: fixed log messages, obp could be NULL and *obp would then crash the program. Found in "Sorry!".
	if(res) {
		OutTraceDW("%s: ERROR lpdds=%#x%s REFIID=%#x obp=%#x ret=%#x(%s) @%d\n",
			ApiRef, lpdds, IsPrim?"(PRIM)":"", riid.Data1, obp ? *obp : 0, res, ExplainDDError(res), __LINE__);
		return res;
	}
	...
	switch(iObjectType){
		case TYPE_OBJECT_UNKNOWN:
			if(caps=dxwss.DuplicateSurface((LPDIRECTDRAWSURFACE)lpdds, (LPDIRECTDRAWSURFACE)*obp, iObjectVersion)){
				char sCaps[256];
				OutTraceDW("%s: MOVE caps=%#x(%s)\n", ApiRef, caps, ExplainDDSCaps(caps, sCaps, 256));
			}
			break;
		case TYPE_OBJECT_DIRECTDRAW:
		...
		case TYPE_OBJECT_DDRAWSURFACE:
			...
			HookDDSurface((LPDIRECTDRAWSURFACE *)obp, dxw.dwDDVersion, IsPrim);
			// v2.06.06: FORCEDX1HOOK required for @#@ "Eryner"
			if((dxw.dwFlags18 & FORCEDX1HOOK) && (dxw.dwDDVersion != 1)) 
				HookDDSurface((LPDIRECTDRAWSURFACE *)obp, 1, IsPrim);
			break;
