...
extern void HookDXGIFactory(void **);
...
{
	ApiName("dxwnd.HookDirect3D11");
	ID3D11Device *lpd3d11;
	int version = dxw.dwTargetDDVersion;

	OutTraceDW("%s: module=%#x version=%d\n", ApiRef, module, version);
	switch(version){
	case 0:
...
void HookD3D11VideoDevice(void **ppDevice)
{
	ApiName("HookD3D11VideoDevice");
	OutTraceD3D("%s\n", ApiRef);
	SetHook((void *)(**(DWORD **)ppDevice +  0), extD3D11QueryInterface, (void **)&pD3D11QueryInterface, "QueryInterface(D11)");
	// 12: CreateVideoDecoder
	// 16: CreateVideoProcessor 
	// 20: CreateAuthenticatedChannel 
	// 24: CreateCryptoSession 
	// 28: CreateVideoDecoderOutputView 
	// 32: CreateVideoProcessorInputView 
	// 36: CreateVideoProcessorOutputView 
	// 40: CreateVideoProcessorEnumerator 
	// 44: GetVideoDecoderProfile 
	// 48: CheckVideoDecoderFormat 
	// 52: GetVideoDecoderConfigCount 
	// 56: GetVideoDecoderConfig 
	// 60: GetContentProtectionCaps 
	// 64: CheckCryptoKeyExchange 
	// 68: SetPrivateData 
	// 72: SetPrivateDataInterface 
}

void HookD3D11Context(ID3D11DeviceContext **ppImmeCtx)
{
	ApiName("dxwnd.HookD3D11Context");
	OutTrace("%s\n", ApiRef);
	/* C DWORD offsets
...
static char *sDriverType(D3D_DRIVER_TYPE DriverType)
{
	char *s;
	char *types[]={
		"UNKNOWN",
		...
		"SOFTWARE",
		"WARP"};
	if(DriverType <= D3D_DRIVER_TYPE_WARP) s = types[DriverType];
	else s = "???";
	return s;
}
...
{
	HRESULT res;
	ApiName("d3d11.D3D11CreateDevice");

	OutTraceD3D("%s: Adapter=%#x DriverType=%#x(%s) Flags=%#x FeatureLevels=%#x SDKVersion=%#x\n", 
		ApiRef, pAdapter, DriverType, sDriverType(DriverType), Flags, FeatureLevels, SDKVersion);
...
{
	HRESULT res;
	ApiName("d3d11.CreateDeviceAndSwapChain");

	OutTraceD3D("%s: DriverType=%#x Flags=%#x FeatureLevels=%#x SDKVersion=%#x\n", ApiRef, DriverType, Flags, FeatureLevels, SDKVersion);

	DXGI_SWAP_CHAIN_DESC SwapChainDesc;
	if(dxw.Windowize){
		memcpy(&SwapChainDesc, pSwapChainDesc, sizeof(DXGI_SWAP_CHAIN_DESC));
		pSwapChainDesc = &SwapChainDesc;
		(BOOL)(pSwapChainDesc->Windowed) = true;	
		HWND hwnd = dxw.GethWnd();
		if(hwnd != 0){
			(HWND)(pSwapChainDesc->OutputWindow) = dxw.GethWnd();
		}
		else {
...
HRESULT WINAPI extD3D11QueryInterface(void *obj, REFIID riid, void** ppvObj)
{
	...
	switch(riid.Data1){
		...
		case 0x1BC6EA02:
			...
			break;
		case 0x10EC4D5B:
			OutTraceD3D("> IID_ID3D11VideoDevice\n");
			//HookD3D11VideoDevice(ppvObj); 
			break;
		default:
