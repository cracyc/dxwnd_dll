...
HWND extMCIWndCreateA(HWND hwndParent, HINSTANCE hInstance, DWORD dwStyle, LPCTSTR szFile)
{
	...
	// v2.04.80: DispelTweak - the game "Dispel" activates the mouse movement procedure only after 
	// it receives a WM_WINDOWPOSCHANGED message that testify the correct window size 640x480.
	// Since this may never arrive because of scaling & styling, it sends itself one. 
	if(dxw.dwFlags15 & DISPELTWEAK){
		WINDOWPOS wp;
		wp.hwnd = g_hwndMCIWnd;
		...
		wp.cy = dxw.iSizY;
		wp.flags = 0;
		SendMessageA(dxw.GethWnd(), WM_WINDOWPOSCHANGED, 0, (LPARAM)&wp);
		//SendMessageA(g_hwndMCIWnd, WM_WINDOWPOSCHANGED, 0, (LPARAM)&wp);
	}
