...
HRESULT IFakeDirectInputDevice::QueryInterface(REFIID riid, void **ppv)
{
	...
	*ppv = this;
	...
	switch(riid.Data1){
		case 0x5944E680: IVersion=1, IsAscii=TRUE;  s_ClassName="DirectInputDeviceA";	break;
		case 0x5944E681: IVersion=1, IsAscii=FALSE; s_ClassName="DirectInputDeviceW";	break;
		case 0x5944E682: IVersion=2, IsAscii=TRUE;  s_ClassName="DirectInputDevice2A";	break;
		case 0x5944E683: IVersion=2, IsAscii=FALSE; s_ClassName="DirectInputDevice2W";	break;
		case 0x57D7C6BC: IVersion=7, IsAscii=TRUE;  s_ClassName="DirectInputDevice7A";	break;
		case 0x57D7C6BD: IVersion=7, IsAscii=FALSE; s_ClassName="DirectInputDevice7W";	break;
		case 0x54D41080: IVersion=8, IsAscii=TRUE;  s_ClassName="DirectInputDevice8A";	break;
		case 0x54D41081: IVersion=8, IsAscii=FALSE; s_ClassName="DirectInputDevice8W";	break;
	}
...
{
	OutTraceIN("%s::AddRef VJOYSTICK\n", s_ClassName);
	return (ULONG)m_lRef++;
}

ULONG IFakeDirectInputDevice::Release(void)
{
	OutTraceIN("%s::Release VJOYSTICK\n", s_ClassName);
	return (ULONG)m_lRef--;
}

HRESULT IFakeDirectInputDevice::GetCapabilities(LPDIDEVCAPS lpDIDevCaps)
{
	OutTraceIN("%s::GetCapabilities: VJOYSTICK size=%d\n", s_ClassName, lpDIDevCaps->dwSize);
	if(lpDIDevCaps->dwSize != sizeof(DIDEVCAPS)) {
		OutErrorIN("%s::GetCapabilities: VJOYSTICK bad size ret=DIERR_NOTINITIALIZED\n", s_ClassName);
	...
	OutTraceIN("%s::GetCapabilities: VJOYSTICK ret=DI_OK\n", s_ClassName);
	return DI_OK;
}
...
HRESULT IFakeDirectInputDeviceA::EnumObjects(LPDIENUMDEVICEOBJECTSCALLBACKA cb, LPVOID lpv, DWORD dwFlags)
{
	...
	OutTraceIN("%s::EnumObjects: VJOYSTICK dwFlags=%#x(%s)\n", s_ClassName, dwFlags, sEnumObjectFlags(dwFlags));
...
{
	OutTraceIN("%s::SetProperty: VJOYSTICK guid=(%s) "
...
{
	OutTraceIN("%s::GetProperty: VJOYSTICK guid=(%s) "
		"ph={size=%d hdrsize=%d how=%#x(%s) obj=%#x}\n",
		s_ClassName,
		sGUIDProperty(guid),
		lpdph->dwSize,
...
{
	OutTraceIN("%s::Acquire: VJOYSTICK res=%s\n", s_ClassName, b_Acquired ? "NOEFFECT" : "OK");
...
{
	OutTraceIN("%s::Unacquire: VJOYSTICK res=%s\n", s_ClassName, b_Acquired ? "OK" : "NOEFFECT");
...
HRESULT IFakeDirectInputDevice::GetDeviceState(DWORD cbData, LPVOID lpvData)
{
	...
	OutTraceIN("%s::GetDeviceState: VJOYSTICK data=%d active=%#x\n", s_ClassName, cbData, dxw.bActive);
...
HRESULT IFakeDirectInputDevice::GetDeviceData(DWORD cbObjectData, LPDIDEVICEOBJECTDATA rgdod, LPDWORD pdwInOut, DWORD dwFlags)
{
	...
	OutDebugIN("%s::GetDeviceData: VJOYSTICK size=%d inout=%d flags=%#x(%s) rgdod=%#x\n", s_ClassName,
		cbObjectData, *pdwInOut, dwFlags, 
		dwFlags ? (dwFlags == DIGDD_PEEK ? "PEEK" : "unknown") : "NULL",
		rgdod);

	if(!dxw.bActive) {
		OutDebugIN("%s::GetDeviceData: VJOYSTICK inactive inout=0\n", s_ClassName);
		*pdwInOut = 0;
		return DI_OK;
	}
	...
	if(((*pGetTickCount)() - dwLastSample) > 10){
		...
		if(iDataAvail == 0){
			...
			OutDebugIN("%s::GetDeviceData: VJOYSTICK emptyqueue inout=0\n", s_ClassName);
			*pdwInOut = 0;
			return DI_OK;
		}
...
HRESULT IFakeDirectInputDevice::SetDataFormat(LPCDIDATAFORMAT lpdf)
{
	...
	if(IsDebugIN){
		for(DWORD i=0; i<lpdf->dwNumObjs; i++){
			...
			switch(i){
				case 0: ofs_xaxis = ofs; break;
				case 1: ofs_yaxis = ofs; break;
				case 2: ofs_btnL =  ofs; break;
				case 3: ofs_btnR =  ofs; break;
				case 4: ofs_btnC =  ofs; break;
			}
		}
	} 
...
HRESULT IFakeDirectInputDevice::SetCooperativeLevel(HWND hwnd, DWORD dwFlags)
{
	...
	OutTraceIN("%s::SetCooperativeLevel(I): hwnd=%#x flags=%#x(%s)\n", 
		s_ClassName, hwnd, dwFlags, ExplainDICooperativeFlags(dwFlags, sFlags, 128));
	return DI_OK;
}
...
HRESULT IFakeDirectInputDeviceA::GetObjectInfo(LPDIDEVICEOBJECTINSTANCEA lpidoi, DWORD dwObj, DWORD dwHow)
{
	...
	switch(dwHow){
		case DIPH_BYOFFSET:
			...
			if((dwObj >= DIJOFS_POV(0)) && (dwObj <= DIJOFS_POV(3))){
		...
		case DIPH_BYUSAGE:
		default:
			OutErrorIN("%s::GetObjectInfo: ERROR unknown how=%#x(%s) ret=\n", s_ClassName, dwHow, sHow(dwHow), DIERR_INVALIDPARAM);
...
HRESULT IFakeDirectInputDeviceA::GetDeviceInfo(LPDIDEVICEINSTANCEA lpidi)
{
	...
	if(lpidi->dwSize != sizeof(DIDEVICEINSTANCEA)) {
		OutErrorIN("%s::GetDeviceInfo: bad size ret=DIERR_NOTINITIALIZED\n", s_ClassName);
		return DIERR_NOTINITIALIZED;
	}
...
*/
#ifndef DXW_NOTRACES
static char *sEffect(DWORD g1)
{
	char *captions[]={
		"ConstantForce", "RampForce", "Square", "Sine",
		"Triangle", "SawtoothUp", "SawtoothDown", "Spring",
		"Damper", "Inertia", "Friction", "CustomForce"
	};
	if((g1 >= 0x13541C20) && (g1 <= 0x13541C2B)) return captions[g1-0x13541C20];
	return "unknown";
}

static void dumpEffect(LPCDIEFFECT e)
{
	OutTrace("> dwSize=%d\n", e->dwSize);
	OutTrace("> dwFlags=%#x\n", e->dwFlags);
	OutTrace("> dwDuration=%d\n", e->dwDuration);
	OutTrace("> dwSamplePeriod=%d\n", e->dwSamplePeriod);
	OutTrace("> dwGain=%d\n", e->dwGain);
	OutTrace("> dwTriggerButton=%#x\n", e->dwTriggerButton);
	OutTrace("> dwTriggerRepeatInterval=%d\n", e->dwTriggerRepeatInterval);
	OutTrace("> cAxes=%#x\n", e->cAxes);
	OutTrace("> rgdwAxes=%d\n", e->rgdwAxes);
	OutTrace("> rglDirection=%d\n", e->rglDirection);
    //LPDIENVELOPE lpEnvelope;
    //DWORD cbTypeSpecificParams;
    //LPVOID lpvTypeSpecificParams;
 	OutTrace("> dwStartDelay=%d\n", e->dwStartDelay);
   //DWORD ;
}
...
static void dumpEffectEscape(LPDIEFFESCAPE e)
{
	OutTrace("> dwSize=%d\n", e->dwSize);
	OutTrace("> dwCommand=%#x\n", e->dwCommand);
	OutTrace("> cbInBuffer=%d\n", e->cbInBuffer);
	OutTrace("> cbOutBuffer=%d\n", e->cbOutBuffer);
}
#endif // DXW_NOTRACES
...
{
#ifndef DXW_NOTRACES
	OutTraceIN("%s::CreateEffect: lpdid=%#x guid=%#x(%s) lpe=%#x\n", 
		s_ClassName, this, giod.Data1, sEffect(giod.Data1), lpe);
	if(IsDebugIN) dumpEffect(lpe);
#endif // DXW_NOTRACES
	if(dxw.dwFlags16 & JOYSTICKEFFECTS){
		IFakeDirectInputEffect *FakeEffect = new (IFakeDirectInputEffect);
		*lpdie = (LPDIRECTINPUTEFFECT)FakeEffect;
		OutTraceIN("%s::CreateEffect: lpdie=%#x ret=DI_OK\n", s_ClassName, *lpdie);
		return DI_OK; 
	}
	*lpdie = NULL;
...
{
	OutTraceIN("%s::EnumEffects: lpdid=%#x lpe=%#x\n", s_ClassName, this, lpe);
	if(IsAscii) 
...
{
	// v2.05.92: the fake joystick at the moment doesn't enumerate any effect. 
	OutTraceIN("%s::EnumEffects: lpdid=%#x lpe=%#x\n", s_ClassName, this, lpe);
	return DI_OK;
}
...
{
	OutTraceIN("%s::EnumEffectsInFile: lpdid=%#x\n", s_ClassName, this);
...
{
	OutTraceIN("%s::WriteEffectToFile: : lpdid=%#x\n", s_ClassName, this);
...
{
	OutTraceIN("%s::BuildActionMap: lpdid=%#x\n", s_ClassName, this);
...
{
	OutTraceIN("%s::SetActionMap: lpdid=%#x\n", s_ClassName, this);
	utrap("SetActionMap");
	return DI_OK;
}
...
{
	OutTraceIN("%s::GetImageInfo: lpdid=%#x\n", s_ClassName, this);
	utrap("GetImageInfo");
	return DI_OK;
}
...
IFakeDirectInputEffect::IFakeDirectInputEffect()
{
	s_ClassName = "IDirectInputEffect";
	m_lRef = 1;
}

HRESULT IFakeDirectInputEffect::QueryInterface(REFIID riid, void **ppv)
{
	OutTraceIN("%s::QueryInterface: lpdie=%#x guid=%s\n", s_ClassName, this, ExplainGUID((GUID *)&riid));
	return (HRESULT)DI_OK;
}

ULONG IFakeDirectInputEffect::AddRef(void)
{
	OutTraceIN("%s::AddRef: lpdie=%#x\n", s_ClassName, this);
	return (ULONG)m_lRef++;
}

ULONG IFakeDirectInputEffect::Release(void)
{
	OutTraceIN("%s::Release: lpdie=%#x\n", s_ClassName, this);
	return (ULONG)m_lRef--;
}

HRESULT IFakeDirectInputEffect::Initialize(HINSTANCE h, DWORD f, REFGUID guid)
{
	OutTraceIN("%s::Initialize: lpdie=%#x\n", s_ClassName, this);
	return DI_OK;
}

HRESULT IFakeDirectInputEffect::GetEffectGuid(LPGUID guid)
{
	OutTraceIN("%s::GetEffectGuid: lpdie=%#x\n", s_ClassName, this);
	return DI_OK;
}

HRESULT IFakeDirectInputEffect::GetParameters(LPDIEFFECT e, DWORD f)
{
	OutTraceIN("%s::GetParameters: lpdie=%#x\n", s_ClassName, this);
	return DI_OK;
}

HRESULT IFakeDirectInputEffect::SetParameters(LPCDIEFFECT e, DWORD f)
{
	OutTraceIN("%s::SetParameters: lpdie=%#x effect=%#x flags=%#x\n", s_ClassName, this, e, f);
	if(IsTraceIN) dumpEffect(e);
	return DI_OK;
}

HRESULT IFakeDirectInputEffect::Start(DWORD dwIterations, DWORD dwFlags)
{
	OutTraceIN("%s::Start: lpdie=%#x iterations=%d flags=%#x\n", s_ClassName, this, dwIterations, dwFlags);
	return DI_OK;
}

HRESULT IFakeDirectInputEffect::Stop()
{
	OutTraceIN("%s::Stop: lpdie=%#x\n", s_ClassName, this);
	return DI_OK;
}

HRESULT IFakeDirectInputEffect::GetEffectStatus(LPDWORD f)
{
	OutTraceIN("%s::GetEffectStatus: lpdie=%#x\n", s_ClassName, this);
	return DI_OK;
}

HRESULT IFakeDirectInputEffect::Download()
{
	OutTraceIN("%s::Download: lpdie=%#x\n", s_ClassName, this);
	return DI_OK;
}

HRESULT IFakeDirectInputEffect::Unload()
{
	OutTraceIN("%s::Unload: lpdie=%#x\n", s_ClassName, this);
	return DI_OK;
}

HRESULT IFakeDirectInputEffect::Escape(LPDIEFFESCAPE lpde)
{
#ifndef DXW_NOTRACES
	OutTraceIN("%s::Escape: lpdie=%#x lpdieffesc=%#x\n", s_ClassName, this, lpde);
	if(IsDebugIN) dumpEffectEscape(lpde);
#endif // DXW_NOTRACES
	lpde->cbOutBuffer = 0;
	return DI_OK;
}
