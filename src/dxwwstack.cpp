...
void dxwWStack::Put(HWND hwnd, WNDPROC wndproc, int w, int h)
{
	...
	for(StackIdx=0; StackIdx<WhndTOS; StackIdx++) 
		if (WhndStack[StackIdx].hwnd==hwnd) {
			...
			OutTrace(">>>(Put): updated hwnd=%#x size=(%dx%d) idx=%d\n", hwnd, w, h, StackIdx);
#endif
			return;
		}
	...
	// increment TOS.
#ifdef DXW_WINPROC_STACK_TRACING
	OutTrace(">>>(Put): added hwnd=%#x size=(%dx%d) idx=%d\n", hwnd, w, h, StackIdx);
#endif
	WhndTOS++;
}
...
void dxwWStack::PutProc(HWND hwnd, WNDPROC wndproc)
{
	...
	for(StackIdx=0; StackIdx<WhndTOS; StackIdx++) 
		if (WhndStack[StackIdx].hwnd==hwnd) {
			// update only valid fields
			if (wndproc) WhndStack[StackIdx].wndproc=wndproc; // v2.05.98 fix
#ifdef DXW_WINPROC_STACK_TRACING
			OutTrace(">>>(PutProc): updated hwnd=%#x wndproc=%#x idx=%d\n", hwnd, wndproc, StackIdx);
#endif
			return;
		}
	WhndStack[WhndTOS].hwnd=hwnd;
...
#ifdef DXW_WINPROC_STACK_TRACING
	OutTrace(">>>(PutProc): added hwnd=%#x wndproc=%#x idx=%d\n", hwnd, wndproc, StackIdx);
#endif
	// increment TOS.
	WhndTOS++;
}
...
#ifdef DXW_WINPROC_STACK_TRACING
			OutTrace(">>>(PutSize): updated hwnd=%#x size=(%dx%d) idx=%d\n", hwnd, w, h, StackIdx);
#endif
			return;
		}
	WhndStack[WhndTOS].hwnd=hwnd;
...
#ifdef DXW_WINPROC_STACK_TRACING
	OutTrace(">>>(PutSize): added hwnd=%#x size=(%dx%d) idx=%d\n", hwnd, w, h, StackIdx);
#endif
	// increment TOS.
	WhndTOS++;
}
...
#ifdef DXW_WINPROC_STACK_TRACING
		OutTrace(">>>(GetSize): hwnd=%#x, size=(%dx%d)\n", hwnd, w ? *w : 0, h ? *h : 0);
#endif
		return TRUE;
	}
#ifdef DXW_WINPROC_STACK_TRACING
...
WNDPROC dxwWStack::GetProc(HWND hwnd)
{
	...
	for(StackIdx=0; StackIdx<WhndTOS; StackIdx++) {
		if (WhndStack[StackIdx].hwnd==hwnd) {
			//OutTraceDW("DEBUG: WNDPROC STACK pop hwnd=%#x, wndproc=%#x\n", hwnd, WhndStack[StackIdx].wndproc);
#ifdef DXW_WINPROC_STACK_TRACING
			OutTrace(">>>(GetProc): hwnd=%#x, wndproc=%#x\n", hwnd, WhndStack[StackIdx].wndproc);
#endif
			return WhndStack[StackIdx].wndproc; // either a good value, or NULL
		}
	}
#ifdef DXW_WINPROC_STACK_TRACING
	OutTrace(">>>(GetProc): hwnd=%#x, wndproc=NULL\n", hwnd);
#endif
	return NULL;
}
