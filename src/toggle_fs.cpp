...
void dx_FullScreenToggle()
{
	...
	// toggle ....
	if (bFullScreen){
		OutTraceDW("DxWnd: exiting fullscreen mode: hwnd=%#x style=%#x extstyle=%#x pos=(%d,%d)-(%d,%d)\n",
			hwnd, OldStyle, OldExtStyle, WinRect.left, WinRect.top, WinRect.right, WinRect.bottom);
		...
		if(ChangeDisplayResult != DISP_CHANGE_SUCCESSFUL){
			OutTraceE("ChangeDisplaySettingsEx ERROR: res=%d @%d\n", ChangeDisplayResult, __LINE__);
		...
		OutTraceDW("DxWnd: entering fullscreen mode: hwnd=%#x\n", hwnd);
		...
		if(!(*pEnumDisplaySettingsA)(mi.szDevice, ENUM_CURRENT_SETTINGS, &oldDisplayMode)){
		...
		for (int i=0; ;i++){
			...
			if(!(*pEnumDisplaySettingsA)(mi.szDevice, i, &DisplayMode))break; // no more modes
		...
		(*pEnumDisplaySettingsA)(mi.szDevice, BestIndex, &DisplayMode);
		...
		// activate new mode
		(*pSetWindowLong)(hwnd, GWL_STYLE, 0);
...
void dx_DesktopToggle(BOOL bWorkArea)
{
