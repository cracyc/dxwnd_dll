...
Blitter_Type *pRendererBlt;

BOOL dxwHWBlitUnsupported(int dxversion, Blt_Type pBlt, LPDIRECTDRAWSURFACE lpdds, LPRECT lpemurect, LPDIRECTDRAWSURFACE lpddssrc, LPRECT lpsrcrect)
{
	ApiName("dxwHWBlitUnsupported");
	// v2.04.74:
	...
	do { // fake loop
		...
		ddsd.dwSize = dwSize;
		res2=(*pGetSurfaceDescMethod(dxversion))((LPDIRECTDRAWSURFACE2)lpddssrc, &ddsd);
		if(res2) {
			OutTraceE("%s: GetSurfaceDesc ERROR lpdds=%#x err=%#x(%s) @%d\n", ApiRef, lpddssrc, res2, ExplainDDError(res2), __LINE__);
			break;
		}
...
BOOL dxwSWBlitUnsupported(int dxversion, Blt_Type pBlt, LPDIRECTDRAWSURFACE lpdds, LPRECT lpemurect, LPDIRECTDRAWSURFACE lpddssrc, LPRECT lpsrcrect)
{
	ApiName("dxwSWBlitUnsupported");
	// v2.04.74:
	...
	do { // fake loop
		...
		res2=(*pGetSurfaceDescMethod(dxversion))((LPDIRECTDRAWSURFACE2)lpddssrc, &ddsdsrc);
		if(res2) {
			OutTraceE("%s: GetSurfaceDesc ERROR lpdds=%#x err=%#x(%s) @%d\n", ApiRef, lpddssrc, res2, ExplainDDError(res2), __LINE__);
			break;
		}
		if(ddsdsrc.ddpfPixelFormat.dwFourCC == 0) {
			OutTraceDW("%s: no fourcc code\n", ApiRef);
			break;
		}
		if(ddsdsrc.lpSurface == 0) {
			OutTraceDW("%s: no surface data\n", ApiRef);
			break;
		}
		//res=(*pLockMethod(dxversion))(lpddssrc, 0, (LPDDSURFACEDESC)&ddsd, DDLOCK_SURFACEMEMORYPTR|DDLOCK_READONLY, 0);
		...
		if(ddsdsrc.ddpfPixelFormat.dwFourCC) pixels = dxwConvertFourCC(&ddsdsrc, ddsdsrc.ddpfPixelFormat.dwRGBBitCount);
		if(!pixels){
...
#ifndef DXW_NOTRACES
		if(res2) OutTraceE("%s: Unlock ERROR err=%#x(%s) @%d\n", ApiRef, res2, ExplainDDError(res2), __LINE__);
#endif // DXW_NOTRACES
		res2=(*pUnlockMethod(dxversion))(lpddssrc, NULL);
#ifndef DXW_NOTRACES
		if(res2) OutTraceE("%s: Unlock ERROR err=%#x(%s) @%d\n", ApiRef, res2, ExplainDDError(res2), __LINE__);
...
static void SurfaceUnlock(LPDIRECTDRAWSURFACE s, int dxversion)
{
	ApiName("SurfaceUnlock");
	DDSURFACEDESC2 ddsd;
	HRESULT res;
	...
	LPDIRECTDRAWSURFACE lpddssrc, LPRECT lpsrcrect, DWORD dwflags, LPDDBLTFX lpddbltfx)
{
	...
	case DDERR_SURFACELOST:
		lpdds->Restore();
		res= (*pBlt)(lpdds, lpdestrect, lpddssrc, lpsrcrect ? &srcrect : NULL, dwflags, lpddbltfx);
		OutTraceDW("%s: Blt SURFACELOST RETRY err=%#x(%s)\n", ApiRef, res, ExplainDDError(res)); 
		break;
	default:
		break;
	}
#ifndef DXW_NOTRACES
	if (res) BlitError(res, lpsrcrect ? &srcrect : NULL, lpdestrect, __LINE__);
	if(IsDebugDW) {
		DescribeSurface(lpdds, dxversion, "[DST]" , __LINE__);
		if (lpddssrc) DescribeSurface(lpddssrc, dxversion, "[SRC]" , __LINE__); // lpddssrc could be NULL!!!
	}
#endif // DXW_NOTRACES
	if(dxw.dwFlags1 & SUPPRESSDXERRORS) res=DD_OK;
	// v2.06.06 SURFACEUNLOCK: forces a surface unlock after the blt operation. Fixes @#@ "H.E.A.D.Z." missing heads
	if(dxw.dwFlags18 & SURFACEUNLOCK) SurfaceUnlock(lpdds, dxversion);
...
HRESULT PrimaryNotEmulated(int dxversion, Blt_Type pBlt, ApiArg, LPDIRECTDRAWSURFACE lpdds, LPRECT lpdestrect,
	LPDIRECTDRAWSURFACE lpddssrc, LPRECT lpsrcrect, DWORD dwflags, LPDDBLTFX lpddbltfx, BOOL isFlipping)
{
	...
	if(res){
		...
		if((res==DDERR_INVALIDPARAMS) && (dwflags & DDBLT_KEYSRC)){
			...
			(*pGetSurfaceDescMethod(dxversion))((LPDIRECTDRAWSURFACE2)lpddssrc, &ddsd);
			res=(*pCreateSurfaceMethod(dxversion))(lpPrimaryDD, &ddsd, (LPDIRECTDRAWSURFACE *)&lpddsTmp, NULL);
#ifndef DXW_NOTRACES
			if(res) OutTraceE("%s: CreateSurface ERROR err=%#x(%s) @%d", ApiRef, res, ExplainDDError(res), __LINE__);
#endif // DXW_NOTRACES
			// copy background
			res= (*pBlt)((LPDIRECTDRAWSURFACE)lpddsTmp, lpsrcrect, lpdds, lpdestrect, DDBLT_WAIT, NULL);
#ifndef DXW_NOTRACES
			if(res) OutTraceE("%s: Blt ERROR err=%#x(%s) @%d", ApiRef, res, ExplainDDError(res), __LINE__);
#endif // DXW_NOTRACES
			// overlay texture
		...
		if(res==DDERR_SURFACELOST){
			lpdds->Restore();
			res=(*pPrimaryBlt)(dxversion, pBlt, lpdds, lpdestrect, lpddssrc, lpsrcrect, dwflags, lpddbltfx);
			OutTraceDW("%s: Blt SURFACELOST RETRY err=%#x(%s) @%d\n", ApiRef, res, ExplainDDError(res), __LINE__); 
		}
		if(dxw.dwFlags1 & SUPPRESSDXERRORS) res=DD_OK;
	}
...
HRESULT NULLBlitter(int dxversion, Blt_Type pBlt, ApiArg, LPDIRECTDRAWSURFACE lpdds, LPRECT lpdestrect,
	LPDIRECTDRAWSURFACE lpddssrc, LPRECT lpsrcrect, DWORD dwflags, LPDDBLTFX lpddbltfx, BOOL isFlipping)
{
	return DD_OK;
}
	...
	LPDIRECTDRAWSURFACE lpddssrc, LPRECT lpsrcrect, DWORD dwflags, LPDDBLTFX lpddbltfx, BOOL isFlipping)
{
	...
	if (gFlippedToScreen) {
		OutTraceDW("%s: FlippedToScreen SKIP\n", ApiRef);
		gFlippedToScreen=0;
		return DD_OK;
	}
	...
	if(CurrentActiveMovieWin && (dxw.dwFlags6 & STRETCHMOVIES)){
		OutDebugDW("%s: STRETCHMOVIES hwnd=%#x\n", ApiRef, CurrentActiveMovieWin);
		lpdestrect = NULL;
		lpsrcrect = NULL;
	}
	...
	if((lpddssrc==0) && !(dwflags & DDBLT_COLORFILL)){
		...
		OutTraceDW("%s: setting flip chain to lpdds=%#x\n", ApiRef, lpddssrc);
	}
...
#ifndef DXW_NOTRACES
		if(res2) OutTraceE("%s: WaitForVerticalBlank ERROR res=%#x\n", ApiRef, res2);
#endif // DXW_NOTRACES
	}
	...
	LPDIRECTDRAWSURFACE lpddssrc, LPRECT lpsrcrect, DWORD dwflags, LPDDBLTFX lpddbltfx, BOOL isFlipping)
{
	...
	if(ToPrim){
		if(dxwss.IsABackBufferSurface(lpddssrc) && (dxw.dwFlags18 & STRETCHBACKBUFFER)) lpdestrect = NULL;
...
HRESULT DDRawBlitToEmu(int dxversion, Blt_Type pBlt, LPDIRECTDRAWSURFACE lpdds, LPRECT lpdestrect,
	LPDIRECTDRAWSURFACE lpddssrc, LPRECT lpsrcrect, DWORD dwflags, LPDDBLTFX lpddbltfx)
{
	ApiName("DDRawBlitToEmu");
	HRESULT res;

	if(lpsrcrect){
		if((int)(lpsrcrect->left) < 0) lpsrcrect->left = 0;
		if((int)(lpsrcrect->top) < 0) lpsrcrect->top = 0;
	}
	if(lpdestrect){
		if((int)(lpdestrect->left) < 0) lpdestrect->left = 0;
		if((int)(lpdestrect->top) < 0) lpdestrect->top = 0;
	}
	...
	if (res) {
		...
		if (res == DDERR_UNSUPPORTED){
			...
			switch(dxw.dwFlags10 & (HWFOURCC | SWFOURCC)){
				case HWFOURCC:
					if(dxwHWBlitUnsupported(dxversion, pBlt, lpdds, lpdestrect, lpddssrc, lpsrcrect)) res=DD_OK;
					break;
				case SWFOURCC:
					if(dxwSWBlitUnsupported(dxversion, pBlt, lpdds, lpdestrect, lpddssrc, lpsrcrect)) res=DD_OK;
					break;
			}
		}
		...
		if(res==DDERR_SURFACEBUSY){
			if (bFlippedDC) {
				(*pReleaseDCMethod())(lpdds, hFlippedDC);
			}
			...
			_if(res && (res!=DDERR_NOTLOCKED)) OutTraceE("%s: Unlock ERROR lpdds=%#x err=%#x(%s)\n", ApiRef, lpddssrc, res, ExplainDDError(res));
			}
