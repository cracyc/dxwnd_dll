...
__inline NTLEA_WND_ASC_DATA* CheckProp(HWND hWnd) {
	...
	if (!p) {
		OutErrorLOC("%s: Unacceptable Empty Window Prop Detected, Force Exit.\n");
		exit(0);
	}
	return p;
}
...
{
	ApiName("SetTopWindowProc");
	OutTraceLOC("%s: hwnd=%#x\n", ApiRef, hWnd);
...
void InstallCbtHook(NTLEA_TLS_DATA * ptls) 
{ 
	ApiName("InstallCbtHook");
	// ebx 
	...
	OutTraceLOC("%s: guard=%d\n", ApiRef, ptls->hWindowHooking);
...
void UninstallCbtHook(NTLEA_TLS_DATA * ptls) 
{ 
	ApiName("UninstallCbtHook");
	// ebx 
	...
	if (ptls->hWindowHooking == 0) {
		...
		OutErrorLOC("%s: unmatching hWindowHooking=%d\n", ApiRef, ptls->hWindowHooking < 0);
	}
}

LRESULT CALLBACK DefConversionProc(ApiArg, LPVOID lpProcAddress, HWND hWnd, HWND hMDIClient,
	BOOL bMDIClientEnabled, INT uMsg, WPARAM wParam, LPARAM lParam)
{
	// debug log - to delete
	HWND target = bMDIClientEnabled ? hMDIClient : hWnd;
	OutDebugLOC("%s: hwnd=%#x hmdiclient=%#x midi=%#x msg=%#x(%s) wparam=%#x lparam=%#x type=%s\n",
		ApiRef, hWnd, hMDIClient, bMDIClientEnabled, uMsg, ExplainWinMessage(uMsg), wParam, lParam,
	...
	case WM_NCCREATE: // 
	{
		if (lParam) { // simplified ...
			...
			if (p->lpWindowName) {
				LPCWSTR wptr;
				OutTrace("%s: wname=\"%s\"\n", ApiRef, p->lpWindowName);
				wptr = MultiByteToWideCharInternal((LPCSTR)p->lpWindowName);
				lpUnicodeWindowName = (LPCWSTR)malloc((wcslen(wptr)+1) * sizeof(WCHAR));
				memcpy((LPVOID)lpUnicodeWindowName, wptr, (wcslen(wptr)+1) * sizeof(WCHAR));
				p->lpWindowName = (LPVOID)lpUnicodeWindowName;
				free((LPVOID)wptr);
			}
			if ((DWORD_PTR)p->lpClassName & WM_CLASSMASK) {
				LPCWSTR wptr;
				OutTrace("%s: cname=\"%s\"\n", ApiRef, p->lpClassName);
				wptr = MultiByteToWideCharInternal((LPCSTR)p->lpClassName);
				lpUnicodeClassName = (LPCWSTR)malloc((wcslen(wptr)+1) * sizeof(WCHAR));
				memcpy((LPVOID)lpUnicodeClassName, wptr, (wcslen(wptr)+1) * sizeof(WCHAR));
				p->lpClassName = (LPVOID)lpUnicodeClassName;
				free((LPVOID)wptr);
			}
			// LN899
	...
	case WM_DEVMODECHANGE: // LN220
	{
		...
		OutTraceLOC("%s: A(%s) -> W(%S)\n", ApiRef, (LPCSTR)lParam, lParamW);
