...
static GetModuleInformation_Type pGetModuleInformation = NULL;

static char *getCeiling(HMODULE module)
{
	MODULEINFO modinfo;
	if(!pGetModuleInformation){
		...
		if(!pGetModuleInformation) MessageBox(0, "GetModuleInformation failed", "dxwnd", 0);
	}
	(*pGetModuleInformation)(GetCurrentProcess(), module, &modinfo, sizeof(MODULEINFO));
	return (char *)module + modinfo.SizeOfImage;
}
...
static char *NextString(char *fname, char *limit)
{
	ApiName("NextString");
...
static BOOL UpdateFunctionAddress(ApiArg, DWORD *OldAddress, DWORD NewAddress)
{
	...
	if(!VirtualProtect(OldAddress, 4, PAGE_EXECUTE_READWRITE, &oldprotect)) {
		OutTraceDW("%s: VirtualProtect error %d @%d\n", ApiRef, GetLastError(), __LINE__);
		return FALSE;
	}
	*OldAddress = NewAddress;
	...
	if (!FlushInstructionCache(GetCurrentProcess(), OldAddress, 4)) {
		OutTraceDW("%s: FlushInstructionCache error %d @%d\n", ApiRef, GetLastError(), __LINE__);
		return FALSE;
	}
	return TRUE;
}
...
void *IATPatchDefault(HMODULE module, DWORD ordinal, char *dll, void *apiproc, const char *apiname, void *hookproc)
{
	...
	__try{
		...
		while(pidesc->FirstThunk){
			...
			if(!lstrcmpi(dll, impmodule)) {
				...
				if(ptaddr->u1.Function) {
					...
					if(!UpdateFunctionAddress(ApiRef, &ptaddr->u1.Function, (DWORD)hookproc)) {
						OutTraceH("%s: ERROR hook flush failed err=%d\n", ApiRef, GetLastError());
						return 0;
					}
					hooked = TRUE;
					// return org; v2.05.59 - search for duplicated entries
				}
			}
			pidesc ++;
		}
		if(!hooked) {
			...
			// return 0; v2.05.59 - search for duplicated entries
		}
...
void *IATPatchSequential(HMODULE module, DWORD ordinal, char *dll, void *apiproc, const char *apiname, void *hookproc)
{
	...
	base = (DWORD)module;
	ceiling = getCeiling(module);
	org = NULL; // by default, ret = NULL => API not found
	...
	__try{
		...
		while(pidesc->FirstThunk){
			impmodule = (PSTR)(base + pidesc->Name);
			if(!(fname = NextString(impmodule, ceiling))) return NULL;
			pidesc ++;
		}
		pidesc = (PIMAGE_IMPORT_DESCRIPTOR)(base + rva);
			...
			if(!lstrcmpi(dll, impmodule)) {
				...
				while(ptaddr->u1.Function){
					...
					if (ptname){
						...
						if(!(fname=NextString(fname, ceiling))) return NULL;
					}
...
void DumpImportTableDefault(HMODULE module)
{
	...
	PIMAGE_THUNK_DATA ptname;
	PIMAGE_IMPORT_BY_NAME piname;
	ApiName("DumpImportTableDefault");
	...
	__try{
		pnth = PIMAGE_NT_HEADERS(PBYTE(base) + PIMAGE_DOS_HEADER(base)->e_lfanew);
		if(!pnth) {
			OutTrace("%s: ERROR no pnth @%d\n", ApiRef, __LINE__);
			return;
		}
		rva = pnth->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress;
		if(!rva) {
			OutTrace("%s: ERROR no rva @%d\n", ApiRef, __LINE__);
			return;
		}
		pidesc = (PIMAGE_IMPORT_DESCRIPTOR)(base + rva);
...
void DumpImportTableSequential(HMODULE module)
{
	...
	__try{
		...
		while(pidesc->FirstThunk){
			impmodule = (PSTR)(base + pidesc->Name);
			if(!(fname  = NextString(impmodule, ceiling))) return;
			pidesc ++;
		}
		pidesc = savepidesc;
		...
		while(pidesc->FirstThunk){
			...
			while(ptaddr->u1.Function){
				...
				if(ptname){
					...
					if(!(fname = NextString(fname, ceiling))) return;
				}
...
BOOL IsIATSequential(HMODULE module)
{
	...
	__try{
		...
		if(!stristr(fname, ".DLL")) {
			OutTrace("%s: ERROR no .DLL @%d\n", ApiRef, __LINE__);
			return FALSE;
		}
		// skip first string
		if(!(fname = NextString(fname, ceiling))) return FALSE;
