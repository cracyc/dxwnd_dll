...
#define MENUGRACEPERIOD 4000

LRESULT CALLBACK MessageHook(int code, WPARAM wParam, LPARAM lParam)
{
	ApiName("dxwnd.MessageHook");
	static BOOL bSizeMoving = FALSE;
	static BOOL bFrameClick = FALSE;
	static DWORD unscaleTimer = 0;

	if(code == HC_ACTION) { 
		if(dxw.IsFullScreen()){
			MSG *msg;
			msg = (MSG *)lParam;
			OutTraceC("%s: hwnd=%#x message=%#x(%s) remove=%d params=(L:%#x,R:%#x) pt=(%d,%d)\n", 
				ApiRef, msg->hwnd, msg->message, ExplainWinMessage(msg->message), wParam, 
				msg->lParam, msg->wParam, msg->pt.x, msg->pt.y);
			...
			if(dxw.hWndMenu) {
				...
				dxw.hWndMenu;
				(*pGetCursorPos)(&pos);
				res = (*pSendMessageA)(dxw.hWndMenu, WM_NCHITTEST, 0, MAKELPARAM(pos.x, pos.y));
				//OutTrace("!!! NCHITTEST res=%#x @xy=(%d,%d)\n", res, pos.x, pos.y);
				if(res == HTMENU) {
					//OutTrace("!!! bypass mouse hovering\n");
					unscaleTimer = (*pGetTickCount)();
					return CallNextHookEx(hMouseHook, code, wParam, lParam);
				}
				if(((*pGetTickCount)() - unscaleTimer) < MENUGRACEPERIOD){
					//OutTrace("!!! bypass mouse scaling msg=%#x\n", msg->message);
					//switch(msg->message){
					//	case WM_LBUTTONDOWN:
					//	case WM_RBUTTONDOWN:
					//	case WM_MBUTTONDOWN: 
					//		// ends grace period
					//		OutTrace("!!! ends grace period\n");
					//		unscaleTimer = 0;
					//		break;
					//}
					return CallNextHookEx(hMouseHook, code, wParam, lParam);
			...
			if ((bSizeMoving) ||
				...
				OutTraceC("%s: fixed hwnd=%#x lparam/pt=(%d,%d)\n", ApiRef, msg->hwnd, pt.x, pt.y);
