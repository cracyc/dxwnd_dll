...
HRESULT WINAPI extDllCanUnloadNow(void)
{
	...
	_if(res) OutErrorDDRAW("ddrawex#DllCanUnloadNow ERROR: res=%#x\n", res);
	return res;
}
...
HRESULT WINAPI extDllGetClassObject(REFCLSID rclsid, REFIID riid, void **out)
{
	...
	_if(res) OutErrorDDRAW("ddrawex#DllGetClassObject ERROR: res=%#x\n", res);
	return res;
}
...
HRESULT WINAPI extDllRegisterServer(void)
{
	...
	_if(res) OutErrorDDRAW("ddrawex#DllRegisterServer ERROR: res=%#x\n", res);
	return res;
}
...
HRESULT WINAPI extDllUnregisterServer(void)
{
	...
	_if(res) OutErrorDDRAW("ddrawex#DllUnregisterServer ERROR: res=%#x\n", res);
	return res;
}

#ifndef WORKINGUNHOOKEDMODE
SetCooperativeLevel_Type pSetCooperativeLevelEX;
extern HRESULT WINAPI extSetCooperativeLevel(char *, int, SetCooperativeLevel_Type, LPDIRECTDRAW, HWND, DWORD);

HRESULT WINAPI extSetCooperativeLevelEX(LPDIRECTDRAW lpdd, HWND hwnd, DWORD dwflags)
{ return extSetCooperativeLevel("IDirectDrawEx::SetCooperativeLevel", 1, pSetCooperativeLevelEX, lpdd, hwnd, dwflags); }
...
{
	HRESULT res;
	ApiName("IDirectDrawEx::CreateDirectDraw");

#ifndef DXW_NOTRACES
	char sCoopFlags[256];
	OutTraceDW("%s: factory=%#x guid=%s hwnd=%#x coopflags=%#x(%s)\n", 
		ApiRef, ddf, sGUID(pGUID), hWnd, dwCoopLevelFlags, ExplainCoopFlags(dwCoopLevelFlags, sCoopFlags, 256));
#endif // DXW_NOTRACES
	...
	// v2.06.06: switched to LOCKNORMALCOOP and LOCKEXCLUSIVECOOP flags
	...
	if(pDirectDrawCreate == NULL){
		OutTraceDW("%s: force hooking DirectDraw session\n", ApiRef);
		HINSTANCE hinst;
	...
	// v2.05.67: to recover "Theocracy" it is necessary to avoid hooking ...
#if 0
...
{
	HRESULT res;
	ApiName("IDirectDrawEx::DirectDrawEnumerate");
...
{ return extQueryInterfaceDX("IDirectDrawEx::QueryInterface", 1, pQueryInterfaceDEX, lpdd, riid, obp); }
HRESULT WINAPI extQueryInterfaceEX(void *lpdd, REFIID riid, LPVOID *obp)
{ return extQueryInterfaceDX("IDirectDrawFactory::QueryInterface", 1, pQueryInterfaceEX, lpdd, riid, obp); }
#endif
