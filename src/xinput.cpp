...
DWORD WINAPI extXInputGetState(DWORD dwUserIndex, XINPUT_STATE *pState)
{
	...
	if(dxw.dwFlags6 & VIRTUALJOYSTICK) {
		// emulate
		res = emuXInputGetState(ApiRef, dwUserIndex, pState);
	}
	else {
		// proxy
	...
	OutTraceIN("%s: ret=%%x(%s)\n", ApiRef, res, res==0 ? "OK" : (res==ERROR_DEVICE_NOT_CONNECTED) ? "ERROR_DEVICE_NOT_CONNECTED" : "??");
	if(res == 0) {
		OutDebugIN("%s: pState={dwPacketNumber=%d GAMEPAD buttons=%x triggers[L:R]=[%d:%d] thumb[L:R/(X,Y)=(%d,%d):(%d,%d)}\n",
			ApiRef, 
			pState->dwPacketNumber,
			pState->Gamepad.wButtons,
			pState->Gamepad.bLeftTrigger, pState->Gamepad.bRightTrigger,
			pState->Gamepad.sThumbLX, pState->Gamepad.sThumbLY, pState->Gamepad.sThumbRX, pState->Gamepad.sThumbRY
			);
	}
