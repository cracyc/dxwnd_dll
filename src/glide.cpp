...
#ifdef HOOKGLIDELIB // currently unhooked ...
...
{
	FARPROC addr;
	OutTraceDW("Remap_Glide_ProcAddress: proc=%s\n", proc);
...
{
	OutTraceSYS("grClipWindow: win=(%d,%d)-(%d,%d)\n", minx, miny, maxx, maxy);
	...
	int num_aux_buffers)
{
	...
	OutTraceSYS("grSstWinOpen: hWin=%#x res=%#x(%s)\n", hWin, res, ExplainGlideRes(res));
	...
	OutTraceSYS("grSstWinOpen: ret=%#x\n", ret);
	return ret;
}
...
FxI32 WINAPI extgrQueryResolutions (const GrResolution *resTemplate, GrResolution *output)
{
	...
	OutTraceSYS("grQueryResolutions: resolution=%#x refresh=%#x numColorBuffers=%d numAuxBuffers=%d\n", 
	...
	OutTraceSYS("grQueryResolutions: res=%d\n", ret); 
	return ret;
}

void WINAPI extgrGlideInit(void)
{
	OutTraceSYS("grGlideInit\n");
	return;
}

void WINAPI extgrGlideShutdown(void)
{
	OutTraceSYS("grGlideShutdown\n");
	return;
}
