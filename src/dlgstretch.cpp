...
static char *ExplainDialogStyle(DWORD f)
{
	...
	char sStyle[256];
	char *sep;
	strcpy(s, ExplainStyle(f, sStyle, 256));
	sep = s+strlen(s);
	strcat(s, "-DS_");
...
static char *ExplainWinClassId(DWORD cid)
{
	char *s;
	switch(cid){
		case 0x0080: s="Button";	break;
		case 0x0081: s="Edit";		break;
		case 0x0082: s="Static";	break;
		case 0x0083: s="ListBox";	break;
		case 0x0084: s="ScrollBar"; break;
		case 0x0085: s="ComboBox";  break;
		default    : s="???";	break;
	}
	return s;
}
...
#ifndef DXW_NOTRACES
	if(bLog) OutTraceDW("%s: style=%#x(%s) extstyle=%#x cdit=%d pos=(%d,%d) size=(%d,%d)\n",
		ApiRef,
...
static DWORD dxwStretchDialogEx(LPVOID lpDialog, DWORD flags)
{
...
#ifndef DXW_NOTRACES
	if(bLog) OutTraceDW("%s: ver=%d helpid=%#x style=%#x(%s) extstyle=%#x cdit=%d pos=(%d,%d) size=(%d,%d)\n",
		ApiRef,
		lpTemplate->dlgVer,
		lpTemplate->helpID,
		lpTemplate->style, ExplainDialogStyle(lpTemplate->style), 
	...
	if(lpTemplate->style & (DS_SETFONT|DS_SHELLFONT)) {
		...
		if(bStretchFont){
			...
			dwScaledFontSize = (WORD)(((long)*lpWord * dxw.iSizY) / dxw.GetScreenHeight());
#ifndef DXW_NOTRACES
			if(bLog) OutTraceDW("> scaled pointsize=%d\n", dwScaledFontSize);
#endif // DXW_NOTRACES
			*lpWord = dwScaledFontSize;
		}
		lpWord++;
...
{
	ApiName("dxwStretchDialog");
	OutTraceDW("%s: lpdialog=%#x flags=%#x\n", ApiRef, lpDialog, flags);
