...
{
	ApiName("MapDebugInformation");
	OutTrace("%s: FileHandle=%#x FileName=%s SymbolPath=%s ImageBase=%#x\n", ApiRef, FileHandle, FileName, SymbolPath, ImageBase);
...
{
	ApiName("UnmapDebugInformation");
	OutTrace("%s: DebugInfo=%#x\n", ApiRef, DebugInfo);
	return TRUE;
}

BOOL WINAPI extSymInitialize(HANDLE hProcess, PCTSTR UserSearchPath, BOOL fInvadeProcess)
{
	ApiName("SymInitialize");
	OutTrace("%s: hprocess=%#x usrsearchpath=\"%s\" invadeproc=%#x\n", ApiRef, hProcess, UserSearchPath, fInvadeProcess);
	return TRUE;
}
...
{
	ApiName("SymFunctionTableAccess");
	OutTrace("%s: hprocess=%#x addrbase=%#x\n", ApiRef, hProcess, AddrBase);
...
{
	ApiName("SymGetSymFromAddr");
	OutTrace("%s: hprocess=%#x address=%#x\n", ApiRef, hProcess, Address);
	// too dangerous, pretend we have an error
	return NULL;
}
