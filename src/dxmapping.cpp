...
void dxwCore::UpdateDesktopCoordinates()
{
	...
	if(dxw.hStickyWindow){
		RECT rect = Client;
		dxw.UnmapWindow(&rect);
		int w, h;
		w = rect.right - rect.left;
		h = rect.bottom - rect.top;
		OutTrace("UpdateDesktopCoordinates: moving sticky hwnd=%#x to pos=(%d,%d) size=(%dx%d)\n", dxw.hStickyWindow, iPosX, iPosY, iSizX, iSizY);
		if(!(*pSetWindowPos)(dxw.hStickyWindow, 0, iPosX, iPosY, iSizX, iSizY, SWP_NOOWNERZORDER)){
			OutTrace("UpdateDesktopCoordinates: sticky move failed err=%d\n", GetLastError());
		}
	}
}
...
{
	if(dwScreenWidth) *nXDest= iPosX + (((*nXDest * iSizX) + (dwScreenWidth >> 1)) / (int)dwScreenWidth);
	if(dwScreenHeight) *nYDest= iPosY + (((*nYDest * iSizY) + (dwScreenHeight >> 1)) / (int)dwScreenHeight);
}

void dxwCore::MapWindow(LPPOINT lppoint) 
{
	if(dwScreenWidth) lppoint->x = iPosX + (((lppoint->x * iSizX) + (dwScreenWidth >> 1)) / dwScreenWidth);
	if(dwScreenHeight) lppoint->y = iPosY + (((lppoint->y * iSizY) + (dwScreenHeight >> 1)) / dwScreenHeight);
}
...
{
	if(dwScreenWidth) lppoint->x= ((lppoint->x * iSizX)+(dwScreenWidth >> 1)) / (int)dwScreenWidth;
	if(dwScreenHeight) lppoint->y= ((lppoint->y * iSizY)+(dwScreenHeight >> 1)) / (int)dwScreenHeight;
}

void dxwCore::MapClient(int *nXDest, int *nYDest)
{
	if(dwScreenWidth) *nXDest= ((*nXDest * iSizX)+(dwScreenWidth >> 1)) / (int)dwScreenWidth;
	if(dwScreenHeight) *nYDest= ((*nYDest * iSizY)+(dwScreenHeight >> 1)) / (int)dwScreenHeight;
}

void dxwCore::MapClient(USHORT *nXDest, USHORT *nYDest)
{
	if(dwScreenWidth) *nXDest= (USHORT)((*nXDest * iSizX)+(dwScreenWidth >> 1)) / (int)dwScreenWidth;
	if(dwScreenHeight) *nYDest= (USHORT)((*nYDest * iSizY)+(dwScreenHeight >> 1)) / (int)dwScreenHeight;
}
...
{
	if(dwScreenWidth) *nXDest= ((*nXDest * (float)iSizX)) / (float)dwScreenWidth;
	if(dwScreenHeight) *nYDest= ((*nYDest * (float)iSizY)) / (float)dwScreenHeight;
}
...
void dxwCore::CalculateWindowPos(HWND hwnd, DWORD width, DWORD height, LPWINDOWPOS wp)
{
	...
	switch(Coordinates){
		case DXW_COLOREMULATED:
			...
			rect.left = wp->x = 0;
			rect.top = wp->y = 0;
			rect.right = wp->cx = width;
			rect.bottom = wp->cy = height;
			break;
		case DXW_DESKTOP_CENTER:
			label = "center";
			if(bAutoScale){
	...
	// no need for adjustments in fullscreen mode
	if(Coordinates == DXW_COLOREMULATED) return;
			...
			case DXW_SET_COORDINATES:
				...
				if(pos != POS_NOWHERE){
					...
					if(!IsEmpty(rIntersect)){
						switch(pos){
							case POS_BOTTOM: if(rect.bottom > rIntersect.top) rect.bottom = rIntersect.top; break;
							case POS_TOP: if(rect.top < rIntersect.bottom) rect.top = rIntersect.bottom; break;
							case POS_LEFT: if(rect.left < rIntersect.right) rect.left = rIntersect.right; break;
							case POS_RIGHT: if(rect.right > rIntersect.left) rect.right = rIntersect.left; break;
						}
					}
				}
				break;
...
void dxwCore::AutoScale()
{
	...
	if(!(*pSetWindowPos)(hWnd, 0, wp.x, wp.y, wp.cx, wp.cy, 0)){
		OutTraceE("%s: SetWindowPos ERROR err=%d @%d\n", ApiRef, GetLastError(), __LINE__);
	}
